...
while (false !== ($entry = $d->read())) {
    if (('.' != $entry) && ('..' != $entry) && ('CVS' != $entry) && (false !== mb_strstr($entry, '.php'))) {
...
if ('user' == $command) {
    ...
    if (('reset' == $action) || ((0 == count($ublocks['main'])) && (0 == count($ublocks['right'])))) {
        $ublocks['main'] = [];
    ...
    if (('reset' == $action) or ((0 == count($ublocks['main'])) and (0 == count($ublocks['right'])))) {
        $ublocks['main'] = [];
...
if ('user' == $command) {
    ...
    print_simple_header($GEDCOMS[$GEDCOM]['title']);
}
...
if ('update' == $action) {
    ...
    if (is_array($main)) {
        foreach ($main as $indexval => $b) {
            ...
            if ('' != $index) {
                unset($ublocks['main'][$index]);
            }

            $newublocks['main'][] = [$b, $config];
        }
    }
    ...
    if (is_array($right)) {
        foreach ($right as $indexval => $b) {
            ...
            if ('' != $index) {
                unset($ublocks['right'][$index]);
            }

            $newublocks['right'][] = [$b, $config];
        }
    }
...
if ('configure' == $action) {
    ...
    <?php
    if ('user' == $command) {
        print '<b>' . str2upper($pgv_lang['customize_page']) . '</b><br>';
    } else {
        print '<b>' . str2upper($pgv_lang['customize_gedcom_page']) . '</b><br>';
    }
    ...
    if ((userIsAdmin($uname)) && ('user' == $command)) {
