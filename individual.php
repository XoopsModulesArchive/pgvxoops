...
$indifacts = [];             // -- array to store the fact records in for sorting and displaying
$globalfacts = [];
$otheritems = [];              //-- notes, sources, media objects
$sex = 'U';
...
if ($bpos1) {
    ...
    while (!empty($birthrec)) {
        ...
        if ($hct > 0) {
            ...
            if ($dct > 0) {
                $hebrew_birthyear = trim($match[1]);
            }

            $dct = preg_match("/2 DATE.*(...) \d\d\d\d/", $birthrec, $match);

            if ($dct > 0) {
                if (isset($monthtonum[mb_strtolower(trim($match[1]))])) {
                    $hebrew_birthmonth = $monthtonum[mb_strtolower(trim($match[1]))];
                }
            }
            ...
            if ($dct > 0) {
                if (isset($monthtonum[mb_strtolower(trim($match[1]))])) {
                    $birthmonth = $monthtonum[mb_strtolower(trim($match[1]))];
                }
            }
...
if (userCanEdit(getUserName())) {
    if (isset($pgv_changes[$pid . '_' . $GEDCOM])) {
        ...
        for ($i = 1; $i <= $lct; $i++) {
            if ($i < $lct) {
                $line = preg_replace("/\r/", '', $indilines[$i]);
            } else {
                $line = ' ';
            }
            ...
            if (($i == $lct) || (1 == $line[0])) {
                ...
                if ('NAME' == $fact) {
                    $newglobal[] = [$linenum, $factrec];
                } // -- handle special source fact case

                elseif ('SOUR' == $fact) {
                    $newother[] = [$linenum, $factrec];
                ...
                elseif ('SEX' == $fact) {
                    $newglobal[] = [$linenum, $factrec];
                } elseif (!in_array($fact, $nonfacts, true)) {
                    $newfacts[$f] = [$linenum, $factrec];
...
for ($i = 1; $i <= $lct; $i++) {
    ...
    if (($i == $lct) || (1 == $line[0])) {
        ...
        if ('NAME' == $fact) {
            $globalfacts[$g] = [$linenum, $factrec];

            $g++;
        } // -- handle special source fact case

        elseif ('SOUR' == $fact) {
            $otheritems[$o] = [$linenum, $factrec];
...
if (isset($newfacts)) {
    ...
    for ($i = 0, $iMax = count($indifacts); $i < $iMax; $i++) {
        $found = false;
    ...
    for ($i = 0, $iMax = count($otheritems); $i < $iMax; $i++) {
        $found = false;
    ...
    for ($i = 0, $iMax = count($globalfacts); $i < $iMax; $i++) {
        $found = false;
...
for ($j = 0; $j < $ct; $j++) {
    ...
    for ($i = 1; $i <= $lct; $i++) {
        ...
        if (($i == $lct) || (1 == $line[0])) {
            ...
            if (('SOUR' != $fact) && ('OBJE' != $fact) && ('NOTE' != $fact) && ('CHAN' != $fact) && ('_UID' != $fact) && ('RIN' != $fact)) {
                if ((!in_array($fact, $nonfacts, true)) && (!in_array($fact, $nonfamfacts, true))) {
                    $factrec .= "\nPGV_SPOUSE: $spouse\n";
...
if (mb_strlen($addname) > 0) {
    print '<span class="name_head">' . PrintReady($addname) . '</span><br>';
}
if ($disp) {
    $SEX_COUNT = 0;
    ...
    foreach ($globalfacts as $key => $value) {
        ...
        if ('SEX' == $fact) {
            print_sex_record($value[1], $value[0]);
        }

        if ('NAME' == $fact) {
            print_name_record($value[1], $value[0]);
        }
    ...
    if (((!empty($birthrec)) && (!FactViewRestricted($pid, $birthrec)) && (showFactDetails('BIRT', $pid))) || ((!empty($deathrec)) && (!FactViewRestricted($pid, $deathrec)) && (showFactDetails('DEAT', $pid))) || $SHOW_LDS_AT_GLANCE) {
        print '<td valign="top"';

        if ($i < $maxi) {
            print 'colspan="' . ($maxi - $i) . '"';
        }

        print '>';

        if (!empty($birthrec) && (!FactViewRestricted($pid, $birthrec)) && (showFactDetails('BIRT', $pid))) {
            print '<span class="label">' . ($pgv_lang['BIRT'] ?? $factarray['BIRT']) . '</span> ';
        ...
        if (!empty($deathrec) && (!FactViewRestricted($pid, $deathrec)) && (showFactDetails('DEAT', $pid))) {
            print '<span class="label">' . ($pgv_lang['DEAT'] ?? $factarray['DEAT']) . '</span> ';
        ...
        if ($SHOW_LDS_AT_GLANCE) {
            print '<b>' . get_lds_glance($indirec) . '</b>';
        }
...
if ('preview' != $view) {
        ...
        if (!empty($user['gedcomid'][$GEDCOM])) {
            if ($pid == $user['gedcomid'][$GEDCOM]) {
                ...
                foreach ($famids as $indexval => $famid) {
                    ...
                    if (preg_match("/1 HUSB @$pid@/", $famrec) > 0) {
                        $canedit = true;
                    }

                    if (preg_match("/1 WIFE @$pid@/", $famrec) > 0) {
                        $canedit = true;
                    }

                    if (preg_match("/1 CHIL @$pid@/", $famrec) > 0) {
                        $canedit = true;
                    }
                }
            }
    ...
    if ($canedit && ('preview' != $view) && ($disp)) {
        ...
        if (userCanEdit($uname)) {
            ...
            if ($SEX_COUNT < 2) {
                ...
                if ('new' == $SEX_LINENUM) {
                    $submenu['onclick'] = "return add_new_record('$pid', 'SEX');";
                } else {
                    $submenu['onclick'] = "return edit_record('$pid', $SEX_LINENUM);";
                }
        ...
        if (isset($pgv_changes[$pid . '_' . $GEDCOM])) {
            ...
            if (userCanAccept(getUserName())) {
                ...
                if ('index' != $PGV_DATABASE) {
                    $submenu['link'] = "individual.php?pid=$pid&amp;action=accept";
                } else {
                    $submenu['link'] = '#';
    ...
    if ($disp && ($SHOW_GEDCOM_RECORD || $ENABLE_CLIPPINGS_CART >= getUserAccessLevel())) {
        ...
        if ($SHOW_GEDCOM_RECORD) {
            ...
            if ('yes' == $show_changes && userCanEdit(getUserName())) {
                $menu['link'] = "javascript:show_gedcom_record('new');";
            } else {
                $menu['link'] = 'javascript:show_gedcom_record();';
            }
        } else {
        ...
        if ($disp && $SHOW_GEDCOM_RECORD) {
            ...
            if ('yes' == $show_changes && userCanEdit(getUserName())) {
                $submenu['link'] = "javascript:show_gedcom_record('new');";
            } else {
                $submenu['link'] = 'javascript:show_gedcom_record();';
            }
...
?>
    ...
    if ('preview' == $view) {
        print '<span class="subheaders">' . $pgv_lang['personal_facts'] . '</span>';
    }
    //-- sort the facts
    ...
    if (!$disp) {
        ...
        if (0 == count($indifacts)) {
            print '<tr><td colspan="2" class="facts_value">' . $pgv_lang['no_tab1'] . "<script language=\"JavaScript\" type=\"text/javascript\">tabstyles[0]='tab_cell_inactive_empty'; document.getElementById('pagetab0').className='tab_cell_inactive_empty';</script></td></tr>\n";
        }

        $yetdied = false;

        foreach ($indifacts as $key => $value) {
            if (mb_stristr($value[1], '1 DEAT')) {
                $yetdied = true;
            }
            ...
            if (preg_match('/PGV_FAMILY_ID: (.*)/', $value[1], $match) > 0) {
                ...
                print_fact($value[1], $pid, $value[0], $indirec);
            }
    ...
    //-- new fact link
    if (('preview' != $view) && (userCanEdit(getUserName())) && ($disp)) {
        $addfacts = [
            'BIRT',
            'CHR',
            'DEAT',
    ...
    if ('preview' == $view) {
        print '<span class="subheaders">' . $pgv_lang['notes'] . '</span>';
    }
    print "\n\t<table class=\"facts_table\">";
    if (!$disp) {
        ...
        if (0 == $notecount) {
            print '<tr><td colspan="2" class="facts_value">' . $pgv_lang['no_tab2'] . "<script language=\"JavaScript\" type=\"text/javascript\">tabstyles[1]='tab_cell_inactive_empty'; document.getElementById('pagetab1').className='tab_cell_inactive_empty';</script></td></tr>\n";
        }
    ...
    if ($SHOW_SOURCES >= getUserAccessLevel(getUserName())) {
        ...
        if ('preview' == $view) {
            print '<span class="subheaders">' . $pgv_lang['ssourcess'] . '</span>';
        }
        ...
        if (!$disp) {
            ...
            if (0 == $sourcecount) {
                print '<tr><td colspan="2" class="facts_value">' . $pgv_lang['no_tab3'] . "<script language=\"JavaScript\" type=\"text/javascript\">tabstyles[2]='tab_cell_inactive_empty'; document.getElementById('pagetab2').className='tab_cell_inactive_empty';</script></td></tr>\n";
            }
    ...
    if (!$disp) {
        ...
        foreach ($otheritems as $key => $factrec) {
            ...
            if ('OBJE' == $fact) {
                if (preg_match('/PGV_FAMILY_ID: (.*)/', $factrec[1], $match) > 0) {
                    print_main_media($factrec[1], 1, trim($match[1]), $factrec[0]);
                } else {
        ...
        if (0 == $mediacount) {
            print '<tr><td colspan="2" class="facts_value">' . $pgv_lang['no_tab4'] . "<script language=\"JavaScript\" type=\"text/javascript\">tabstyles[3]='tab_cell_inactive_empty'; document.getElementById('pagetab3').className='tab_cell_inactive_empty';</script></td></tr>\n";
        }
    ...
    if (('preview' != $view) && (userCanEdit(getUserName())) && ($disp)) {
        if (('yes' == $show_changes) && (isset($pgv_changes[$pid . '_' . $GEDCOM]))) {
            if (empty($newrec)) {
                $newrec = find_record_in_file($pid);
            }
            ...
            for ($i = 0; $i < $ct; $i++) {
                if (!in_array($match[$i][1], $hfamids, true)) {
                    $hfamids[] = $match[$i][1];
                }
            }
        }
    ...
    if (count($hfamids) > 0) {
        ...
        for ($j = 0, $jMax = count($hfamids); $j < $jMax; $j++) {
            if (!empty($hfamids[$j])) {
                ...
                if (empty($famrec)) {
                    $famrec = find_record_in_file($hfamids[$j]);
                }
                ...
                if (userCanEdit(getUserName()) && ($disp)) {
                    if (('yes' == $show_changes) && (isset($pgv_changes[$hfamids[$j] . '_' . $GEDCOM]))) {
                ...
                if ($hparents) {
                    ...
                    if ($hparents['HUSB'] != $newhparents['HUSB']) {
                        ...
                        if (!empty($newhparents['HUSB'])) {
                            ...
                            print_pedigree_person($newhparents['HUSB'], 2, 'preview' != $view);
                    ...
                    if (!empty($hparents['HUSB'])) {
                        ...
                        if (empty($srec)) {
                            $srec = find_record_in_file($hparents['HUSB']);
                        }
                        ...
                        print_pedigree_person($hparents['HUSB'], 2, 'preview' != $view);
                    ...
                    if ($hparents['WIFE'] != $newhparents['WIFE']) {
                        ...
                        if (!empty($newhparents['WIFE'])) {
                            ...
                            print_pedigree_person($newhparents['WIFE'], 2, 'preview' != $view);
                    ...
                    if (!empty($hparents['WIFE'])) {
                        ...
                        if (empty($srec)) {
                            $srec = find_record_in_file($hparents['WIFE']);
                        }
                        ...
                        print_pedigree_person($hparents['WIFE'], 2, 'preview' != $view);
                ...
                for ($i = 0; $i < $num; $i++) {
                    ...
                    if (mb_strtoupper($spid) != mb_strtoupper($pid)) {
                        $chil[] = $spid;

                        $styleadd = '';

                        if (isset($newchil) && (!in_array($spid, $newchil, true))) {
                            $styleadd = 'red';
                        }
                        ...
                        if ($ct > 0) {
                            ...
                            print $pgv_lang['brother'];
                        }

                        print "</td><td class=\"facts_value$styleadd, person_box$isf\">";

                        print_pedigree_person($spid, 2, 'preview' != $view);
                ...
                if (isset($newchil)) {
                    for ($i = 0, $iMax = count($newchil); $i < $iMax; $i++) {
                        if ((!in_array($newchil[$i], $chil, true)) && (mb_strtoupper($newchil[$i]) != mb_strtoupper($pid))) {
                            ...
                            print_pedigree_person($newchil[$i], 2, 'preview' != $view);
                ...
                foreach ($hparents as $indexval => $parent) {
                    if (!empty($parent)) {
                        ...
                        for ($f = 0, $fMax = count($stepfamids); $f < $fMax; $f++) {
                            $num = 0;

                            //-- don't show families we already showed before

                            if (!in_array($stepfamids[$f], $hfamids, true)) {
                                $famrec = find_family_record($stepfamids[$f]);

                                if (empty($famrec)) {
                                    $famrec = find_record_in_file($stepfamids[$f]);
                                }
                                ...
                                if ($num > 0) {
                                    ...
                                    if ($stepparents) {
                                        if ($parent != $stepparents['HUSB']) {
                                            if ('M' == $parentsex) {
                                                print $pgv_lang['fathers_family_with'];
                                            } else {
                                                print $pgv_lang['mothers_family_with'];
                                            }

                                            if (!empty($stepparents['HUSB'])) {
                                                if (displayDetailsByID($stepparents['HUSB']) && showLivingNameByID($stepparents['HUSB'])) {
                                                    print PrintReady(get_person_name($stepparents['HUSB']));
                                                } else {
                                            ...
                                            if (!empty($stepparents['HUSB'])) {
                                                ...
                                                print_pedigree_person($stepparents['HUSB'], 2, 'preview' != $view);

                                                $personcount++;

                                                print "</td></tr>\n";
                                            }
                                        } elseif ($parent != $stepparents['WIFE']) {
                                            if ('F' == $parentsex) {
                                                print $pgv_lang['mothers_family_with'];
                                            } else {
                                                print $pgv_lang['fathers_family_with'];
                                            ...
                                            if (!empty($stepparents['WIFE'])) {
                                                if (displayDetailsByID($stepparents['HUSB']) && showLivingNameByID($stepparents['HUSB'])) {
                                                    print PrintReady(get_person_name($stepparents['WIFE']));
                                                } else {
                                                    print $pgv_lang['private'];
                                                }
                                            } else {
                                            ...
                                            if (!empty($stepparents['WIFE'])) {
                                                ...
                                                print_pedigree_person($stepparents['WIFE'], 2, 'preview' != $view);
    ...
    if (('preview' != $view) && (userCanEdit(getUserName())) && ($disp)) {
        if (('yes' == $show_changes) && (isset($pgv_changes[$pid . '_' . $GEDCOM]))) {
            ...
            for ($i = 0; $i < $ct; $i++) {
                if (!in_array($match[$i][1], $famids, true)) {
                    $famids[] = $match[$i][1];
                }
            }
        }
    ...
    if (count($famids) > 0) {
        ...
        for ($f = 0, $fMax = count($famids); $f < $fMax; $f++) {
            if (!empty($famids[$f])) {
                $famrec = find_family_record($famids[$f]);

                if (empty($famrec)) {
                    $famrec = find_record_in_file($famids[$f]);
                }
                ...
                if ($famrec) {
                    ...
                    if ('preview' != $view) {
                        print " - <a href=\"family.php?famid=$famids[$f]\"> [" . $pgv_lang['view_family'];
                    }

                    if ($SHOW_FAM_ID_NUMBERS) {
                        print " &lrm;($famids[$f])&lrm;";
                    }
                    ...
                    if (userCanEdit(getUserName()) && ($disp)) {
                        if (('yes' == $show_changes) && (isset($pgv_changes[$famids[$f] . '_' . $GEDCOM]))) {
                    ...
                    if ($parents) {
                        if (!empty($spid)) {
                            ...
                            if ((!is_array($newparents)) || (!in_array($spid, $newparents, true))) {
                                if ($pid != $newparents['HUSB']) {
                                    $nspid = $newparents['HUSB'];
                                } else {
                                    $nspid = $newparents['WIFE'];
                                }
                                ...
                                if (empty($srec)) {
                                    $srec = find_record_in_file($nspid);
                                }
                            ...
                            if (userCanEdit(getUserName()) && ($disp)) {
                                ...
                                print "<a href=\"#\" onclick=\"return addnewspouse('$famids[$f]', '$spousetag');\">" . $pgv_lang['add_' . mb_strtolower($spousetag) . '_to_family'] . '</a>';
                    ...
                    if (isset($newchil)) {
                        for ($i = 0, $iMax = count($newchil); $i < $iMax; $i++) {
                            if (!in_array($newchil[$i], $chil, true)) {
                                print '<tr><td class="facts_labelblue">';
    ...
    if (('preview' != $view) && (userCanEdit(getUserName())) && ($disp)) {
        ...
        print "<a href=\"#\" onclick=\"return add_famc('$pid');\">" . $pgv_lang['link_as'] . ('german' == $LANGUAGE ? $pgv_lang['child'] : str2lower($pgv_lang['child'])) . '</a>';
        ...
        if ('F' != $sex) {
            print "<a href=\"#\" onclick=\"return add_fams('$pid','HUSB');\">" . $pgv_lang['link_as'] . ('german' == $LANGUAGE ? $pgv_lang['husband'] : str2lower($pgv_lang['husband'])) . '</a>';
        ...
        if ('M' != $sex) {
            print "<a href=\"#\" onclick=\"return add_fams('$pid','WIFE');\">" . $pgv_lang['link_as'] . ('german' == $LANGUAGE ? $pgv_lang['wife'] : str2lower($pgv_lang['wife'])) . '</a>';
    ...
    if (file_exists('modules/researchlog.php') && ($SHOW_RESEARCH_LOG >= getUserAccessLevel())) {
        ...
        if ('preview' == $view) {
            print '<span class="subheaders">' . $pgv_lang['research_log'] . '</span>';
        }
        ...
        if (!$disp) {
            ...
            require_once __DIR__ . '/modules/researchlog/researchlog.php';
