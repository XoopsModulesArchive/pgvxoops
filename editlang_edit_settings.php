...
if ('new_lang' == $action) {
    ...
    $ln = mb_strtolower($lng_codes[$new_shortcut]);
...
} elseif (!isset($v_flagsfile) && isset($flagsfile[$ln])) {
    $v_flagsfile = $flagsfile[$ln];
} elseif (!isset($v_flagsfile)) {
    $v_flagsfile = '';
}
...
if ('save' != $action) {
    ...
    if ('new_lang' == $action) {
        ...
        print '<input type="hidden" name="new_old" value="old">';
    }
    ...
    if ('new_lang' != $action) {
        ...
        print '<input type="hidden" name="v_lang_use" value="' . $pgv_lang_use[$ln] . '">';
    }

    print '<tr>';

    if (!isset($v_original_lang_name)) {
        $v_original_lang_name = $pgv_lang[$ln];
    }
    ...
    if ('new_lang' != $action) {
        ...
        if ($handle = opendir($dire)) {
            ...
            while (false !== ($file = readdir($handle))) {
                $pos1 = mb_strpos($file, 'gif');

                if ('.' != $file && '..' != $file && $pos1) {
                    $filelang = mb_substr($file, 0, $pos1 - 1);
        ...
        if (file_exists($flagsfile[$ln])) {
            print '<img src="' . $flagsfile[$ln] . '" alt="" class="flag">';
        } elseif ('' != $flagsfile[$ln]) {
            print '<b class="error">' . $pgv_lang['file_does_not_exist'] . '</b>';
        }
    ...
    $dayArray = [$pgv_lang['sunday'], $pgv_lang['monday'], $pgv_lang['tuesday'], $pgv_lang['wednesday'], $pgv_lang['thursday'], $pgv_lang['friday'], $pgv_lang['saturday']];
    ...
    if (!isset($v_lang_filename)) {
        $v_lang_filename = 'languages/lang.' . $v_lang_shortcut . '.php';
    }

    if (!isset($v_config_filename)) {
        $v_config_filename = 'languages/configure_help.' . $v_lang_shortcut . '.php';
    }

    if (!isset($v_factsfile)) {
        $v_factsfile = 'languages/facts.' . $v_lang_shortcut . '.php';
    }

    if (!isset($v_helpfile)) {
        $v_helpfile = 'languages/help_text.' . $v_lang_shortcut . '.php';
    }
    ...
    if ('new_lang' != $action) {
        ...
        if (!file_exists($v_config_filename)) {
            print '&nbsp;&nbsp;<b class="error">' . $pgv_lang['file_does_not_exist'] . '</b>';
        }
        ...
        if (!file_exists($v_lang_filename)) {
            print '&nbsp;&nbsp;<b class="error">' . $pgv_lang['file_does_not_exist'] . '</b>';
        }
...
if ('save' == $action) {
    ...
    if (';' != mb_substr($lang_langcode[$ln], mb_strlen($lang_langcode[$ln]) - 1, 1)) {
        $lang_langcode[$ln] .= ';';
    }
    ...
    if ($file_array = file($Filename)) {
        ...
        if ($fp = @fopen($Filename, 'wb')) {
            for ($x = 0, $xMax = count($file_array); $x < $xMax; $x++) {
                fwrite($fp, $file_array[$x]);

                $dDummy00 = trim($file_array[$x]);

                if ('//-- NEVER manually delete or edit this entry and every line below this entry! --START--//' == $dDummy00) {
                    break;
                }
            }
            ...
            for ($x = 0, $xMax = count($file_array); $x < $xMax; $x++) {
                ...
                if ('//-- NEVER manually delete or edit this entry and every line above this entry! --END--//' == $dDummy00) {
                    fwrite($fp, "\r\n");
            ...
            print '<center class="error">' . $pgv_lang['lang_config_write_error'] . '</center><br><br>';
        }
    } else {
        print '<span class="error">' . $pgv_lang['lang_set_file_read_error'] . '</span><br><br>';
    }
    ...
    print '<input type="submit" value="' . $pgv_lang['close_window'] . '"' . " onclick=\"window.opener.showchanges('&dv=" . mt_rand() . "#a4_0'); self.close();\">";
