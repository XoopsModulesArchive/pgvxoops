...
if ('update' == $action) {
    ...
    if (!empty($fact)) {
        ...
        if (false !== $pos1) {
            ...
            if (false === $pos2) {
                $pos2 = mb_strlen($gedrec) - 1;
            }

            $oldfac = mb_substr($gedrec, $pos1, $pos2 - $pos1);
            ...
            if ($noupdfact) {
                ...
                $gedrec = mb_substr($gedrec, 0, $pos1) . "\r\n" . $factrec . mb_substr($gedrec, $pos2);
            }
        } else {
            $gedrec .= "\r\n" . $factrec;
        }
    ...
    if (!empty($MDATE) || !empty($MPLAC)) {
        if (empty($newfamid)) {
            ...
            if (0 == count($famids)) {
                ...
                if (preg_match('/1 SEX M/', $gedrec) > 0) {
                    $famrec .= "1 HUSB @$pid@\r\n";
                } else {
                    $famrec .= "1 WIFE @$pid@";
                }
        ...
        $pos1 = mb_strpos($famrec, '1 MARR');

        $noupdfact = false;

        if (false !== $pos1) {
            $pos2 = mb_strpos($famrec, "\n1 ", $pos1 + 1);

            if (false === $pos2) {
                $pos2 = mb_strlen($famrec) - 1;
            }

            $oldfac = mb_substr($famrec, $pos1, $pos2);
            ...
            if ($noupdfact) {
                ...
                $famrec = mb_substr($famrec, 0, $pos1) . "\r\n" . $factrec . mb_substr($famrec, $pos2);
            }
        } else {
            $famrec .= "\r\n" . $factrec;
        }
    ...
    if (!empty($CGIVN) || !empty($CSURN)) {
        ...
        if (empty($newfamid) && (0 == count($famids))) {
            $famrec = "0 @REF@ FAM\r\n";
            ...
            if (!isset($pgv_changes[$newfamid . '_' . $GEDCOM])) {
                $famrec = trim(find_gedcom_record($newfamid));
            } else {
                $famrec = trim(find_record_in_file($newfamid));
            }
...
?>
    ...
    <form method="post" action="edit_quickupdate.php?pid=<?php print $pid; ?>" name="quickupdate" enctype="multipart/form-data">
        ...
        <table>
            <tr>
                <td colspan="4"><b><?php print $pgv_lang['update_name']; ?></b><?php print_help_link('quick_update_name_help', 'qm'); ?></td>
            </tr>
            <tr>
                <td align="right"><?php print $factarray['GIVN'];
                    print_help_link('edit_given_name_help', 'qm'); ?></td>
                <td><input type="text" name="GIVN"></td>
                <td align="right"><?php print $factarray['SURN'];
                    print_help_link('edit_surname_help', 'qm'); ?></td>
                <td><input type="text" name="SURN"></td>
            </tr>
            <tr>
                <td colspan="4"><br></td>
            </tr>
            ...
            <tr>
                <td colspan="4"><b><?php print $pgv_lang['update_fact']; ?></b><?php print_help_link('quick_update_fact_help', 'qm'); ?></td>
            </tr>
            <tr>
                <td></td>
            ...
            <tr>
                ...
                <td align="right"><?php print $factarray['PLAC'];
                    print_help_link('edit_PLAC_help', 'qm'); ?></td>
            ...
            <?php if ($SHOW_QUICK_RESN) { ?>
                <tr>
                    <td align="right"><?php print $factarray['RESN'];
                        print_help_link('RESN_help', 'qm'); ?></td>
                    <td colspan="3">
                        <?php
            ...
            <?php if ($MULTI_MEDIA && (is_writable($MEDIA_DIRECTORY))) { ?>
                <tr>
                    <td colspan="4"><b><?php print $pgv_lang['update_photo']; ?></b><?php print_help_link('quick_update_photo_help', 'qm'); ?></td>
                </tr>
                <tr>
                    <td align="right">
            ...
            <?php if (!is_dead_id($pid)) { /*-- don't show address for dead people */ ?>
                <tr>
                    <td colspan="4"><b><?php print $pgv_lang['update_address']; ?></b><?php print_help_link('quick_update_address_help', 'qm'); ?></td>
                </tr>
                <tr>
                    <td align="right">
            ...
            <tr>
                <td colspan="4"><b><?php if (preg_match('/1 SEX M/', $gedrec) > 0) {
                            print $pgv_lang['add_new_wife'];
                        } else {
                            print $pgv_lang['add_new_husb'];
                        } ?></b><?php print_help_link('quick_update_spouse_help', 'qm'); ?></td>
            </tr>
            <tr>
                <td align="right"><?php print $factarray['GIVN'];
                    print_help_link('edit_given_name_help', 'qm'); ?></td>
                <td><input type="text" name="SGIVN"></td>
                <td align="right"><?php print $factarray['SURN'];
                    print_help_link('edit_surname_help', 'qm'); ?></td>
                <td><input type="text" name="SSURN"></td>
            </tr>
            <tr>
                <td align="right"><?php print $pgv_lang['sex'];
                    print_help_link('edit_sex_help', 'qm'); ?></td>
                <td colspan="3">
                    <select name="SSEX">
                        <option value="M"<?php if (preg_match('/1 SEX F/', $gedrec) > 0) {
                        print ' selected="selected"';
                    } ?>><?php print $pgv_lang['male']; ?></option>
                        <option value="F"<?php if (preg_match('/1 SEX M/', $gedrec) > 0) {
                        print ' selected="selected"';
                    } ?>><?php print $pgv_lang['female']; ?></option>
                    </select>
                </td>
            </tr>
            <tr>
            ...
            <tr>
                <td colspan="4"><b><?php print $pgv_lang['add_new_chil']; ?></b><?php print_help_link('quick_update_child_help', 'qm'); ?></td>
            </tr>
            <tr>
                <td align="right"><?php print $factarray['GIVN'];
                    print_help_link('edit_given_name_help', 'qm'); ?></td>
                <td><input type="text" name="CGIVN"></td>
                <td align="right"><?php print $factarray['SURN'];
                    print_help_link('edit_surname_help', 'qm'); ?></td>
                <td><input type="text" name="CSURN"></td>
            </tr>
            <tr>
                <td align="right"><?php print $pgv_lang['sex'];
                    print_help_link('edit_sex_help', 'qm'); ?></td>
