...
function roots2number()
{
        ...
        //--	nrgenstr= string that contains anchestorline so far
    {
        ...
        $levelgen[(string)$key] = $romeins[$level];

        $nrgen[(string)$key] = $nrgenstr;

        $tabbladname[(string)$key] = $tabblad;
        ...
        while ($ii < $ctf) { //--	loop for every relation
            $fams = $match[$ii][1];
        ...
        //--	level= level of anchestors
    {
        ...
        while ($ll < $maxperson) {
            ...
            $mylevel = $levelgen[(string)$key];

            $mygennum = $nrgen[(string)$key];

            $mytabblad = $tabbladname[(string)$key];
            ...
            while ($ii < $ctf) { //--	loop for every relation
                ...
                if ($stop > 0) {
                    ...
                    $personkey1[$maxperson1] = $chil;

                    $famkey1[$maxperson1] = $xfam;

                    $fatherkey1[$maxperson1] = $xfather;

                    $motherkey1[$maxperson1] = $xmother;
        ...
        while ($i < $ct) {
            ...
            if ($code < 2) {
                ...
                if ($namen == $name) {
                    ...
                    $tabbladnr[(string)$key] = $maxmulti;

                    $tabbladnrreverse[$maxmulti] = $i;

                    print('gevonden:' . $maxmulti . ':' . $key . ':' . $name . ':' . $tabbladnr[(string)$key] . '<br>');
                }
            } elseif ($tabbladnr[(string)$key] < 1) {
                //--		$person= find_person_record($key);
    ...
    while ($j <= $maxmulti) {
        ...
        $tabnr = $tabbladnr[(string)$key];

        $tabblad = $tabbladname[(string)$key];

        if ($oldtab !== $tabblad) {
            $begintab[(string)$tabblad] = $myrecord;
...
function putmylist()
{
    ...
    $refrecord[(string)$mykey] = $myrecord;
...
function get_number_date($datestr)
{
    ...
    if ('' !== $datestr) {
        ...
        for ($i = 0; $i < $ct; $i++) {
            ...
            $dstr_beg = trim(mb_strtolower(mb_substr($datestr, 0, $pos1)));
            ...
            $month = trim(mb_strtolower($match[$i][2]));
    ...
    //--then find the DATE and PLACE variables
{
    ...
    if (false !== $objerec) {
        ...
        if ($dct > 0) {
            $match1[1] = get_number_date($match1[1]);
        }
    ...
    //-- take care: Mary/Anna Groot/ is non conformant and should be Mary Anna/Groot/
{
    ...
    $strpos1 = mb_strpos($namen, ',');

    if (false !== $strpos1) {
        $strpos2 = mb_strpos($namen, ',', $strpos1 + 1);

        if ($strpos2 > 0) {
            $tussen = trim(mb_substr($namen, $strpos1 + 1, $strpos2 - $strpos1 - 1));

            $strpos1 = $strpos2;

            $initialt = mb_substr($tussen, 0, 1);
        }

        if (0 == $strpos1) {
            $surname = '';
        } else {
            $surname = mb_substr($namen, 0, $strpos1);
        }
    } else {
        return false;
    }
    ...
    $rest = trim(mb_substr($namen, $strpos1 + 1));

    $birthname = $rest;

    while (mb_strlen($rest) > 0) {
        $rest01 = mb_substr($rest, 0, 1);

        $initials .= $rest01;

        $strpos2 = mb_strpos($rest, ' ');

        if ($strpos2 > 0) {
            $rest = trim(mb_substr($rest, $strpos2 + 1));
        } else {
            $rest = '';
        }
    }
...
function addinitials($str1, $str2)
{
    ...
    while ('NULL' !== gettype($usedinitials[(string)$strnew])) {
        $stradd = mb_substr('abcdefghijklmnopqrstuvwxyz', $i, 1);
...
function slkvalue($myval)
{
    ...
    if ('' != $myval) {
        fwrite($ALLslk, 'C;X' . $posnr . ';K' . $myval . "\n");
    }
}
...
function open_slk($file)
{
    ...
    ($ALLslk = fopen($file, 'wb')) or die("error on opening $file");
...
function splitindilines($lct, $indilines)
{
    ...
    while ($i1 <= $lct) {
        ...
        $j = mb_strpos($line, ' ', $i + 1);

        $content = trim(mb_substr($line, $j));
        ...
        if ('@' == mb_substr($naam, 0, 1)) {
            $naam = mb_substr($naam, 0, 2) . 'x@';
...
function roots2excel()
{
    ...
    while ($i < $myrecord) {
        ...
        if ('' !== $myfather) {
            $absfa = $refrecord[(string)$myfather];
        }

        if ('' !== $mymother) {
            $absmo = $refrecord[(string)$mymother];
        }
        ...
        if (1 == $mytype) {
            ...
            $namen = get_sortable_name($myparent);
        }
        ...
        while ($i1 < $nrcode) {
            ...
            if ($check > 0) {
                $arelement[$check] = trim($ar1content[$str]);
            }
        }

        $arelement[pos_NOTE] .= $notes;
        ...
        if (1 == $mytype) {
            ...
            if ('M' == $arelement[pos_SEX]) {
                $arelement[pos_SEX] = 1;
            } elseif ('F' == $arelement[pos_SEX]) {
                $arelement[pos_SEX] = 2;
            } else {
                $arelement[pos_SEX] = 3;
            }
        ...
        if (1 == $mytype) {
            ...
            if ('' != $myfather) {
                slkref($i, $absfa, pos_FATHERref, pos_GENref);
            } else {
                slkstr('');
            }

            if ('' != $mymother) {
                slkref($i, $absmo, pos_MOTHERref, pos_GENref);
            } else {
                slkstr('');
            }
...
maakdefs('@Ix@:NAME', -1);
maakdefs('@Ix@:NAME:NOTE', pos_none);
maakdefs('@Ix@:NAME:NOTE:CONC', pos_none);
maakdefs('@Ix@:NAME:NOTE:CONT', pos_none);
maakdefs('@Ix@:NAME:SOUR', pos_none);
maakdefs('@Ix@:SEX', pos_SEX);
maakdefs('@Ix@:RELI:PLAC', pos_none);
maakdefs('@Ix@:CHR:DATE', pos_CHR_DATE);
maakdefs('@Ix@:CHR:PLAC', pos_none);
maakdefs('@Ix@:CHR:RELI', pos_none);
maakdefs('@Ix@:CHR:WITN', pos_none);
maakdefs('@Ix@:CHR:NOTE', pos_none);
maakdefs('@Ix@:CHR:NOTE:CONC', pos_none);
maakdefs('@Ix@:BIRT:DATE', pos_BIRT_DATE);
maakdefs('@Ix@:BIRT:PLAC', pos_BIRT_PLAC);
maakdefs('@Ix@:BIRT:NOTE', pos_none);
maakdefs('@Ix@:BIRT:NOTE:CONT', pos_none);
maakdefs('@Ix@:BIRT:TYPE', pos_none);
maakdefs('@Ix@:DEAT:DATE', pos_DEAT_DATE);
maakdefs('@Ix@:DEAT:PLAC', pos_DEAT_PLAC);
maakdefs('@Ix@:DEAT:NOTE', pos_none);
maakdefs('@Ix@:DEAT:NOTE:CONT', pos_none);
maakdefs('@Ix@:DEAT:NOTE:CONC', pos_none);
maakdefs('@Ix@:DEAT:TYPE', pos_none);
maakdefs('@Ix@:DEAT:CAUS', pos_none);
maakdefs('@Ix@:BURI:DATE', pos_none);
maakdefs('@Ix@:BURI:PLAC', pos_none);
maakdefs('@Ix@:BURI:NOTE', pos_none);
maakdefs('@Ix@:BURI:NOTE:CONT', pos_none);
maakdefs('@Ix@:BURI:TYPE', pos_none);
maakdefs('@Ix@:CHAN:DATE', pos_CHAN_DATE);
maakdefs('@Ix@:CHAN:NOTE', pos_CHAN_NOTE);
maakdefs('@Ix@:RESI:DATE', pos_none);
maakdefs('@Ix@:RESI:ROLE', pos_none);
maakdefs('@Ix@:_ORIG:DATE', pos_none);
maakdefs('@Ix@:_ORIG:PLAC', pos_none);
maakdefs('@Ix@:REFN', pos_REFN);
maakdefs('@Ix@:ADDR', pos_none);
maakdefs('@Ix@:SOUR', pos_SOUR);
maakdefs('@Ix@:OCCU', pos_OCCU);
maakdefs('@Ix@:OCCU:PLAC', pos_none);
maakdefs('@Ix@:EVEN', pos_none);
maakdefs('@Ix@:EVEN:TYPE', pos_none);
maakdefs('@Ix@:EVEN:PLAC', pos_none);
maakdefs('@Ix@:PICT', pos_PICT);
maakdefs('@Ix@:OBJE', -1);
maakdefs('@Ix@:OBJE:FORM', -1);
maakdefs('@Ix@:OBJE:FILE', -1);
maakdefs('@Ix@:FAMC', -1);
maakdefs('@Ix@:FAMS', -1);
maakdefs('@Ix@:NOTE', pos_NOTE);
maakdefs('@Ix@:NOTE:CONT', pos_NOTE);
maakdefs('@Ix@:NOTE:CONC', pos_NOTE);
maakdefs('@Fx@:MARR:DATE', pos_MARR_DATE);
maakdefs('@Fx@:MARR:PLAC', pos_MARR_PLAC);
maakdefs('@Fx@:MARB:DATE', pos_none);
maakdefs('@Fx@:MARB:PLAC', pos_none);
maakdefs('@Fx@:MARS:DATE', pos_none);
maakdefs('@Fx@:MARR:TYPE', pos_none);
maakdefs('@Fx@:MARR:RELI', pos_none);
maakdefs('@Fx@:MARB:TYPE', pos_none);
maakdefs('@Fx@:MARR:NOTE', pos_none);
maakdefs('@Fx@:MARR:NOTE:CONT', pos_none);
maakdefs('@Fx@:MARR:NOTE:CONC', pos_none);
maakdefs('@Fx@:WITN', pos_none);
maakdefs('@Fx@:DIV', pos_none);
maakdefs('@Fx@:DIV:DATE', pos_none);
maakdefs('@Fx@:DIV:PLAC', pos_none);
maakdefs('@Fx@:CHAN:DATE', pos_CHAN_DATE);
maakdefs('@Fx@:CHAN:NOTE', pos_CHAN_NOTE);
maakdefs('@Fx@:TYPE', pos_none);
maakdefs('@Fx@:_STRT:DATE', pos_none);
maakdefs('@Fx@:HUSB', -1);
maakdefs('@Fx@:WIFE', -1);
maakdefs('@Fx@:CHIL', -1);
maakdefs('@Fx@:CHIL:ADOP', pos_none);
maakdefs('@Fx@:OBJE', -1);
maakdefs('@Fx@:OBJE:FORM', -1);
maakdefs('@Fx@:OBJE:FILE', -1);
maakdefs('@Fx@:REFN', pos_REFN);
maakdefs('@Fx@:SOUR', pos_SOUR);
maakdefs('@Fx@:EVEN', pos_none);
maakdefs('@Fx@:EVEN:TYPE', pos_none);
maakdefs('@Fx@:EVEN:PLAC', pos_none);
maakdefs('@Fx@:NOTE', pos_NOTE);
maakdefs('@Fx@:NOTE:CONT', pos_NOTE);
maakdefs('@Fx@:NOTE:CONC', pos_NOTE);
