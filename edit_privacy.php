...
function search_ID_details($checkVar, $outputVar)
{
    ...
    if (empty($indirec)) {
        $indirec = find_record_in_file($checkVar);
    }
...
if ('update' == $action) {
    ...
    $configtext_beg = mb_substr($configtext, 0, mb_strpos($configtext, '//-- start person privacy --//'));

    $configtext_end = mb_substr($configtext, mb_strpos($configtext, '//-- end person privacy --//'));

    $person_privacy_text = "//-- start person privacy --//\n\$person_privacy = array();\n";

    if (!isset($v_person_privacy_del)) {
        $v_person_privacy_del = [];
    }

    if (!is_array($v_person_privacy_del)) {
        $v_person_privacy_del = [];
    }

    if (!isset($v_person_privacy)) {
        $v_person_privacy = [];
    }

    if (!is_array($v_person_privacy)) {
        $v_person_privacy = [];
    }

    foreach ($person_privacy as $key => $value) {
        if (!isset($v_person_privacy_del[$key])) {
            if (isset($v_person_privacy[$key])) {
                $person_privacy_text .= "\$person_privacy['$key'] = " . $v_person_privacy[$key] . ";\n";
            } else {
                $person_privacy_text .= "\$person_privacy['$key'] = " . $PRIVACY_CONSTANTS[$value] . ";\n";
            }
        }
    }
    ...
    $configtext_beg = mb_substr($configtext, 0, mb_strpos($configtext, '//-- start user privacy --//'));

    $configtext_end = mb_substr($configtext, mb_strpos($configtext, '//-- end user privacy --//'));

    $person_privacy_text = "//-- start user privacy --//\n\$user_privacy = array();\n";

    if (!isset($v_user_privacy_del)) {
        $v_user_privacy_del = [];
    }

    if (!is_array($v_user_privacy_del)) {
        $v_user_privacy_del = [];
    }

    if (!isset($v_user_privacy)) {
        $v_user_privacy = [];
    }

    if (!is_array($v_user_privacy)) {
        $v_user_privacy = [];
    }
    ...
    foreach ($user_privacy as $key => $value) {
        foreach ($value as $id => $setting) {
            if (!isset($v_user_privacy_del[$key][$id])) {
                if (isset($v_user_privacy[$key][$id])) {
                    $person_privacy_text .= "\$user_privacy['$key']['$id'] = " . $v_user_privacy[$key][$id] . ";\n";
                } else {
                    $person_privacy_text .= "\$user_privacy['$key']['$id'] = " . $PRIVACY_CONSTANTS[$setting] . ";\n";
                }
            }
        }
    }
    ...
    $configtext_beg = mb_substr($configtext, 0, mb_strpos($configtext, '//-- start global facts privacy --//'));

    $configtext_end = mb_substr($configtext, mb_strpos($configtext, '//-- end global facts privacy --//'));

    $person_privacy_text = "//-- start global facts privacy --//\n\$global_facts = array();\n";

    if (!isset($v_global_facts_del)) {
        $v_global_facts_del = [];
    }

    if (!is_array($v_global_facts_del)) {
        $v_global_facts_del = [];
    }

    if (!isset($v_global_facts)) {
        $v_global_facts = [];
    }

    if (!is_array($v_global_facts)) {
        $v_global_facts = [];
    }
    ...
    foreach ($global_facts as $tag => $value) {
        foreach ($value as $key => $setting) {
            if (!isset($v_global_facts_del[$tag][$key])) {
                if (isset($v_global_facts[$tag][$key])) {
                    $person_privacy_text .= "\$global_facts['$tag']['$key'] = " . $v_global_facts[$tag][$key] . ";\n";
                } else {
                    $person_privacy_text .= "\$global_facts['$tag']['$key'] = " . $PRIVACY_CONSTANTS[$setting] . ";\n";
                }
            }
        }
    }
    ...
    $configtext_beg = mb_substr($configtext, 0, mb_strpos($configtext, '//-- start person facts privacy --//'));

    $configtext_end = mb_substr($configtext, mb_strpos($configtext, '//-- end person facts privacy --//'));

    $person_privacy_text = "//-- start person facts privacy --//\n\$person_facts = array();\n";

    if (!isset($v_person_facts_del)) {
        $v_person_facts_del = [];
    }

    if (!is_array($v_person_facts_del)) {
        $v_person_facts_del = [];
    }

    if (!isset($v_person_facts)) {
        $v_person_facts = [];
    }

    if (!is_array($v_person_facts)) {
        $v_person_facts = [];
    }
                                ...
                                foreach ($value as $key => $setting) {
                                    ...
                                    <tr>
                                        ...
                                        <td class="list_value" style="text-align: center; "><?php
                                            if ('show' == $key) {
                                                print_text('fact_show');
                                            }

                                    if ('details' == $key) {
                                        print_text('fact_details');
                                    } ?></td>
                                        <td class="list_value">
