...
function indi2roots()
{
    ...
    while ($i < $ct) {
        ...
        if ($ctc > 0) {
            ...
            if (('' == $parents['WIFE']) and ('' == $parents['HUSB'])) {
                $famc = '';
            }
        }
        ...
        if ('' == $famc) {
            ...
            $orignames[(string)$key]['name'] = $value;

            $orignames[(string)$key]['file'] = $value2;
        }
    ...
    while ($i < $ct) {
        ...
        $tmpnames = preg_preg_split('/,/', $value);

        $tmpnames[0] = preg_replace(["/ [jJsS][rR]\.?,/", '/ I+,/', "/^[a-z\. ]*/"], [',', ',', ''], $tmpnames[0]);
        ...
        if ((!isset($alpha)) || ($alpha = $tmpletter)) {
            ...
            if (!isset($patrialpha[$letter])) {
                $patrialpha[$letter]['letter'] = (string)$letter;

                $patrialpha[$letter]['gid'] = (string)$key;
            } else {
...
if (true === $pass) {
    if ('@' == $alpha) {
        print '<span class="warning">' . PrintReady($pgv_lang['NN']) . '</span>';
    } else {
        print '<a href="patriarchlist.php?alpha=@&amp;surname_sublist=yes&amp;surname=@N.N.">' . PrintReady($pgv_lang['NN']) . '</a>';
    }
...
if (('yes' == $surname_sublist) && ('yes' == $show_all)) {
    ...
    foreach ($surnames as $surname => $namecount) {
        if (begRTLText($namecount['name'])) {
        ...
        } elseif ('@N.N' == mb_substr(trim($namecount['name']), 0, 4)) {
            ...
            print '<div class ="ltr"><a href="patriarchlist.php?alpha=' . $namecount['alpha'] . "&amp;surname_sublist=$surname_sublist&amp;surname=" . $namecount['name'] . '">' . $namecount['name'] . ' - &lrm;[' . ($namecount['match']) . ']&lrm;';
        }
    ...
    if ($count > 1 || $indi_hide > 0) {
        ...
        if ($count > 1) {
            print $pgv_lang['total_indis'] . ' ' . $count_indi . '&nbsp;';
        }
    ...
    foreach ($surnames as $surname => $namecount) {
        if (begRTLText($namecount['name'])) {
            ...
            print '<div class ="ltr">&nbsp;<a href="patriarchlist.php?alpha=' . $alpha . "&amp;surname_sublist=$surname_sublist&amp;surname=" . $namecount['name'] . '">' . $namecount['name'] . ' - &lrm;[' . ($namecount['match']) . ']&lrm;';
        }
    ...
    if ((!empty($surname)) && ('yes' == $surname_sublist)) {
        ...
        foreach ($tpatrilist as $gid => $fam) {
            if (mb_stristr($fam['name'], $surname)) {
                $npatrilist[$gid] = $fam;
            }
        }

        $tpatrilist = $npatrilist;
    } elseif (('no' == $surname_sublist) && ('yes' == $show_all)) {
        $tpatrilist = get_patri_list();
    } else {
        $tpatrilist = get_alpha_patri($alpha);
    }
    ...
    if (!empty($surnames)) {
        ...
        foreach ($surnames as $k => $surname) {
            print_list_person($surname['gid'], [$surname['name'], $surname['file']]);

            $i++;

            if ($i == floor($count / 2)) {
                print "</ul></td><td class=\"list_value\"><ul>\n";
            }
        }
...
if ('yes' == $show_all && '@' != $alpha) {
    if ('yes' == $surname_sublist) {
        print '<br><a href="patriarchlist.php?show_all=yes&amp;surname_sublist=no">' . $pgv_lang['skip_surnames'] . '</a>';
    } else {
        print '<br><a href="patriarchlist.php?show_all=yes&amp;surname_sublist=yes">' . $pgv_lang['show_surnames'] . '</a>';
    }
} elseif ((!isset($alpha)) || ('' == $alpha && '@' != $alpha)) {
    if ('yes' == $surname_sublist) {
        print '<br><a href="patriarchlist.php?show_all=yes&amp;surname_sublist=no">' . $pgv_lang['skip_surnames'] . '</a>';
    } else {
        print '<br><a href="patriarchlist.php?show_all=yes&amp;surname_sublist=yes">' . $pgv_lang['show_surnames'] . '</a>';
    ...
    if ('yes' == $surname_sublist) {
        print "<br><a href=\"patriarchlist.php?alpha=$alpha&amp;surname_sublist=no\">" . $pgv_lang['skip_surnames'] . '</a>';
    } else {
        print "<br><a href=\"patriarchlist.php?alpha=$alpha&amp;surname_sublist=yes\">" . $pgv_lang['show_surnames'] . '</a>';
    }
}
