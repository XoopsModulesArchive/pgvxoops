...
function print_child_descendancy($pid, $depth)
{
    ...
    if ($depth == $generations) {
        print '<img src="' . $PGV_IMAGE_DIR . '/' . $PGV_IMAGES['spacer']['other'] . "\" height=\"2\" width=\"$Dindent\" border=\"0\" alt=\"\"></td><td>\n";
    } else {
        print '<img src="' . $PGV_IMAGE_DIR . '/' . $PGV_IMAGES['hline']['other'] . "\" height=\"2\" width=\"$Dindent\" border=\"0\" alt=\"\"></td><td>\n";
    }
    ...
    foreach ($sfamids as $indexval => $sfamid) {
        ...
        if ($parents) {
            ...
            if ('' != $parid) {
                print_url_arrow($parid . $pid, "?pid=$parid&amp;generations=$generations&amp;show_full=$show_full&amp;box_width=$box_width", $pgv_lang['start_at_parents'], 2);
            }
        }
    }
    ...
    if ($depth > 0 and count($sfam) < 1) {
        ...
        print '<td class="details1" ><img src="'
              . $PGV_IMAGE_DIR
              . '/'
              . $PGV_IMAGES['plus']['other']
              . '" align="middle" hspace="0" vspace="3" border="0" style="filter:alpha(opacity=40);-moz-opacity:0.4" alt="'
              . $pgv_lang['no_children']
              . '" title="'
              . $pgv_lang['no_children']
              . '"> '
              . $pgv_lang['no_children']
...
function print_family_descendancy($pid, $famid, $depth)
{
    ...
    if ($parents) {
        ...
        print "<a href=\"#\" onclick=\"expand_layer('"
              . $famid
        ...
        if (showFact('MARR', $famid)) {
            print_simple_fact($famrec, 'MARR', $id);
        } else {
        ...
        print_pedigree_person((string)$id, 1, 'preview' != $view);
...
if ($generations > $MAX_DESCENDANCY_GENERATIONS) {
    $generations = $MAX_DESCENDANCY_GENERATIONS;
}
if (!isset($view)) {
    $view = '';
}
...
if ('preview' != $view) {
    ...
    print '<br><br><br><br></td><td class="subheaders" style="width: 60px; vertical-align:bottom;text-align: ' . ('rtl' == $TEXT_DIRECTION ? 'left' : 'right') . ';">';
...
// -- root descendancy
print '<ul style="list-style: none; display: block;" id="descendancy_chart' . ('rtl' == $TEXT_DIRECTION ? '_rtl' : '') . "\">\r\n";
