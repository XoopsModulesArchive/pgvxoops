...
function print_child_ascendancy($pid, $sosa, $depth)
{
    ...
    $new = ('' == $pid or !isset($pidarr[(string)$pid]));

    if (1 == $sosa) {
        print '<img src="' . $PGV_IMAGE_DIR . '/' . $PGV_IMAGES['spacer']['other'] . "\" height=\"2\" width=\"$Dindent\" border=\"0\"></td><td>\n";
    } else {
        print '<img src="' . $PGV_IMAGE_DIR . '/' . $PGV_IMAGES['hline']['other'] . "\" height=\"2\" width=\"$Dindent\" border=\"0\"></td><td>\n";
    }

    print_pedigree_person($pid, 1, 'preview' != $view);
    ...
    if ($sosa > 1) {
        print_url_arrow($pid, "?rootid=$pid&amp;PEDIGREE_GENERATIONS=$OLD_PGENS&amp;show_full=$show_full&amp;box_width=$box_width&amp;chart_style=$chart_style", $pgv_lang['ancestry_chart'], 3);
    }

    print '</td>';

    print '<td class="details1">&nbsp;<span class="person_box' . ((1 == $sosa) ? 'NN' : (($sosa % 2) ? 'F' : '')) . "\">&nbsp;$sosa&nbsp;</span>&nbsp;";
    ...
    if (!$new) {
        $relation = '<br>[=<a href="#sosa' . $pidarr[(string)$pid] . '">' . $pidarr[(string)$pid] . '</a> - ' . get_sosa_name($pidarr[(string)$pid]) . ']';
    } else {
        $pidarr[(string)$pid] = $sosa;
    }
    ...
    if (($parents or $SHOW_EMPTY_BOXES) and $new and $depth > 0) {
        ...
        print '<img src="'
              . $PGV_IMAGE_DIR
              . '/'
              . $PGV_IMAGES['spacer']['other']
              . "\" height=\"2\" width=\"$Dindent\" border=\"0\" align=\"middle\"><a href=\"#\" onclick=\"expand_layer('"
              . $sosa
              . "'); return false;\" class=\"top\"><img id=\""
              . $sosa
              . '_img" src="'
              . $PGV_IMAGE_DIR
              . '/'
              . $PGV_IMAGES['minus']['other']
              . '" align="middle" hspace="0" vspace="3" border="0" alt="'
              . $pgv_lang['view_family']
              . '"></a> ';
...
if (!isset($chart_style)) {
    $chart_style = 1;
}
if ('' == $chart_style) {
    $chart_style = 1;
}
if (!isset($show_cousins)) {
    $show_cousins = 1;
}
if ('' == $show_cousins) {
    $show_cousins = 0;
}
if ((!isset($PEDIGREE_GENERATIONS)) || ('' == $PEDIGREE_GENERATIONS)) {
...
if (mb_strlen($name) < 30) {
    ...
    $cellwidth = (mb_strlen($name) * 14);
}
print "\n\t<table class=\"list_table, $TEXT_DIRECTION\"><tr><td width=\"${cellwidth}px\" valign=\"top\">\n\t\t";
if ('preview' == $view) {
    print '<h2>' . str_replace('#PEDIGREE_GENERATIONS#', convert_number($PEDIGREE_GENERATIONS), $pgv_lang['gen_ancestry_chart']) . ':';
} else {
    print '<h2>' . $pgv_lang['ancestry_chart'] . ':';
}
...
if ('preview' != $view) {
    ...
    if (!$chart_style) {
        ...
        print " onclick=\"document.people.chart_style.value='1';\"";
    }
    ...
    if ($chart_style) {
        ...
        print " onclick=\"document.people.chart_style.value='0';\"";
    }
    ...
    if ($show_cousins) {
        print "1\" checked=\"checked\" onclick=\"document.people.show_cousins.value='0';\"";
    } else {
        print "0\" onclick=\"document.people.show_cousins.value='1';\"";
    }
    ...
    if ($show_full) {
        print "1\" checked=\"checked\" onclick=\"document.people.show_full.value='0';\"";
    } else {
        print "0\" onclick=\"document.people.show_full.value='1';\"";
    }
    ...
    print '</td><td class="subheaders" style="width: 60px; vertical-align:bottom;text-align: ' . ('rtl' == $TEXT_DIRECTION ? 'left' : 'right') . ';">';
...
END;
    ...
    for ($i = 0; $i < $treesize; $i++) {
        ...
        if ($pid) {
            ...
            if ($parents) {
                print_sosa_family($famids[0], $pid, $i + 1);
            } // show empty family only if it is the first and only one

            elseif (0 == $i) {
                print_sosa_family('', $pid, $i + 1);
            }
        }
    }
} else {
    ...
    print '<ul style="list-style: none; display: block;" id="ancestry_chart' . ('rtl' == $TEXT_DIRECTION ? '_rtl' : '') . "\">\r\n";
