...
if (mb_strstr($_SERVER['PHP_SELF'], 'hitcount')) {
...
if ($SHOW_COUNTER) {
    ...
    if (!file_exists($PGV_COUNTER_FILENAME)) {
        $fp = fopen($PGV_COUNTER_FILENAME, 'wb');
    ...
    if (isset($pid)) { //individual counter
        ...
        if (isset($_SESSION[$PGV_INDI_COUNTER_NAME][$pid])) {
            ...
            if ($ct > 0) { //found individual increment counter
                ...
                $fp = fopen($PGV_COUNTER_FILENAME, 'r+b');

                fwrite($fp, $l_fcontents);
        ...
        if (isset($_SESSION[$PGV_COUNTER_NAME])) {
            ...
            if ($ct) {
                ...
                if ($ct) { //found missing return & newline
                    ...
                    $l_fcontents = preg_replace("/^(\d+)/", (string)$hits, $l_fcontents);
                }
    ...
    for ($i = 0; $i < 10; $i++) {
        $hits = str_replace((string)$i, '<img src="' . $PGV_IMAGE_DIR . '/' . $PGV_IMAGES[$i]['digit'] . "\" alt='pgv_counter'>", (string)$hits);
    }
