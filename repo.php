    ...
    if ('preview' != $view) {
        if (isset($pgv_changes[$rid . '_' . $GEDCOM])) {
            if (!isset($show_changes)) {
                ...
                if (userCanAccept(getUserName())) {
                    print "<a href=\"repo.php?rid=$rid&amp;action=accept\">" . $pgv_lang['accept_all'] . '</a> | ';
                }
...
$repo = [];
if (isset($repo_id_list[$rid])) {
    $repo = $repo_id_list[$rid];
} else {
...
//-- new fact link
if (('preview' != $view) && (userCanEdit(getUserName()))) {
    $addfacts = array_merge(CheckFactUnique(['NAME', 'ADDR'], $repofacts, 'REPO'), ['PHON', 'EMAIL', 'FAX', 'WWW', 'NOTE']);
...
if ('preview' != $view) {
    ...
    if ($ENABLE_CLIPPINGS_CART >= getUserAccessLevel()) {
        print "<td align=\"center\" valign=\"top\"><span class=\"link\"><a href=\"clippings.php?action=add&amp;id=$rid&amp;type=repository\"><img class=\"icon\" src=\""
              . $PGV_IMAGE_DIR
              . '/'
              . $PGV_IMAGES['clippings']['small']
              . '" border="0" alt=""><br>'
              . $pgv_lang['add_to_cart']
              . '</a>';
