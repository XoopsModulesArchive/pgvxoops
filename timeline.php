    ...
    if ($ct > 0) {
        ...
        if ($ct > 0) {
            ...
            if ($ct > 0) {
                ...
                if (isset($familyfacts[$famid . $fact]) && ($familyfacts[$famid . $fact] != $factitem['p'])) {
                    return;
                }
            ...
            $yoffset += (($month / 12) * $scale);

            $yoffset += (($day / 30) * ($scale / 12));
            ...
            if ((0 != $date[0]['year']) && (false === mb_stristr($date[0]['ext'], 'hebrew'))) {
                print "\n\t\t<div id=\"fact$factcount\" style=\"position:absolute; "
                      . ('ltr' == $TEXT_DIRECTION ? 'left: ' . ($xoffset) : 'right: ' . ($xoffset))
                      . 'px; top:'
                      . ($yoffset)
                      . 'px; font-size: 8pt; height: '
                      . ($bheight)
                      . "px; \" onmousedown=\"factMD(this, '"
                      . $factcount
                      . "', "
                      . ($yoffset - $tyoffset)
                      . ");\">\n";
                ...
                if ($ct > 0) {
                    ...
                    if ('' !== $spouse) {
                        for ($p = 0, $pMax = count($pids); $p < $pMax; $p++) {
                            if ($pids[$p] == $spouse) {
                                break;
                            }
                        }
                ...
                print "\n\t\t<div id=\"dbox$factcount\" style=\"position:absolute; "
                      . ('ltr' == $TEXT_DIRECTION ? 'left: ' . ($basexoffset + 20) : 'right: ' . ($basexoffset + 20))
                      . 'px; top:'
                      . ($dyoffset)
                      . 'px; font-size: 8pt; height: '
                      . (abs($tyoffset))
...
if (!empty($pids[0])) {
    if (!displayDetailsByID($pids[0])) {
        if (showLivingNameByID($pids[0])) {
            print "&nbsp;<a href=\"individual.php?pid=$pids[0]\">" . PrintReady(get_person_name($pids[0])) . '</a>';
        }
...
if (!empty($newpid)) {
    ...
    if (!displayDetailsByID($newpid)) {
        if ('preview' != $view) {
            if (showLivingNameByID($newpid)) {
                print "&nbsp;<a href=\"individual.php?pid=$newpid\">" . PrintReady(get_person_name($newpid)) . '</a>';
            }
...
for ($p = 0, $pMax = count($pids); $p < $pMax; $p++) {
    ...
    if ((false !== $indirec) && (displayDetailsByID($pid))) {
        ...
        foreach ($facts as $indexval => $factrec) {
            ...
            if ($ct > 0) {
                ...
                if ($ct > 0) {
                    ...
                    if ((false === mb_stristr($date[0]['ext'], 'hebrew')) && (0 != $date[0]['year'])) {
                        if ('BIRT' == $fact) {
                        ...
                        if ($date[0]['year'] < $baseyear) {
                            $baseyear = $date[0]['year'];
                        }

                        if ($date[0]['year'] > $topyear) {
                            $topyear = $date[0]['year'];
                        }
...
for ($p = 0, $pMax = count($pids); $p < $pMax; $p++) {
    ...
    if (0 == preg_match('/[A-Za-z]+/', $pid)) {
        $pid = $GEDCOM_ID_PREFIX . $pid;
    }
    ...
    if ((false !== $indirec) && (displayDetailsByID($pid))) {
        ...
        if ('preview' != $view) {
            ...
            for ($pp = $p + 1, $ppMax = count($pids); $pp < $ppMax; $pp++) {
                print "pids[$j]=$pids[$pp]&amp;";
        ...
        if ((isset($birthyears[$pid])) && ('preview' != $view)) {
...
if (0 != $p) {
    ...
    print "\n\t\t<div id=\"line\" style=\"position:absolute; " . ('ltr' == $TEXT_DIRECTION ? 'left: ' . ($basexoffset + 20) : 'right: ' . ($basexoffset + 20)) . 'px; top:' . ($baseyoffset) . "px; \">\n";
    ...
    print "\n\t\t<div id=\"scale$baseyear\" style=\"font-family: Arial; position:absolute; " . ('ltr' == $TEXT_DIRECTION ? "left: $basexoffset" : "right: $basexoffset") . 'px; top:' . ($baseyoffset - 5) . 'px; font-size: 7pt; text-align:' . ('ltr' == $TEXT_DIRECTION ? 'left' : 'right') . ";\">\n";
    ...
    for ($i = $baseyear + 1; $i < $topyear; $i++) {
        if (0 == $i % (25 / $scale)) {
            print "\n\t\t<div id=\"scale$i\" style=\"font-family: Arial; position:absolute; "
                  . ('ltr' == $TEXT_DIRECTION ? "left: $basexoffset" : "right: $basexoffset")
                  . 'px; top:'
                  . floor($baseyoffset + (($i - $baseyear) * $scale) - $scale / 2)
                  . 'px; font-size: 7pt; text-align:'
                  . ('ltr' == $TEXT_DIRECTION ? 'left' : 'right')
                  . ";\">\n";
    ...
    print "\n\t\t<div id=\"scale$topyear\" style=\"font-family: Arial; position:absolute; "
          . ('ltr' == $TEXT_DIRECTION ? "left: $basexoffset" : "right: $basexoffset")
          . 'px; top:'
          . floor($baseyoffset + (($topyear - $baseyear) * $scale))
          . 'px; font-size: 7pt; text-align:'
          . ('ltr'
    ...
    for ($p = 0, $pMax = count($pids); $p < $pMax; $p++) {
        ...
        print "\n\t\t<div id=\"agebox$p\" style=\"position:absolute; " . ('ltr' == $TEXT_DIRECTION ? 'left: ' . ($basexoffset + 20) : 'right: ' . ($basexoffset + 20)) . 'px; top:' . $ageyoffset . 'px; height:' . $bheight . "px; visibility: hidden;\" onmousedown=\"ageMD(this, $p);\">";
    ...
    for ($c = 0, $cMax = count($pids); $c < $cMax; $c++) {
        if (isset($birthyears[$pids[$c]])) {
            print "\nbirthyears[" . $c . ']=' . $birthyears[$pids[$c]] . ';';
        }

        if (isset($birthmonths[$pids[$c]])) {
            print "\nbirthmonths[" . $c . ']=' . $birthmonths[$pids[$c]] . ';';
        }

        if (isset($birthdays[$pids[$c]])) {
