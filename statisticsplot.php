...
if ((!file_exists($PGV_BASE_DIRECTORY . 'jpgraph/jpgraph.php')) or (!file_exists($PGV_BASE_DIRECTORY . 'jpgraph/jpgraph_line.php')) or (!file_exists($PGV_BASE_DIRECTORY . 'jpgraph/jpgraph_bar.php'))) {
...
{
    $colors = ['blue', 'orange', 'red', 'brown', 'green', 'yellow'];
    ...
    if (1 == $n) {
        $accbar = new GroupBarPlot([$b[0]]);
    }

    if (2 == $n) {
        $accbar = new GroupBarPlot([$b[0], $b[1]]);
    }

    if (3 == $n) {
        $accbar = new GroupBarPlot([$b[0], $b[1], $b[2]]);
    }

    if (4 == $n) {
        $accbar = new GroupBarPlot([$b[0], $b[1], $b[2], $b[3]]);
    }

    if (5 == $n) {
        $accbar = new GroupBarPlot([$b[0], $b[1], $b[2], $b[3], $b[4]]);
    }
...
function calc_axis($xas_grenzen)
{
    ...
    while (isset($hulpar[$i])) {
        ...
        if (($hulpar[$i] - $hulpar[$i1]) == 1) {
            $xdata[$i] = (string)$hulpar[$i1];
        } else {
            $xdata[$i] = (string)$hulpar[$i1] . '-' . (string)$hulpar[$i];
        }
    ...
    $xdata[$xmax] = '>' . (string)$hulpar[$xmax1];
...
function calc_legend($grenzen_zas)
{
    ...
    while (isset($hulpar[$i])) {
        ...
        $legend[$i] = (string)$hulpar[$i1] . '-' . (string)$hulpar[$i];
    ...
    $legend[$zmax] = '>' . (string)$hulpar[$zmax1];
...
function set_params($current, $indfam, $xg, $zg, $titstr, $xt, $yt, $gx, $gz, $myfunc)
{
    ...
    if ($x_as == $current) {
        ...
        $title = $pgv_lang[(string)$titstr];

        $xtitle = $pgv_lang[(string)$xt];
        ...
        if (!function_exists($myfunc)) {
            print 'not implemented function' . $xas . '<BR>';
