...
if (('setdefault' == $action) && isset($default_ged)) {
...
<?php if (!empty($ged)) {
    print '<br>' . $pgv_lang['now_import'] . '<br><br>';
} ?>
...
if (count($GEDCOMS) > 0) {
    foreach ($GEDCOMS as $gedc => $gedarray) {
        if (userGedcomAdmin($username, $gedc)) {
            ...
            if ($DEFAULT_GEDCOM == $gedc) {
                print '<span class="label">' . PrintReady($gedarray['title']) . '</span></td>';
            } else {
                print PrintReady($gedarray['title']) . '</td>';
            }
            ...
            while ($file = readdir($dir_var)) {
                if ((mb_strpos($file, '.log') > 0) && (false !== mb_strstr($file, 'srch-' . $gedc))) {
...
print "</table>\n";
if (isset($GEDCOMS[$current_ged]) && file_exists($GEDCOMS[$current_ged]['config'])) {
    require $GEDCOMS[$current_ged]['config'];
}
