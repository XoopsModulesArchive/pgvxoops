...
$PGV_BLOCKS['print_html_block']['config'] = [
    'html' => "<p class=\"blockhc\"><b>Put your title here</b></p>\r\n<p>Click the configure button <img src=\""
              . $PGV_IMAGE_DIR
              . '/'
              . $PGV_IMAGES['admin']['small']
              . '" alt="'
              . $pgv_lang['config_block']
              . '"> to change what is printed here.</p>',
];

function print_html_block($block, $config, $side, $index)
{
    ...
    if ($ct > 0) {
        if (isset($pgv_lang[$match[1]])) {
            $config['html'] = preg_replace("/$match[0]/", $pgv_lang[$match[1]], $config['html']);
        }
    }
