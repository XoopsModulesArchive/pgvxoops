...
$PGV_BLOCKS['print_gedcom_stats']['config'] = ['show_common_surnames' => 'yes'];

//-- function to print the gedcom statistics block

function print_gedcom_stats($block, $config, $side, $index)
{
    ...
    if ($ct > 0) {
        ...
        if (empty($title)) {
            $text = str_replace('#DATE#', get_changed_date($date), $pgv_lang['gedcom_created_on']);
        } else {
            $text = $text = str_replace('#DATE#', get_changed_date($date), $pgv_lang['gedcom_created_on2']);
        }
    ...
    if ('yes' == $config['show_common_surnames']) {
        ...
        if (count($surnames) > 0) {
            ...
            foreach ($surnames as $indexval => $surname) {
                if (false === mb_stristr($surname['name'], '@N.N')) {
                    if ($i > 0) {
                        print ', ';
                    }
...
function print_gedcom_stats_config($config)
{
    ...
    <option value="yes"<?php if ('yes' == $config['show_common_surnames']) {
        print ' selected="selected"';
    } ?>><?php print $pgv_lang['yes']; ?></option>
    <option value="no"<?php if ('no' == $config['show_common_surnames']) {
        print ' selected="selected"';
