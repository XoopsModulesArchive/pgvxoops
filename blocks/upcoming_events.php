...
function print_upcoming_events($block, $config, $side, $index)
{
    ...
    if (empty($config)) {
        $config = $PGV_BLOCKS['print_upcoming_events']['config'];
    }
    ...
    if (('user' == $command) && isset($_SESSION['upcoming_events'][$command][$GEDCOM]) && (!isset($DEBUG) || (false === $DEBUG))) {
        $found_facts = $_SESSION['upcoming_events'][$command][$GEDCOM];

        $cache_load = true;
    } elseif (('gedcom' == $command) && (file_exists($INDEX_DIRECTORY . $GEDCOM . '_upcoming.php')) && (!isset($DEBUG) || (false === $DEBUG))) {
