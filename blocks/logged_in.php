 ...
 * @param $index
 */
function print_logged_in_users($block, $config, $side, $index)
{
    ...
    foreach ($users as $indexval => $user) {
        if ('Y' == $user['loggedin']) {
            if (time() - $user['sessiontime'] > $PGV_SESSION_TIME) {
                userLogout($user['username']);
            } elseif ((userIsAdmin($cusername)) || ($user['visibleonline'])) {
                $loggedusers[] = $user;
            }
        }
    }

    if (count($loggedusers) < 2) {
        return;
    }
    ...
    foreach ($loggedusers as $indexval => $user) {
        ...
        if ('ltr' == $TEXT_DIRECTION) {
            print ' &lrm; - ' . $user['username'] . '&lrm;<br>';
        } else {
            print ' &rlm; - ' . $user['username'] . '&rlm;<br>';
        }

        if ($cusername != $user['username']) {
            if ('none' != $user['contactmethod']) {
