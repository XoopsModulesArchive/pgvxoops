...
function print_user_favorites($block, $config, $side, $index)
{
    ...
    if (0 == count($userfavs)) {
        ...
        foreach ($userfavs as $key => $favorite) {
            if ('INDI' == $favorite['type']) {
                ...
                if ('user' == $command || userIsAdmin(getUserName())) {
                    print "<font size=\"1\"><a href=\"index.php?command=$command&amp;action=deletefav&amp;fv_id=" . $key . "\" onclick=\"return confirm('" . $pgv_lang['confirm_fav_remove'] . "');\">" . $pgv_lang['remove'] . "</a><br>\n";
                }
