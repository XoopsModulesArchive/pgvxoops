...
$PGV_BLOCKS['review_changes_block']['config'] = ['days' => 1, 'sendmail' => 'yes'];
 ...
 * @param mixed $index
 */
function review_changes_block($block, $config, $side, $index)
{
    ...
    if (count($pgv_changes) > 0) {
        ...
        if (userCanEdit(getUserName())) {
            ...
            if ($PGV_BLOCKS['review_changes_block']['canconfig']) {
                ...
                if ((('gedcom' == $command) && (userGedcomAdmin($username))) || (('user' == $command) && (!empty($username)))) {
                    if ('gedcom' == $command) {
            ...
            if (userCanAccept(getUserName())) {
                print "<a href=\"#\" onclick=\"window.open('edit_changes.php','','width=600,height=600,resizable=1,scrollbars=1'); return false;\">" . $pgv_lang['accept_changes'] . "</a><br>\n";
            }
            ...
            */
            ...
            foreach ($pgv_changes as $cid => $changes) {
                ...
                if ($change['gedcom'] == $GEDCOM) {
                    ...
                    if ('INDI' == $type) {
                        print '<b>' . PrintReady(get_person_name($change['gid'])) . '</b> &lrm;(' . $change['gid'] . ")&lrm;\n";
                    } elseif ('FAM' == $type) {
                        print '<b>' . PrintReady(get_family_descriptor($change['gid'])) . '</b> &lrm;(' . $change['gid'] . ")&lrm;\n";
                    } elseif ('SOUR' == $type) {
                        if ($SHOW_SOURCES >= getUserAccessLevel(getUserName())) {
                            print '<b>' . PrintReady(get_source_descriptor($change['gid'])) . '</b> &lrm;(' . $change['gid'] . ")&lrm;\n";
                        }
                    } else {
                        print '<b>' . $factarray[$type] . '</b> &lrm;(' . $change['gid'] . ")&lrm;\n";
                    }
                    ...
                    if ('INDI' == $type) {
                        print ' <a href="individual.php?pid=' . $change['gid'] . '&amp;ged=' . $change['gedcom'] . '&amp;show_changes=yes">' . $pgv_lang['view_change_diff'] . "</a>\n<br>";
                    }

                    if ('FAM' == $type) {
                        print ' <a href="family.php?famid=' . $change['gid'] . '&amp;ged=' . $change['gedcom'] . '&amp;show_changes=yes">' . $pgv_lang['view_change_diff'] . "</a>\n<br>";
                    }

                    if (('SOUR' == $type) && ($SHOW_SOURCES >= getUserAccessLevel(getUserName()))) {
                        print ' <a href="source.php?sid=' . $change['gid'] . '&amp;ged=' . $change['gedcom'] . '&amp;show_changes=yes">' . $pgv_lang['view_change_diff'] . "</a>\n<br>";
                    }
                }
            }
...
function review_changes_block_config($config)
{
    ...
    <option value="yes"<?php if ('yes' == $config['sendmail']) {
        print ' selected="selected"';
    } ?>><?php print $pgv_lang['yes']; ?></option>
    <option value="no"<?php if ('no' == $config['sendmail']) {
        print ' selected="selected"';
