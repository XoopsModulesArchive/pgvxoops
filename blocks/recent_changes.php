...
$PGV_BLOCKS['print_recent_changes']['config'] = ['days' => 30];

//-- Recent Changes block
//-- this block prints a list of changes that have occurred recently in your gedcom
function print_recent_changes($block, $config, $side, $index)
{
    ...
    $monthstart = mktime(1, 0, 0, $monthtonum[mb_strtolower($month)], $day, $year);

    $mmon = mb_strtolower(date('M', $monthstart));

    $mmon2 = mb_strtolower(date('M', $monthstart - (60 * 60 * 24 * $config['days'])));

    $mday2 = date('d', $monthstart - (60 * 60 * 24 * $config['days']));

    if ('mar' == $mmon && 'jan' == $mmon2) {
        $mmon2 = 'feb';
    }
    ...
    if ($SHOW_SOURCES >= getUserAccessLevel(getUserName())) {
        $dayrepolist = search_repos($query);
    }

    if ($SHOW_SOURCES >= getUserAccessLevel(getUserName())) {
        $daysourcelist = search_sources($query);
    }
    ...
    if ($mmon != $mmon2) {
        ...
        if ($SHOW_SOURCES >= getUserAccessLevel(getUserName())) {
            $dayrepolist2 = search_repos($query);
        }

        if ($SHOW_SOURCES >= getUserAccessLevel(getUserName())) {
            $daysourcelist2 = search_sources($query);
        }
        ...
        if ($SHOW_SOURCES >= getUserAccessLevel(getUserName())) {
            $daysourcelist = array_merge($daysourcelist, $daysourcelist2);
        }

        if ($SHOW_SOURCES >= getUserAccessLevel(getUserName())) {
            $dayrepolist = array_merge($dayrepolist, $dayrepolist2);
        }
    }
