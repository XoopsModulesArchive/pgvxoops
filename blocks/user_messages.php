...
//-- print user messages
function print_user_messages($block, $config, $side, $index)
{
        ...
        foreach ($usermessages as $key => $message) {
            ...
            if ($tempuser) {
                ...
                if ('ltr' == $TEXT_DIRECTION) {
                    print ' &lrm; - ' . htmlspecialchars($message['from'], ENT_QUOTES | ENT_HTML5) . '&lrm;';
                } else {
                    print ' &rlm; - ' . htmlspecialchars($message['from'], ENT_QUOTES | ENT_HTML5) . '&rlm;';
                }
            } else {
            ...
            $message['body'] = nl2br(htmlspecialchars($message['body'], ENT_QUOTES | ENT_HTML5));

            $message['body'] = preg_replace('/(http:.*)(<)/', '<a href="$1" dir ="ltr">$1</a>$2', $message['body']);

            print PrintReady($message['body']) . "<br><br>\n";

            if (0 == preg_match('/RE:/', $message['subject'])) {
                $message['subject'] = 'RE:' . $message['subject'];
            }

            if ($tempuser) {
                print "<a href=\"#\" onclick=\"reply('" . $message['from'] . "', '" . $message['subject'] . "'); return false;\">" . $pgv_lang['reply'] . '</a> | ';
            }
    ...
    if (count($users) > 1) {
        ...
        foreach ($users as $indexval => $user) {
            if ($username != $user['username'] && $user['verified_by_admin']) {
                ...
                if ('ltr' == $TEXT_DIRECTION) {
                    print ' &lrm; - ' . $user['username'] . "&lrm;</option>\n";
                } else {
                    print ' &rlm; - ' . $user['username'] . "&rlm;</option>\n";
                }
            }
        }
