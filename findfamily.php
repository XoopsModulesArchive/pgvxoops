...
if (count($famalpha) > 0) {
    ...
    if (true === $pass) {
        if (isset($alpha) && '@' == $alpha) {
            print '<a href="findfamily.php?alpha=@&amp;surname_sublist=yes&amp;surname=@N.N."><span class="warning">' . PrintReady($pgv_lang['NN']) . '</span></a>';
        } else {
            print '<a href="findfamily.php?alpha=@&amp;surname_sublist=yes&amp;surname=@N.N.">' . PrintReady($pgv_lang['NN']) . '</a>';
        }
    ...
    if ('yes' == $show_all) {
        print "<a href=\"findfamily.php?show_all=yes&amp;surname_sublist=$surname_sublist\"><span class=\"warning\">" . $pgv_lang['all'] . "</span>\n";
    } else {
...
if (('yes' == $surname_sublist) && ('yes' == $show_all)) {
    ...
    if ((!empty($surname)) && ('yes' == $surname_sublist) || ('no' == $surname_sublist) && (!empty($alpha))) {
        $newfamlist = [];
        ...
        if ('@N.N' != $surname) {
            ...
            foreach ($tfamlist as $gid => $fam) {
                ...
                if (!empty($sname[0]) || !empty($sname[1])) {
                    if (mb_stristr(mb_strtoupper(mb_substr($names[0], 0, mb_strpos($names[0], ','))), $sname[0])) {
                        if (isset($names[0])) {
                            $newname = $names[0] . ' + ';
                        }
                    ...
                    if (get_first_letter(trim($names[0])) == get_first_letter(trim($names[1]))) {
                        $match += 2;
                    } else {
                        $match++;
                    }
    ...
    if (('no' == $surname_sublist) && ('yes' == $show_all)) {
        ...
        foreach ($tfamlist as $gid => $fam) {
            ...
            print_list_family($gid, [$fam['name'], $fam['file']], true);
        ...
        if ($count > 1) {
            ...
            if (oneRTLText($alpha) || ('@' == $alpha && begRTLText($pgv_lang['NN']))) {
                print $pgv_lang['total_indis'] . ' &lrm;(' . mb_strtolower($pgv_lang['surname']) . ' ' . ('@' == $alpha ? $pgv_lang['NN'] : $alpha) . ')&lrm;&nbsp;&rlm;' . $match . '&rlm;&nbsp;<br>';
            } else {
                print $pgv_lang['total_indis'] . ' &rlm;(' . mb_strtolower($pgv_lang['surname']) . ' ' . ('@' == $alpha ? $pgv_lang['NN'] : $alpha) . ')&rlm;&nbsp;&lrm;' . $match . '&lrm;&nbsp;<br>';
            }
...
if ('yes' == $show_all && '@' != $alpha) {
    if ('yes' == $surname_sublist) {
        print '<br><a href="findfamily.php?show_all=yes&amp;surname_sublist=no">' . $pgv_lang['skip_surnames'] . '</a>';
    } else {
        print '<br><a href="findfamily.php?show_all=yes&amp;surname_sublist=yes">' . $pgv_lang['show_surnames'] . '</a>';
    }
} elseif (empty($alpha)) {
    if ('yes' == $surname_sublist) {
        print '<br><a href="findfamily.php?show_all=yes&amp;surname_sublist=no">' . $pgv_lang['skip_surnames'] . '</a>';
    } else {
        print '<br><a href="findfamily.php?show_all=yes&amp;surname_sublist=yes">' . $pgv_lang['show_surnames'] . "</a><br>\n";
    }
    ...
    if ('yes' == $surname_sublist) {
        print "<br><a href=\"findfamily.php?alpha=$alpha&amp;surname_sublist=no\">" . $pgv_lang['skip_surnames'] . '</a>';
    } else {
        print "<br><a href=\"findfamily.php?alpha=$alpha&amp;surname_sublist=yes\">" . $pgv_lang['show_surnames'] . '</a>';
    }
}
