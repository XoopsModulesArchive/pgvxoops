...
if ('newentry' == $action) {
    ...
    if (0 == $ct) {
        ...
        $m_media = 'M' . $pad[mb_strlen($m_id)] . $m_id;
...
if ('showmedia' == $action) {
    ...
    if (count($medialist) > 0) {
        ...
        foreach ($medialist as $indexval => $media) {
            ...
            print_list_person($media['INDI'], [get_person_name($media['INDI']), $GEDCOM]);
...
if ('showmediaform' == $action) {
    ...
    print_autopaste_link('m_ext', ['avi', 'bmp', 'gif', 'jpeg', 'mp3', 'ole', 'pcx', 'tiff', 'wav'], false);
...
if ('injectmedia' == $action) {
    ...
    if ($test) {
        ...
        foreach ($medialist as $indexval => $media) {
            ...
            $fcontents = mb_substr($fcontents, 0, $pos1) . trim($mediarec) . "\r\n" . mb_substr($fcontents, $pos1);
        ...
        for ($i = 0, $iMax = count($mappinglist); $i < $iMax; $i++) {
            $media = $mappinglist[$i];
            ...
            if ($indi != $oldindi) {
                if ($i > 0) {
                    db_replace_gedrec($oldindi, $indirec);
                }
