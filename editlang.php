...
$QUERY_STRING = preg_replace('/&&/', '&', $QUERY_STRING);
if (mb_strpos($QUERY_STRING, '&dv=')) {
    $QUERY_STRING = mb_substr($QUERY_STRING, 0, mb_strpos($QUERY_STRING, '&dv='));
}
...
if (('edit' != $action) and ('export' != $action) and ('compare' != $action) and ('config_lang' != $action) and ('new_lang' != $action)) {
    ...
    if (1 == ($cnt % 2)) {
        print '<td class="facts_value">&nbsp;</td><td class="facts_value">&nbsp;</td><td class="facts_value">&nbsp;</td></tr>';
    }
...
if (('' == $action) || ('edit' == $action) || ('bom' == $action)) {
...
if (('edit' != $action) and ('compare' != $action) and ('config_lang' != $action) and ('new_lang' != $action)) {
...
if (('edit' != $action) and ('export' != $action) and ('config_lang' != $action) and ('new_lang' != $action)) {
...
if (('edit' != $action) && ('export' != $action) && ('compare' != $action) && ('config_lang' != $action) && ('bom' != $action)) {
    print "\n<a name=\"a5_0\"></a>\n";
...
if ('edit' == $action) {
    ...
    for ($z = 0, $zMax = count($english_language_array); $z < $zMax; $z++) {
        if (isset($english_language_array[$z][1])) {
            ...
            if ('' == stripslashes(mask_all($english_language_array[$z][1]))) {
                ...
                $dummy_output .= '<i>' . stripslashes(mask_all($english_language_array[$z][1])) . '</i>';
            }
            ...
            for ($y = 0, $yMax = count($new_language_array); $y < $yMax; $y++) {
            ...
            if ((($hide_translated) and (!$found)) or (!$hide_translated)) {
                ...
                if (!$found) {
                    print "<a style=\"color: #FF0000\" href=\"#\" onclick=\"return helpPopup00('" . 'ls01=' . $z . '&amp;ls02=' . (0 - (int)$lastfound - 1) . '&amp;language2=' . $language2 . '&amp;file_type=' . $file_type . '&amp;anchor=a1_' . $z . "');\">";
                    ...
                    if ('' == stripslashes(mask_all($english_language_array[$z][1]))) {
                        ...
                        print stripslashes(mask_all($english_language_array[$z][1]));
                    }
...
if ('export' == $action) {
    ...
    for ($z = 0, $zMax = count($language_array); $z < $zMax; $z++) {
        if (isset($language_array[$z][0])) {
            if (mb_strpos($language_array[$z][0], '_help') > 0) {
                $language_array[$z][0] = mb_substr($language_array[$z][0], mb_strpos($language_array[$z][0], '"') + 1);

                $language_array[$z][0] = mb_substr($language_array[$z][0], 0, mb_strpos($language_array[$z][0], '"'));
    ...
    for ($z = 0, $zMax = count($new_language_array); $z < $zMax; $z++) {
        for ($x = 0, $xMax = count($language_array); $x < $xMax; $x++) {
            $dDummy = $new_language_array[$z][0];

            $dDummy = mb_substr($dDummy, 0, mb_strpos($dDummy, '_help'));

            if (isset($language_array[$x][0])) {
                if (mb_strpos($language_array[$x][0], '"' . $dDummy . '"') > 0) {
                    if ('config_help' != $new_language_array[$z][0]) {
                        if ('welcome_help' != $new_language_array[$z][0]) {
...
if ('compare' == $action) {
    ...
    if (file_exists($PGV_BASE_DIRECTORY . $pgv_language[$language2])) {
    ...
    if (file_exists($PGV_BASE_DIRECTORY . $factsfile[$language2])) {
        require $PGV_BASE_DIRECTORY . $factsfile[$language2];
    }
    ...
    if (file_exists($confighelpfile[$language2])) {
        ...
        if (file_exists($PGV_BASE_DIRECTORY . $confighelpfile[$language2])) {
            require $PGV_BASE_DIRECTORY . $confighelpfile[$language2];
        }
    ...
    if (file_exists($helptextfile[$language2])) {
        ...
        if (file_exists($PGV_BASE_DIRECTORY . $helptextfile[$language2])) {
            require $PGV_BASE_DIRECTORY . $helptextfile[$language2];
        }
