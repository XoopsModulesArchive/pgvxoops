...
function split_align_text($data, $maxlen)
{
    ...
    if (count($lines) > 1) {
        ...
        foreach ($lines as $indexval => $line) {
            $text .= split_align_text($line, $maxlen) . "\r\n";
        }
    ...
    foreach ($RTLOrd as $indexval => $ord) {
        if (false !== mb_strpos($data, chr($ord))) {
            $found = true;
        }
    }
    ...
    if ($found) {
        ...
        foreach ($split as $indexval => $word) {
            ...
            if (($len + $wlen) < $maxlen) {
                ...
                if (!empty($line)) {
                    $line = str_repeat(' ', $p) . (string)$line; // center alignment using spaces
...
function print_fan_chart($treeid, $fanw = 640, $fandeg = 270)
{
    ...
    $color = imagecolorallocate($image, hexdec(mb_substr($rgb, 1, 2)), hexdec(mb_substr($rgb, 3, 2)), hexdec(mb_substr($rgb, 5, 2)));
    ...
    $bgcolor = imagecolorallocate($image, hexdec(mb_substr($rgb, 1, 2)), hexdec(mb_substr($rgb, 3, 2)), hexdec(mb_substr($rgb, 5, 2)));
    ...
    $bgcolorM = imagecolorallocate($image, hexdec(mb_substr($rgb, 1, 2)), hexdec(mb_substr($rgb, 3, 2)), hexdec(mb_substr($rgb, 5, 2)));
    ...
    $bgcolorF = imagecolorallocate($image, hexdec(mb_substr($rgb, 1, 2)), hexdec(mb_substr($rgb, 3, 2)), hexdec(mb_substr($rgb, 5, 2)));
    ...
    while ($gen >= 0) {
        ...
        while ($sosa >= $p2) {
            ...
            if (!empty($pid)) {
                ...
                $bbox = imagettfbbox((float)$fontsize, 0, $fontfile, $text);
                ...
                imagettftext($image, (float)$fontsize, $tangle, $tx, $ty, $color, $fontfile, $text);
                ...
                if ($reltome) {
                    print '<br><a href="relationship.php?pid1='
                ...
                print "<br><a href=\"fanchart.php?rootid=$pid&amp;PEDIGREE_GENERATIONS=$PEDIGREE_GENERATIONS&amp;fan_width=$fan_width&amp;fan_style=$fan_style\" onmouseover=\"clear_family_box_timeout('"
                      . $pid
                      . '.'
                      . $count
                      . "');\" onmouseout=\"family_box_timeout('"
                        ...
                        for ($f = 0, $fMax = count($famids); $f < $fMax; $f++) {
                            ...
                            if ($famrec) {
                                ...
                                if ($parents) {
                                    ...
                                    if (!empty($spid)) {
                                        ...
                                        if (displayDetailsByID($spid) || showLivingNameByID($spid)) {
                                            print PrintReady(rtrim(get_person_name($spid)));
                                        } else {
                                ...
                                for ($i = 0; $i < $num; $i++) {
                                    ...
                                    if (displayDetailsByID($cpid) || showLivingNameByID($cpid)) {
                                        print PrintReady(rtrim(get_person_name($cpid)));
                                    } else {
                                        print $pgv_lang['private'];
                                    }
...
if ('preview' == $view) {
    print '<h2>' . str_replace('#PEDIGREE_GENERATIONS#', convert_number($PEDIGREE_GENERATIONS), $pgv_lang['gen_fan_chart']) . ':';
} else {
    print '<h2>' . $pgv_lang['fan_chart'] . ':';
}
