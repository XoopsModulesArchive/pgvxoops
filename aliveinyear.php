...
function check_alive($indirec, $year)
{
    ...
    if (!empty($deathrec)) {
        ...
        if ($ct > 0) {
            if (false === mb_strstr($match[1], '@#DHEBREW@')) {
    ...
    foreach ($subrecs as $ind => $subrec) {
        ...
        if ($ct > 0 && false === mb_strstr($match[0], '@#DHEBREW@')) {
            $bdate = parse_date($match[1]);

            if (!empty($bdate[0]['year'])) {
                $years[] = $bdate[0]['year'];
            }
        }
    }
...
if ('index' == $PGV_DATABASE) {
    uasort($indialpha, 'lettersort');
} else {
    uasort($indialpha, 'stringsort');
}

if (isset($alpha) && !isset($indialpha[(string)$alpha])) {
    unset($alpha);
}
...
if (count($indialpha) > 0) {
    ...
    if (true === $pass) {
        if (isset($alpha) && '@' == $alpha) {
            print "<a href=\"aliveinyear.php?year=$year&amp;alpha=@&amp;surname_sublist=yes&amp;surname=@N.N.\"><span class=\"warning\">" . PrintReady($pgv_lang['NN']) . '</span></a>';
        } else {
            print "<a href=\"aliveinyear.php?year=$year&amp;alpha=@&amp;surname_sublist=yes&amp;surname=@N.N.\">" . PrintReady($pgv_lang['NN']) . '</a>';
        }
    ...
    if ('yes' == $show_all) {
        print "<a href=\"aliveinyear.php?year=$year&amp;show_all=yes&amp;surname_sublist=$surname_sublist\"><span class=\"warning\">" . $pgv_lang['all'] . "</span>\n";
    } else {
        print "<a href=\"aliveinyear.php?year=$year&amp;show_all=yes&amp;surname_sublist=$surname_sublist\">" . $pgv_lang['all'] . "</a>\n";
    }
...
$expalpha = $alpha;
if ('(' == $expalpha || '[' == $expalpha || '?' == $expalpha || '/' == $expalpha || '*' == $expalpha || '+' == $expalpha) {
    $expalpha = '\\' . $expalpha;
}

print "<br><br><table class=\"list_table, $TEXT_DIRECTION\"><tr>";
if (('yes' == $surname_sublist) && ('yes' == $show_all)) {
    get_indi_list();
    ...
    foreach ($surnames as $surname => $namecount) {
        if (begRTLText($namecount['name'])) {
            print "<div class =\"rtl\" dir=\"rtl\">&nbsp;<a href=\"aliveinyear.php?year=$year&amp;alpha="
                  . urlencode($namecount['alpha'])
                  . "&amp;surname_sublist=$surname_sublist&amp;surname="
                  . urlencode($namecount['name'])
                  . '">&nbsp;'
                  . PrintReady($namecount['name'])
                  . '&rlm; - ['
                  . ($namecount['match'])
                  . ']&rlm;';
        } elseif ('@N.N.' == mb_substr($namecount['name'], 0, 5)) {
            ...
            print "<div class =\"ltr\" dir=\"ltr\">&nbsp;<a href=\"aliveinyear.php?year=$year&amp;alpha="
                  . urlencode($namecount['alpha'])
                  . "&amp;surname_sublist=$surname_sublist&amp;surname="
                  . urlencode($namecount['name'])
                  . '">'
    ...
    if ($count > 1 || $indi_hide > 0) {
        ...
        if ($SHOW_MARRIED_NAMES && $count > 1) {
            print $pgv_lang['total_names'] . ' ' . $count_indi . '<br>';
        }

        if ($count > 1) {
            print $pgv_lang['total_indis'] . ' ' . count($indilist) . '&nbsp;';
        }
        ...
        if ($count > 1) {
            print '<br>' . $pgv_lang['surnames'] . ' ' . $count;
        }
...
} elseif (('yes' == $surname_sublist) && (empty($surname)) && ('no' == $show_all)) {
    if (!isset($alpha)) {
        $alpha = '';
    }
            ...
            if (0 == $ret) {
                ...
                foreach ($indi['names'] as $name) {
                    if ('danish' == $LANGUAGE || 'norwegian' == $LANGUAGE) {
                        if ("\xC3\x98" == $alpha) {
                            $text = 'OE';
                        } elseif ("\xC3\x86" == $alpha) {
                            $text = 'AE';
                        } elseif ("\xC3\x85" == $alpha) {
                            $text = 'AA';
                        }

                        if (isset($text)) {
                            if ((preg_match("/^$expalpha/", $name[1]) > 0) || (preg_match("/^$text/", $name[1]) > 0)) {
                                surname_count($name[2]);
                            }
                        } elseif (preg_match("/^$expalpha/", $name[1]) > 0) {
                            surname_count($name[2]);
                        }
                    } else {
                        if (preg_match("/^$expalpha/", $name[1]) > 0) {
                            surname_count($name[2]);
                        }
                    }
                }
    ...
    if ((!empty($surname)) && ('yes' == $surname_sublist)) {
    ...
    if (('no' == $surname_sublist) && (!empty($alpha)) && ('no' == $show_all)) {
        $tindilist = get_alpha_indis($alpha);
    }

    //-- simplify processing for ALL indilist

    if (('no' == $surname_sublist) && ('yes' == $show_all)) {
            ...
            if ($indi['file'] == $GEDCOM) {
                ...
                if (0 == $ret) {
                    foreach ($indi['names'] as $indexval => $namearray) {
                        $names[] = [$namearray[0], $namearray[1], $namearray[2], $namearray[3], $gid];
                    }
        ...
        foreach ($names as $indexval => $namearray) {
            ...
            print_list_person($namearray[4], [$name, $GEDCOM]);
        ...
        if ($firstname_alpha) {
                ...
                foreach ($tindilist as $gid => $indi) {
                    foreach ($indi['names'] as $indexval => $namearray) {
                        ...
                        if (!isset($firstalpha[$letter])) {
                            $firstalpha[$letter] = ['letter' => $letter, 'ids' => $gid];
                        } else {
            ...
            if (true === $pass) {
                if (isset($falpha) && '@' == $falpha) {
                    print "<a href=\"aliveinyear.php?year=$year&amp;alpha=" . urlencode($alpha) . '&amp;surname=' . urlencode($surname) . '&amp;falpha=@&amp;surname_sublist=yes"><span class="warning">' . PrintReady($pgv_lang['NN']) . '</span></a>';
                } else {
                    print "<a href=\"aliveinyear.php?year=$year&amp;alpha=" . urlencode($alpha) . '&amp;surname=' . urlencode($surname) . '&amp;falpha=@&amp;surname_sublist=yes">' . PrintReady($pgv_lang['NN']) . '</a>';
                }
            ...
            if ('yes' == $show_all_firstnames) {
                print "<a href=\"aliveinyear.php?year=$year&amp;alpha=" . urlencode($alpha) . '&amp;surname=' . urlencode($surname) . '&amp;show_all_firstnames=no"><span class="warning">' . $pgv_lang['all'] . "</span>\n";
            } else {
                print "<a href=\"aliveinyear.php?year=$year&amp;alpha=" . urlencode($alpha) . '&amp;surname=' . urlencode($surname) . '&amp;show_all_firstnames=yes">' . $pgv_lang['all'] . "</a>\n";
            }
            ...
            if ('no' == $show_all_firstnames) {
                $findilist = [];

                $ids = preg_preg_split('/,/', $firstalpha[$falpha]['ids']);
        ...
        foreach ($tindilist as $gid => $indi) {
            ...
            if (0 == $ret) {
                foreach ($indi['names'] as $indexval => $namearray) {
                    ...
                    if ((empty($surname) && ((preg_match("/^$alpha/", $namearray[1]) > 0) || (!empty($text) && preg_match("/^$text/", $namearray[1]) > 0))) || (false !== mb_stristr($namearray[2], $surname))) {
                        $name = check_NN(sortable_name_from_name($namearray[0]));

                        $names[] = ['gid' => $gid, 'name' => $name];
                    }
                }
            } elseif ($ret < 0) {
                $indi_dead++;
        ...
        foreach ($names as $indexval => $namearray) {
            ...
            print_list_person($gid, [$name, $indi['file']]);
...
if ('@' != $alpha) {
    if ('yes' == $surname_sublist) {
        print "<br><a href=\"aliveinyear.php?year=$year&amp;alpha=$alpha&amp;surname_sublist=no&amp;show_all=$show_all\">" . $pgv_lang['skip_surnames'] . '</a>';
    } else {
        print "<br><a href=\"aliveinyear.php?year=$year&amp;alpha=$alpha&amp;surname_sublist=yes&amp;show_all=$show_all\">" . $pgv_lang['show_surnames'] . '</a>';
    }
}
if ('@' != $alpha) {
