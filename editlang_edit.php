...
if (file_exists($PGV_BASE_DIRECTORY . 'rs_functions.php')) {
    require $PGV_BASE_DIRECTORY . 'rs_functions.php';
}
...
if ('save' != $action) {
    ...
    print '<td class="facts_value" style="text-align:center; ">' . '(' . mb_substr($lang_filename, mb_strpos($lang_filename, '/') + 1) . ')' . '</td>';
    ...
    if ($ls02 > 0) {
        print stripslashes(mask_all(find_in_file($ls02, $lang_filename)));
    }
...
if ('save' == $action) {
    ...
    if ((0 == $new_message_line) || ($new_message_line > count($new_language_array))) {
        $new_message_line = count($new_language_array) - 2;
    }
    ...
    $dummyArray[3] = mb_substr($dummyArray[3], 0, $dummyArray[2]) . $new_message . '";';
    ...
    if ($Write_Ok) {
        print '<td class="facts_label03" style="color: #0000FF; font-weight: bold; ">' . print_text('original_message', 0, 1);
    } else {
    ...
    print '<input type="submit" value="' . $pgv_lang['close_window'] . '"' . " onclick=\"window.opener.showchanges('&dv=" . mt_rand() . '#' . $anchor . "'); self.close();\">";
