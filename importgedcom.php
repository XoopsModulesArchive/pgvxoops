...
if (isset($GEDCOM_FILE)) {
    if ((!mb_strstr($GEDCOM_FILE, '://')) && (!file_exists($GEDCOM_FILE))) {
...
if (1 == $stage) {
    ...
    while (!feof($fpged)) {
        ...
        while (false !== $pos1) {
            ...
            if (0 == preg_match('/1 BLOB/', $indirec)) {
        ...
        foreach ($listtype as $indexval => $type) {
            ...
            $show_table1 .= '<td class="list_value indent_rtl" style="text-align: right;">' . ('0' == $type['bytes'] ? '++' : $type['bytes']) . "</td>\n";
        ...
        $fcontents = mb_substr($fcontents, $pos2);
    }
...
if (4 == $stage) {
    ...
    print "</tr></table>\n"; ?>
    <br><br>
    <form method="post" name="marriedform" action="importgedcom.php">
        <input type="hidden" name="ged" value="<?php print $ged; ?>">
...
if (6 == $stage) {
    ...
    if ('french' == $LANGUAGE || 'italian' == $LANGUAGE) {
                    ...
                    if (isset($famlist[$match[$j][1]])) {
                        ...
                        if ($marrrec) {
                            ...
                            if (isset($indilist[$spid])) {
                                ...
                                if (false === mb_strpos($indi['gedcom'], "_MARNM $newname")) {
                                    $pos1 = mb_strpos($indi['gedcom'], '1 NAME');

                                    if (false !== $pos1) {
                                        $pos1 = mb_strpos($indi['gedcom'], "\n1", $pos1 + 1);

                                        if (false !== $pos1) {
                                            $indi['gedcom'] = mb_substr($indi['gedcom'], 0, $pos1) . "\n2 _MARNM $newname\r\n" . mb_substr($indi['gedcom'], $pos1 + 1);
                                        } else {
                                        ...
                                        $fcontents = mb_substr($fcontents, 0, $pos1) . trim($indi['gedcom']) . "\r\n" . mb_substr($fcontents, $pos2);
