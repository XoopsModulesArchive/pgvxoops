...
//-- gedcom was uploaded
if ('upload' == $action) {
    if (0 != filesize($_FILES['gedcomfile']['tmp_name'])) {
        ...
        if (!isset($GEDCOMS[basename($_FILES['gedcomfile']['name'])])) {
            ...
            $upload_errors = [print_text('file_success', 0, 1), print_text('file_too_big', 0, 1), print_text('file_too_big', 0, 1), print_text('file_partial', 0, 1), print_text('file_missing', 0, 1)];
    ...
    if ((mb_strstr($GEDCOMPATH, '://')) || (file_exists($GEDCOMPATH))) {
        ...
        if (!isset($GEDCOMS[$FILE])) {
            ...
            if (empty($gedcom_title)) {
                $ged['title'] = str_replace('#GEDCOMFILE#', $FILE, print_text('new_gedcom_title', 0, 1));
            } else {
                $ged['title'] = $gedcom_title;
            }
    ...
    <form method="post" name="configform" action="uploadgedcom.php">
        ...
        <table class="facts_table">
            <tr>
                ...
                <td class="facts_value"><input type="text" name="GEDCOMPATH" value="" size="60" dir="ltr" tabindex="<?php $i++;
        ...
        print $i ?>" onfocus="getHelp('gedcom_path_help');">
            ...
            <tr>
                ...
                <td class="facts_value"><input type="text" name="gedcom_title" value="" size="40" tabindex="<?php $i++;

        print $i ?>" onfocus="getHelp('gedcom_title_help');"></td>
            </tr>
        </table>
        <input type="submit" value="<?php print_text('add'); ?>" tabindex="<?php $i++;

        print $i ?>" onclick="closeHelp();">
    </form>
    <?php
    } elseif ('verify_upload' == $action) {
        ?>
