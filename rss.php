 ...
 */
$oldlang = $_SESSION['CLANGUAGE'] ?? 'english';
if (!empty($lang)) {
    $changelanguage = 'yes';
...
require 'config.php';
if (empty($rssStyle) || ('HTML' != $rssStyle && 'JS' != $rssStyle && 'MBOX' != $rssStyle)) {
...
if (!empty($module)) {
    if ('today' == $module) {
        $printTodays = true;
    } elseif ('upcoming' == $module) {
        $printUpcoming = true;
    } elseif ('gedcomStats' == $module) {
        $printGedcomStats = true;
    } elseif ('gedcomNews' == $module) {
        $printGedcomNews = true;
    } elseif ('top10Surnames' == $module) {
    ...
    if (0 == count($main)) {
        ...
        foreach ($main as $mname => $value) {
            if ('print_todays_events' == $value[0]) {
                $printTodays = true;
            } elseif ('print_upcoming_events' == $value[0]) {
                $printUpcoming = true;
            } elseif ('print_gedcom_stats' == $value[0]) {
                $printGedcomStats = true;
            } elseif ('print_gedcom_news' == $value[0]) {
                $printGedcomNews = true;
            } elseif ('print_block_name_top10' == $value[0]) {
                $printTop10Surnames = true;
            }
        }
    }
