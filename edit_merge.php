...
if ('choose' != $action) {
    if ($gid1 == $gid2 && $GEDCOM == $ged2) {
        ...
        if (!isset($pgv_changes[$gid1 . '_' . $GEDCOM])) {
            ...
            $gedrec1 = find_record_in_file($gid1);
        }
        ...
        if (!isset($pgv_changes[$gid2 . '_' . $ged2])) {
            ...
            $gedrec2 = find_record_in_file($gid2);
        }
        ...
        if (empty($gedrec1) || empty($gedrec2)) {
            ...
            if (!empty($type1) && ($type1 != $type2)) {
                ...
                for ($i = 0; $i < $ct; $i++) {
                    ...
                    $facts1[] = ['fact' => $fact, 'subrec' => trim($subrec)];
                }
                ...
                for ($i = 0; $i < $ct; $i++) {
                    ...
                    $facts2[] = ['fact' => $fact, 'subrec' => trim($subrec)];
                }
                ...
                if ('select' == $action) {
                    ...
                    foreach ($facts1 as $i => $fact1) {
                        if (('CHAN' != $fact1['fact']) && (!in_array($i, $skip1, true))) {
                    ...
                    foreach ($facts2 as $j => $fact2) {
                        if (('CHAN' != $fact2['fact']) && (!in_array($j, $skip2, true))) {
                    ...
                    foreach ($records as $indexval => $record) {
                        ...
                        if (false !== mb_strstr($record, "@$gid2@")) {
