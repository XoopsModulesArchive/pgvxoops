...
if (mb_strstr($_SERVER['PHP_SELF'], 'authentication')) {
...
function getUsers($field = 'username', $order = 'asc')
{
    ...
    $sql = 'SELECT * FROM ' . $TBLPREFIX . 'users ORDER BY u_' . $field . ' ' . mb_strtoupper($order);
    ...
    if ($res) {
        while (false !== ($user_row = &$res->fetchRow(DB_FETCHMODE_ASSOC))) {
            $user = [];
            ...
            if ('N' != $user_row['u_visibleonline']) {
                $user['visibleonline'] = true;
            } else {
                $user['visibleonline'] = false;
            }

            if ('N' != $user_row['u_editaccount']) {
...
function userCanEdit($username)
{
    ...
    if (isset($user['canedit'][$GEDCOM])) {
        if ('yes' == $user['canedit'][$GEDCOM] || 'edit' == $user['canedit'][$GEDCOM] || 'admin' == $user['canedit'][$GEDCOM] || 'accept' == $user['canedit'][$GEDCOM] || true === $user['canedit'][$GEDCOM]) {
            return true;
        }
            ...
            if ($table == $TBLPREFIX . 'users') {
                if ('sqlite' != $DBTYPE) {
                    ...
                    foreach ($info as $indexval => $field) {
                        if (('u_gedcomid' == $field['name']) && (preg_match('/(text)|(blob)/i', $field['type']) > 0)) {
                            $has_gedcomid = true;
                        }

                        if (('u_email' == $field['name']) && (preg_match('/(text)|(blob)/i', $field['type']) > 0)) {
                            $has_email = true;
                        }

                        if ('u_sessiontime' == $field['name']) {
                            $has_sessiontime = true;
                        }

                        if ('u_contactmethod' == $field['name']) {
                            $has_contactmethod = true;
                        }

                        if ('u_visibleonline' == $field['name']) {
                            $has_visible = true;
                        }

                        if ('u_editaccount' == $field['name']) {
                            $has_account = true;
                        }

                        if ('u_defaulttab' == $field['name']) {
                            $has_defaulttab = true;
                        }
                    }
                    ...
                    if (!$has_gedcomid) {
                        ...
                        $sql = 'ALTER TABLE '
                                . $TBLPREFIX
                                . "users ADD COLUMN (u_email TEXT, u_verified VARCHAR(20), u_verified_by_admin VARCHAR(20), u_language VARCHAR(50), u_pwrequested VARCHAR(20), u_reg_timestamp VARCHAR(50), u_reg_hashcode VARCHAR(255), u_theme VARCHAR(50), u_loggedin ENUM('Y','N'), u_sessiontime INT)";

                        $pres = dbquery($sql);
                    } elseif (!$has_sessiontime) {
                        $sql = 'ALTER TABLE ' . $TBLPREFIX . 'users ADD COLUMN (u_sessiontime INT)';

                        $pres = dbquery($sql);
                    } elseif (!$has_contactmethod) {
...
function deleteUser($username, $msg = 'deleted')
{
    ...
    if (('changed' != $msg) && ('reqested password for' != $msg) && ('verified' != $msg)) {
    ...
    if ($res) {
        while (false !== ($user_row = &$res->fetchRow(DB_FETCHMODE_ASSOC))) {
            if ($user_row) {
                ...
                if ('N' != $user_row['u_editaccount'] || $user['canadmin']) {
                    $user['editaccount'] = true;
                } else {
                    $user['editaccount'] = false;
                }
...
function addMessage($message)
{
    ...
    if ($PGV_STORE_MESSAGES && ('messaging3' != $message['method'] && 'mailto' != $message['method'] && 'none' != $message['method'])) {
    ...
    if ('messaging' != $message['method']) {
        if (!isset($message['no_from'])) {
            if (mb_stristr($from, 'phpgedview-noreply@')) {
...
function getBlocks($username)
{
    ...
    if ($res->numRows() > 0) {
        while (false !== ($row = &$res->fetchRow(DB_FETCHMODE_ASSOC))) {
            ...
            if ('main' == $row['b_location']) {
                $blocks['main'][$row['b_order']] = [$row['b_name'], unserialize($row['b_config'])];
            }

            if ('right' == $row['b_location']) {
                $blocks['right'][$row['b_order']] = [$row['b_name'], unserialize($row['b_config'])];
            }
        }
    } else {
        $user = getUser($username);
        ...
        if ($user) {
            ...
            while (false !== ($row = &$res2->fetchRow(DB_FETCHMODE_ASSOC))) {
                $row = db_cleanup($row);
