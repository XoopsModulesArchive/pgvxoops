...
-924 => DB_ERROR_CONNECT_FAILED,
     ...
     * @param mixed $result
     *
     * @return false if a result is available otherwise return false
     */
    ...
    public function prepare($query)
    {
        ...
        $newquery = mb_substr($newquery, 0, -1);
    ...
    public function &execute($stmt, $data = [])
    {
        ...
        foreach ($data as $key => $value) {
            ...
            } elseif (DB_PARAM_OPAQUE == $types[$i]) {
    ...
    {
        $sqn = mb_strtoupper($this->getSequenceName($seq_name));
        ...
        do {
            ...
            $result = &$this->query(
                "SELECT GEN_ID(${sqn}, 1) " . 'FROM RDB$GENERATORS ' . "WHERE RDB\$GENERATOR_NAME='${sqn}'"
            );
    ...
    {
        return $this->query(
            'DELETE FROM RDB$GENERATORS ' . "WHERE RDB\$GENERATOR_NAME='" . mb_strtoupper($this->getSequenceName($seq_name)) . "'"
        );
    }
    ...
    {
        $sql = 'SELECT R.RDB$CONSTRAINT_TYPE CTYPE' . ' FROM RDB$INDEX_SEGMENTS I' . '  JOIN RDB$RELATION_CONSTRAINTS R ON I.RDB$INDEX_NAME=R.RDB$INDEX_NAME' . ' WHERE I.RDB$FIELD_NAME=\'' . $field_name . '\'' . '  AND UPPER(R.RDB$RELATION_NAME)=\'' . mb_strtoupper($table_name) . '\'';
        ...
        if ($obj = @ibase_fetch_object($result)) {
            ...
            if (isset($obj->CTYPE) && 'PRIMARY KEY' == trim($obj->CTYPE)) {
                $flags .= 'primary_key ';
            }

            if (isset($obj->CTYPE) && 'UNIQUE' == trim($obj->CTYPE)) {
                $flags .= 'unique_key ';
            }
        }
        ...
        if ($obj = @ibase_fetch_object($result)) {
            ...
            if (isset($obj->FTYPE) && 261 == $obj->FTYPE) {
                $flags .= 'blob ';
            }
        }
     ...
     *
     * @param null $nativecode the error code returned by the DBMS
     *
    ...
    public function errorCode($nativecode = null)
    {
        ...
        if (!isset($error_regexps)) {
            $error_regexps = [
                '/generator .* is not defined/' => DB_ERROR_SYNTAX,  // for compat. w ibase_errcode()
