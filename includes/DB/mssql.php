...
8134 => DB_ERROR_DIVZERO,
    ...
    public function connect($dsn, $persistent = false)
    {
        ...
        if ($dsn['port']) {
            $params[0] .= (('WIN' == mb_substr(PHP_OS, 0, 3)) ? ',' : ':') . $dsn['port'];
        }
    ...
    public function nextId($seq_name, $ondemand = true)
    {
        ...
        do {
            ...
            if ($ondemand && DB::isError($result)
                && (DB_ERROR == $result->getCode() || DB_ERROR_NOSUCHTABLE == $result->getCode())) {
                $repeat = 1;
...
'flags' => $got_string ? $this->_mssql_field_flags(
    $result,
    ...
    public function _mssql_field_flags($table, $column)
    {
        ...
        if ($table != $tableName) {
            ...
            foreach ($res as $val) {
                ...
                if (mb_strpos($val['index_description'], 'primary key')) {
                    foreach ($keys as $key) {
                        $this->_add_flag($flags[$key], 'primary_key');
                    }
                } elseif (mb_strpos($val['index_description'], 'unique')) {
                    foreach ($keys as $key) {
            ...
            foreach ($res as $val) {
                ...
                if (mb_strpos($val['type_name'], 'identity')) {
                    $this->_add_flag($flags[$val['column_name']], 'auto_increment');
                }

                if (mb_strpos($val['type_name'], 'timestamp')) {
