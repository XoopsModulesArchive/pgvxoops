        ...
        if (!file_exists($dsn['database'])) {
            ...
            if (empty($dsn['fields']) || !is_array($dsn['fields'])) {
                return $this->raiseError(
                    ...
                    null,
                    null,
                    'the dbase file does not exist and ' . 'it could not be created because ' . 'the "fields" element of the DSN ' . 'is not properly set'
                );
            }
            ...
            if (!$this->connection) {
                return $this->raiseError(
                    ...
                    null,
                    null,
                    'the dbase file does not exist and ' . 'the attempt to create it failed: ' . $php_errormsg
                );
            }
        } else {
    ...
    public function tableInfo($result = null, $mode = null)
    {
        if (function_exists('dbase_get_header_info')) {
            ...
            while (!feof($db)) {
                ...
                if (mb_substr($line, 0, 1) == chr(13)) {
                    break;
                }

                $pos = mb_strpos(mb_substr($line, 0, 10), chr(0));

                $pos = (0 == $pos ? 10 : $pos);

                $id[$i] = [
                        'name' => mb_substr($line, 0, $pos),
'type' => $this->types[mb_substr($line, 11, 1)],
'length' => ord(mb_substr($line, 16, 1)),
'precision' => ord(mb_substr($line, 17, 1)),
                    ];
