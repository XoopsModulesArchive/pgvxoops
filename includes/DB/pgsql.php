...
'transactions' => true,
    ...
    public function connect($dsn, $persistent = false)
    {
        ...
        if (isset($dsn['new_link'])
            && ('true' == $dsn['new_link'] || true === $dsn['new_link'])) {
    ...
    public function fetchInto($result, &$arr, $fetchmode, $rownum = null)
    {
        ...
        $rownum = $rownum ?? $this->row[$result_int];
    ...
    public function numRows($result)
    {
        ...
        return $rows ?? $this->pgsqlRaiseError();
    }
    ...
    {
        return $this->query(
            'DROP SEQUENCE ' . $this->getSequenceName($seq_name)
        );
    }
     ...
     * @return int  an error number from a DB error constant
     */
...
'flags' => $got_string ? $this->_pgFieldFlags($id, $i, $result) : '',
    ...
    public function _pgFieldFlags($resource, $num_field, $table_name)
    {
        ...
        $result = @pg_exec(
            $this->connection,
            "SELECT f.attnotnull, f.atthasdef
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if (in_array($num_field + 1, $keys, true)) {
                $flags .= ('t' == $row[0] && 'f' == $row[1]) ? 'unique_key ' : '';

                $flags .= ('t' == $row[1]) ? 'primary_key ' : '';
