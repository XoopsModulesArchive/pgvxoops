...
'optimize' => 'performance',  // Deprecated.  Use 'portability'.
     ...
     *
     * @param null $string $string the string to quote
     *
    ...
    {
        return (null === $string) ? 'NULL' : "'" . str_replace("'", "''", $string) . "'";
    }
     ...
     *
     * @param int    $fetchmode      DB_FETCHMODE_ORDERED, DB_FETCHMODE_ASSOC
     ...
     *            $GLOBALS['xoopsDB']->queryF() if this value is 0.  (Release 1.7.0)
    ...
    {
        return $this->options[$option] ?? $this->raiseError("unknown option $option");
    }
    ...
    public function &execute($stmt, $data = [])
    {
        ...
        if (DB_OK === $result || DB::isError($result)) {
            return $result;
        }
    ...
    public function executeEmulateQuery($stmt, $data = [])
    {
        ...
        foreach ($data as $value) {
            if (DB_PARAM_SCALAR == $this->prepare_types[$stmt][$i]) {
                $realquery .= $this->quoteSmart($value);
            } elseif (DB_PARAM_OPAQUE == $this->prepare_types[$stmt][$i]) {
                $fp = @fopen($value, 'rb');
    ...
    public function &getRow(
        ...
        if (!is_array($params)) {
            if (is_array($fetchmode)) {
                $tmp = $params ?? DB_FETCHMODE_DEFAULT;
    ...
    public function &getAssoc(
        ...
        if ($cols > 2 || $force_array) {
            ...
            } elseif (DB_FETCHMODE_OBJECT == $fetchmode) {
                while (false !== ($row = $res->fetchRow(DB_FETCHMODE_OBJECT))) {
    ...
    {
        return $this->errorcode_map[$nativecode] ?? DB_ERROR;
    }
     ...
     *                                deprecated.
     * @param null          $mode     either unused or one of the tableInfo modes:
     *                                <kbd>DB_TABLEINFO_ORDERTABLE</kbd>,
