...
if (mb_strstr($_SERVER['PHP_SELF'], 'session')) {
...
if (!$CONFIGURED) {
    if ((false === mb_strstr($PHP_SELF, 'admin.php'))
        && (false === mb_strstr($PHP_SELF, 'login.php')) && (false === mb_strstr($PHP_SELF, 'editconfig.php')) && (false === mb_strstr($PHP_SELF, 'config_download.php'))
        && (false === mb_strstr($PHP_SELF, 'editconfig_help.php'))) {
...
foreach ($CONFIG_VARS as $indexval => $VAR) {
    ...
    if (in_array($VAR, $incoming, true)) {
        print "Config variable override detected. Possible hacking attempt. Script terminated.\n";

        if ((!ini_get('register_globals')) || ('Off' == ini_get('register_globals'))) {
...
if (empty($SERVER_URL)) {
    $SERVER_URL = stripslashes('http://' . $_SERVER['SERVER_NAME'] . dirname($PHP_SELF) . '/');
}
...
} elseif (floor($CONFIG_VERSION) == floor($VERSION)) {
    $GEDCOMS = [];
}
...
if (empty($_REQUEST['GEDCOM'])) {
    if (isset($_SESSION['GEDCOM'])) {
        ...
        } elseif ((empty($GEDCOM)) && (count($GEDCOMS) > 0)) {
            check_db();
 ...
 */
if (false === mb_strstr($PHP_SELF, 'client.php')) {
...
if (($ENABLE_MULTI_LANGUAGE) && (empty($_SESSION['CLANGUAGE']))) {
    ...
    if (isset($accept_langs)) {
        if (mb_strstr($accept_langs, ',')) {
            $langs_array = preg_preg_split("/(,\s*)|(;\s*)/", $accept_langs);

            for ($i = 0, $iMax = count($langs_array); $i < $iMax; $i++) {
            ...
            if (!empty($langcode[$accept_langs])) {
                $LANGUAGE = $langcode[$accept_langs];
            }
        }
    }
}
...
} elseif (!empty($HTTP_SESSION_VARS['CLANGUAGE'])) {
...
if ($ENABLE_MULTI_LANGUAGE) {
    if ((isset($changelanguage)) && ('yes' == $changelanguage)) {
...
if (isset($pgv_language[$LANGUAGE]) && (file_exists($PGV_BASE_DIRECTORY . $pgv_language[$LANGUAGE]))) {
    require_once $PGV_BASE_DIRECTORY . $pgv_language[$LANGUAGE];
}    //-- load language file
...
if (isset($SHOW_CONTEXT_HELP) && 'yes' === $show_context_help) {
    $_SESSION['show_context_help'] = true;
}
if (isset($SHOW_CONTEXT_HELP) && 'no' === $show_context_help) {
    $_SESSION['show_context_help'] = false;
}
if (!isset($USE_THUMBS_MAIN)) {
    $USE_THUMBS_MAIN = false;
}
if ((false === mb_strstr($PHP_SELF, 'editconfig.php'))
    && (false === mb_strstr($PHP_SELF, 'editconfig_help.php'))) {
    ...
    if (($CONFIG_VERSION < $REQUIRED_CONFIG_VERSION)
        && (false === mb_strstr($PHP_SELF, 'login.php')) && (false === mb_strstr($PHP_SELF, 'admin.php')) && (false === mb_strstr($PHP_SELF, 'help_text.php'))
        && (false === mb_strstr($PHP_SELF, 'upgrade.php'))) {
        header('Location: upgrade.php');

        exit;
    }

    if ((false === mb_strstr($PHP_SELF, 'editconfig_gedcom.php'))
        && (false === mb_strstr($PHP_SELF, 'help_text.php')) && (false === mb_strstr($PHP_SELF, 'editconfig_help.php')) && (false === mb_strstr($PHP_SELF, 'editgedcoms.php')) && (false === mb_strstr($PHP_SELF, 'uploadgedcom.php')) && (false === mb_strstr($PHP_SELF, 'login.php')) && (false === mb_strstr($PHP_SELF, 'admin.php')) && (false === mb_strstr($PHP_SELF, 'config_download.php')) && (false === mb_strstr($PHP_SELF, 'addnewgedcom.php')) && (false === mb_strstr($PHP_SELF, 'validategedcom.php')) && (false === mb_strstr($PHP_SELF, 'addmedia.php')) && (false === mb_strstr($PHP_SELF, 'importgedcom.php')) && (false === mb_strstr($PHP_SELF, 'client.php')) && (false === mb_strstr($PHP_SELF, 'edit_privacy.php'))
        && (false === mb_strstr($PHP_SELF, 'useradmin.php'))) {
        if ((0 == count($GEDCOMS)) || (!check_for_import($GEDCOM))) {
    ...
    if ($REQUIRE_AUTHENTICATION) {
        if (empty($pgv_username)) {
            if ((false === mb_strstr($PHP_SELF, 'login.php'))
                && (false === mb_strstr($PHP_SELF, 'login_register.php')) && (false === mb_strstr($PHP_SELF, 'client.php')) && (false === mb_strstr($PHP_SELF, 'help_text.php'))
                && (false === mb_strstr($PHP_SELF, 'message.php'))) {
                $url = basename($_SERVER['PHP_SELF']) . '?' . $QUERY_STRING;

                if (false !== mb_stristr($url, 'index.php')) {
                    if (false === mb_stristr($url, 'command=')) {
                        if ((!isset(Request::getString('HTTP_REFERER', '', 'SERVER'))) || (false === mb_stristr(Request::getString('HTTP_REFERER', '', 'SERVER'), $SERVER_URL))) {
                            $url .= '&command=gedcom';
                        }
                    }
                }

                if (false === mb_stristr($url, 'ged=')) {
                    $url .= '&ged=' . $GEDCOM;
                }
...
if (file_exists($PGV_BASE_DIRECTORY . $THEME_DIR . 'theme.php')) {
    require_once $PGV_BASE_DIRECTORY . $THEME_DIR . 'theme.php';
} else {
...
if ($Languages_Default) {                    // If Languages not yet configured
    ...
    $pgv_lang_use[(string)$LANGUAGE]              = true;        //     and enable according to Browser pref.
    $language_settings['english']['pgv_lang_use'] = false;

    $language_settings[(string)$LANGUAGE]['pgv_lang_use'] = true;
}
