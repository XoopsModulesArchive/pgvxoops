...
function print_place_subfields($element_id)
{
    ...
    if ('DEAT_PLAC' == $element_id) {
        return;
    } // known bug - waiting for a patch
    ...
    $HEAD_PLAC_FORM = mb_substr($HEAD_PLAC_FORM, 6);

    if (empty($HEAD_PLAC_FORM)) {
        $HEAD_PLAC_FORM = $pgv_lang['default_form'];
    }

    $plac_label = preg_preg_split('/,/', $HEAD_PLAC_FORM);

    $plac_label = array_reverse($plac_label);

    if ($HEAD_PLAC_FORM == $pgv_lang['default_form']) {
        $plac_label[0] = $factarray['CTRY'];
    } ?>
    ...
    <script type="text/javascript">
        ...
        function splitplace(place_tag) {
            ...
            var place_array = place_value.preg_split(",");
        ...
        function setPlaceState(txt) {
            ...
            document.getElementsByName('PLAC_STAE_div')[0].style.height = 'auto';
            p = txt.indexOf(' (');
            if (1 < p) txt = txt.substring(0, p); // remove code (XX)
    ...
    for ($i = 0, $iMax = count($plac_label); $i < $iMax; $i++) {
        ...
        if (in_array($plac_label[$i], ['Country', $factarray['CTRY']], true)) {
            $cols = '8';
        ...
        if ($i == $istate) {
            $subtagname = 'PLAC_STAE';
        }

        if ($i == $icounty) {
            $subtagname = 'PLAC_CNTY';
        }

        if ($i == $icity) {
            $subtagname = 'PLAC_CITY';
        }

        $key = mb_strtolower($plac_label[$i]);
        ...
        if ($icountry < $i and $i <= $icity) {
            print ' acdropdown="true" autocomplete_list="url:places/getdata.php?field=' . $subtagname . '&amp;s=" autocomplete="off" autocomplete_matchbegin="false"';
        }
        ...
        if ($i == $icountry) {
            ...
            if ($icountry < $i and $i <= $icity) {
                print ' <img id="' . $subtagid . '_auto" name="' . $subtagname . '_auto" src="images/autocomplete.gif" alt="autocompletion active" title="autocompletion active"> ';
            }
        ...
        if ($i < $icountry or $i > $icounty) {
            ...
            print "<div id='"
                  ...
                  . "_div' name='"
                  . $subtagname
                  . "_div' style='overflow:hidden; height:32px; width:auto; border-width:thin; border-style:none;'><img name='"
                  . $subtagname
                  . "_img' src='images/spacer.gif' usemap='usemap' border='0' alt='' title='' style='height:inherit; width:inherit;'></div>";
        }
    }
