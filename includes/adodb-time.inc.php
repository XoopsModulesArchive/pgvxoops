...
function adodb_date_test()
{
    ...
    print ' 15 Oct 1582, Fri=' . (5 == adodb_dow(1582, 10, 15) ? 'Fri' : '<b>Error</b>') . '<br>';

    print ' 4 Oct 1582, Thu=' . (4 == adodb_dow(1582, 10, 4) ? 'Thu' : '<b>Error</b>') . '<br>';
...
&nbsp; \"$s1\" (date len=" . mb_strlen($s1) . ")
&nbsp; \"$s2\" (adodb_date len=" . mb_strlen($s2) . ')</b></pre><br>';
    ...
    for ($i = 0; $i < $max; $i++) {
        switch ($fmt[$i]) {
            ...
            case 'Y':
                ...
                break;
            case 'y':
                $dates .= mb_substr($year, mb_strlen($year) - 2, 2);
                break;
            // MONTH
            case 'm':
                if ($month < 10) {
