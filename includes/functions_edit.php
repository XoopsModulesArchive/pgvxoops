...
$NPFX_accept = ['Adm', 'Amb', 'Brig', 'Can', 'Capt', 'Chan', 'Chapln', 'Cmdr', 'Col', 'Cpl', 'Cpt', 'Dr', 'Gen', 'Gov', 'Hon', 'Lady', 'Lt', 'Mr', 'Mrs', 'Ms', 'Msgr', 'Pfc', 'Pres', 'Prof', 'Pvt', 'Rep', 'Rev', 'Sen', 'Sgt', 'Sir', 'Sr', 'Sra', 'Srta', 'Ven'];
$SPFX_accept = ['al', 'da', 'de', 'den', 'dem', 'der', 'di', 'du', 'el', 'la', 'van', 'von'];
$NSFX_accept = ['Jr', 'Sr', 'I', 'II', 'III', 'IV', 'MD', 'PhD'];
$FILE_FORM_accept = ['avi', 'bmp', 'gif', 'jpeg', 'mp3', 'ole', 'pcx', 'tiff', 'wav'];
$emptyfacts = [
    'BIRT',
    'CHR',
    'DEAT',
...
function replace_gedrec($gid, $gedrec, $chan = true)
{
    ...
    $pos1 = mb_strpos($fcontents, "0 @$gid@");
    ...
    if (false !== ($gedrec = check_gedcom($gedrec, $chan))) {
        $pos2 = mb_strpos($fcontents, "\n0", $pos1 + 1);

        if (false === $pos2) {
            $undo = mb_substr($fcontents, $pos1);

            $fcontents = mb_substr($fcontents, 0, $pos1) . "\r\n" . trim($gedrec) . "\r\n0 TRLR\r\n";
        } else {
            $pos2++;

            $undo = mb_substr($fcontents, $pos1, $pos2 - $pos1);
        ...
        if (!isset($pgv_changes[$gid . '_' . $GEDCOM])) {
            $pgv_changes[$gid . '_' . $GEDCOM] = [];
        }

        $pgv_changes[$gid . '_' . $GEDCOM][] = $change;

        if (!isset($manual_save) || (false === $manual_save)) {
...
function delete_gedrec($gid)
{
    ...
    if (false === $pos2) {
        $pos2 = mb_strpos($fcontents, '0 TRLR', $pos1 + 1);
    } else {
        $pos2++;
    }
    ...
    $fcontents = mb_substr($fcontents, 0, $pos1) . mb_substr($fcontents, $pos2);
...
function check_gedcom($gedrec, $chan = true)
{
    ...
    if ($chan) {
        $pos1 = mb_strpos($gedrec, '1 CHAN');

        if (false !== $pos1) {
            $pos2 = mb_strpos($gedrec, "\n1", $pos1 + 4);

            if (false === $pos2) {
                $pos2 = mb_strlen($gedrec);
            }

            $newgedrec = mb_substr($gedrec, 0, $pos1);
            ...
            $newgedrec .= mb_substr($gedrec, $pos2);
...
function undo_change($cid, $index)
{
    ...
    if (isset($pgv_changes[$cid])) {
        ...
        if ('delete' == $change['type']) {
            ...
            $fcontents = mb_substr($fcontents, 0, $pos1) . trim($change['undo']) . "\r\n" . mb_substr($fcontents, $pos1);
        } elseif ('append' == $change['type']) {
            $pos1 = mb_strpos($fcontents, '0 @' . $change['gid'] . '@');
        ...
        if (0 == $index) {
            ...
            for ($i = $index, $iMax = count($pgv_changes[$cid]); $i < $iMax; $i++) {
                unset($pgv_changes[$cid][$i]);
            }

            if (0 == count($pgv_changes[$cid])) {
                unset($pgv_changes[$cid]);
            }
        }
...
function write_file()
{
    ...
    if (0 == preg_match('/0 TRLR/', $fcontents)) {
        $fcontents .= "0 TRLR\n";
    }
...
function print_indi_form($nextaction, $famid, $linenum = '', $namerec = '', $famtag = 'CHIL')
{
    ...
    if (empty($namerec)) {
        ...
        if ('CHIL' == $famtag and 'addchildaction' == $nextaction) {
        ...
        if ('HUSB' == $famtag and 'addnewparentaction' == $nextaction) {
    ...
    if (mb_strlen($npfx) and false === mb_strpos($name, $npfx)) {
        $name = $npfx . ' ' . $name;
    }
    ...
    if (empty($namerec)) {
        ...
        if ('NEW' != $namerec) {
            ...
            $fields = preg_preg_split("/\s/", $gedlines[0]);
            ...
            $namefacts = ['NPFX', 'GIVN', 'NICK', 'SPFX', 'SURN', 'NSFX', 'NAME', '_HEB', 'ROMN'];

            do {
                if (!in_array($type, $namefacts, true)) {
                    $text = '';
    ...
    <script type="text/javascript">
        ...
        var npfx_accept = new Array(<?php foreach ($NPFX_accept as $indexval => $npfx) {
        print "'" . $npfx . "',";
    }

    print "''"; ?>);
        var spfx_accept = new Array(<?php foreach ($SPFX_accept as $indexval => $spfx) {
        print "'" . $spfx . "',";
    }
    ...
    print "''"; ?>);
        ...
        function togglename() {
            ...
            if (ronly) {
                ...
                var name_array = name.preg_split("/");
                var givn = trim(name_array[0]);
                var givn_array = givn.preg_split(" ");
                var surn = trim(name_array[1]);
                var surn_array = surn.preg_split(" ");
...
function print_calendar_popup($id)
{
    ...
    foreach (['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday'] as $indexval => $day) {
...
function print_findsource_link($element_id)
{
    ...
    if (isset($PGV_IMAGES['source']['small'])) {
        print '<img src="' . $PGV_IMAGE_DIR . '/' . $PGV_IMAGES['source']['small'] . '" alt="' . $text . '" title="' . $text . '" border="0" align="middle">';
    } else {
...
function print_findrepository_link($element_id)
{
    ...
    if (isset($PGV_IMAGES['source']['small'])) {
        ...
        print $pgv_lang['find_repository'];
    }
...
function print_findfamily_link($element_id)
{
    ...
    if (isset($PGV_IMAGES['sfamily']['small'])) {
        print '<img src="' . $PGV_IMAGE_DIR . '/' . $PGV_IMAGES['sfamily']['small'] . '" alt="' . $text . '" title="' . $text . '" border="0" align="middle">';
    } else {
        print $pgv_lang['find_family'];
    }
...
function print_findmedia_link($element_id)
{
    ...
    if (isset($PGV_IMAGES['media']['small'])) {
        print '<img src="' . $PGV_IMAGE_DIR . '/' . $PGV_IMAGES['media']['small'] . '" alt="' . $text . '" title="' . $text . '" border="0" align="middle">';
    } else {
...
function add_simple_tag($tag, $upperlevel = '')
{
    ...
    $largetextfacts = ['TEXT', 'PUBL', 'NOTE'];

    $subnamefacts = ['NPFX', 'GIVN', 'NICK', 'SPFX', 'SURN', 'NSFX'];
    ...
    $islink = ('@' == mb_substr($value, 0, 1) and '@#' != mb_substr($value, 0, 2));

    if ($islink) {
        $value = trim($value, ' @');
    } else {
        $value = trim(mb_substr($tag, mb_strlen($fact) + 3));
    }
    ...
    if ('DATE' == $fact or 'TIME' == $fact or 'TYPE' == $fact) {
        $cols = 20;
    }

    if ('LATI' == $fact or 'LONG' == $fact) {
        $cols = 12;
    }

    if (in_array($fact, $subnamefacts, true)) {
        $cols = 25;
    }

    if (in_array($fact, $largetextfacts, true)) {
        $rows = 10;
    ...
    if ('REPO' == $fact) {
        $cols = mb_strlen($REPO_ID_PREFIX) + 4;
    }
    ...
    if (!in_array($fact, $emptyfacts, true)) {
        if ('DATE' == $fact) {
            print_help_link('def_gedcom_date_help', 'qm');
        } elseif ('RESN' == $fact) {
    ...
    if ('NOTE' == $fact and $islink) {
        ...
        if (false !== $nt) {
            $value = trim(strip_tags(@$n1match[1] . get_cont(1, $noterec)));
        }
    ...
    if (in_array($fact, $emptyfacts, true)) {
        ...
        foreach ($assorela as $key => $value) {
            ...
            print '>' . $assorela[(string)$key] . "</option>\n";
        }
        ...
        print "</tr></table>\n";
    } elseif ('_PRIM' == $fact or '_THUM' == $fact) {
        print '<select tabindex="' . $tabkey . '" id="' . $element_id . '" name="' . $element_name . "\" >\n";
        ...
        if ($rows > 1) {
            print '<textarea tabindex="' . $tabkey . '" id="' . $element_id . '" name="' . $element_name . '" rows="' . $rows . '" cols="' . $cols . '">' . $value . "</textarea>\n";
        } // text

        else {
            print '<input tabindex="' . $tabkey . '" type="text" id="' . $element_id . '" name="' . $element_name . '" value="' . htmlspecialchars($value, ENT_QUOTES | ENT_HTML5) . '" size="' . $cols . '" dir="ltr"';

            if ('NPFX' == $fact) {
                print ' onkeyup="wactjavascript_autoComplete(npfx_accept,this,event)" autocomplete="off" ';
            }

            if (in_array($fact, $subnamefacts, true)) {
                print ' onchange="updatewholename();"';
            }

            if ('DATE' == $fact) {
                print ' onblur="valid_date(this);"';
            }
        ...
        if ('PLAC' == $fact) {
            ...
            if ($SPLIT_PLACES) {
                if (!function_exists('print_place_subfields')) {
                    require 'includes/functions_places.php';
                }

                print_place_subfields($element_id);
            }
        } elseif ($cols > 20 and 'NPFX' != $fact) {
            print_specialchar_link($element_id, false);
        }
    }
    ...
    if ('TYPE' == $fact and 'MARR' == $tags[0]) {
        ...
        foreach (['Unknown', 'Civil', 'Religious', 'Partners'] as $indexval => $key) {
            ...
            $b = mb_strtolower($value);

            if (false !== @mb_strpos($a, $b) or false !== @mb_strpos($b, $a)) {
                print ' selected="selected"';
            }

            print '>' . $factarray['MARR_' . mb_strtoupper($key)] . "</option>\n";
        }
    ...
    if ('ASSO' == $fact) {
        print_findindi_link($element_id, get_person_name($value));
    }

    if ('FILE' == $fact) {
        print_findmedia_link($element_id);
    }
    ...
    if ('DATE' == $fact) {
        print get_changed_date($value);
    }

    if ('ASSO' == $fact and $value) {
        print ' ' . get_person_name($value) . ' (' . $value . ')';
    }

    if ('SOUR' == $fact and $value) {
        print ' ' . get_source_descriptor($value) . ' (' . $value . ')';
    }

    // pastable values

    if ('NPFX' == $fact) {
        print ' <img src="images/autocomplete.gif" alt="autocompletion active" title="autocompletion active" border="0" align="middle">';
    }
    ...
    if ('FORM' == $fact) {
        print_autopaste_link($element_id, $FILE_FORM_accept, false);
    }
    ...
    if ('NAME' == $fact) {
        ...
        print '<a href="javascript: '
              . $pgv_lang['show_details']
              . '" onclick="togglename(); return false;"><img style="display:none;" id="'
              . $element_id
              . '_minus" src="'
              . $PGV_IMAGE_DIR
              . '/'
              . $PGV_IMAGES['minus']['other']
              . "\" border=\"0\" width=\"11\" height=\"11\" alt=\"\" title=\"\"></a>\n";
    }
    ...
    if ('FILE' == $fact and is_writable($MEDIA_DIRECTORY)) {
...
function handle_updates($newged)
{
    ...
    for ($j = 0, $jMax = count($glevels); $j < $jMax; $j++) {
        //-- update external note records first

        if (($islink[$j]) && ('NOTE' == $tag[$j])) {
            if (empty($NOTE[$text[$j]])) {
                ...
                $notelines = preg_preg_split("/\r?\n/", $noterec);

                for ($k = 1, $kMax = count($notelines); $k < $kMax; $k++) {
                    if (0 == preg_match('/1 CON[CT] /', $notelines[$k])) {
                        $newnote .= trim($notelines[$k]) . "\r\n";
                    }
                }
        ...
        if ((!empty($text[$j])) || (true === $pass)) {
            ...
            for ($k = 0, $kMax = count($newlines); $k < $kMax; $k++) {
                if ($k > 0) {
                    $newlines[$k] = ($glevels[$j] + 1) . ' CONT ' . $newlines[$k];
                }
