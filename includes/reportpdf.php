...
if (mb_strstr($_SERVER['PHP_SELF'], 'reportpdf.php')) {
...
$ascii_langs = ['english', 'danish', 'dutch', 'french', 'german', 'norwegian', 'spanish', 'spanish-ar'];
...
if (!isset($embed_fonts)) {
    if (in_array($LANGUAGE, $ascii_langs, true)) {
        $embed_fonts = false;
    } else {
        $embed_fonts = true;
    }
...
class PGVReport
{
    ...
    public function setup($pw, $ph, $o, $m)
    {
        ...
        $this->pdf = new PGVRPDF('P', 'pt', [$pw * 72, $ph * 72]);
    ...
    {
        if ('H' == $this->processing) {
            return $this->pdf->addHeader($element);
        }

        if ('PH' == $this->processing) {
            return $this->pdf->addPageHeader($element);
        }
    ...
    public function run()
    {
        ...
        if ($embed_fonts) {
            $this->pdf->AddFont('LucidaSansUnicode', '', 'LucidaSansRegular.php');
        }
...
class PGVRElement
{
    ...
    public function setWrapWidth($width)
    {
    }
...
class PGVRCell extends PGVRElement
{
    ...
    public function PGVRCell($width, $height, $align, $style, $top = '.', $left = '.')
    {
        $this->text = '';
...
class PGVRTextBox extends PGVRElement
{
    ...
    public function PGVRTextBox($width, $height, $border, $fill, $newline, $left = '.', $top = '.')
    {
        $this->width = $width;
    ...
    public function render(&$pdf)
    {
        ...
        for ($i = 0; $i < count($this->elements); $i++) {
            ...
            if ('footnotetexts' != $element) {
                ...
                elseif (('PGVRFootnote' != $element->get_type()) || ('' != trim($element->getValue()))) {
                    if ('' != $lastelement) {
                        $newelements[] = $lastelement;
        ...
        for ($i = 0; $i < count($this->elements); $i++) {
            if ('footnotetexts' != $this->elements[$i]) {
        ...
        if (!empty($this->style)) {
            $pdf->Rect($pdf->GetX(), $pdf->GetY(), $this->width, $this->height, $this->style);
        }
        ...
        if ($this->newline) {
            ...
            if ($curn != $pdf->page) {
                $ny = $cury + $pdf->getCurrentStyleHeight();
            } else {
                $ny = $cury + $this->height;
            }
            ...
            if (($ty > $startY) && ($ty < $startY + $this->height)) {
                $ty = $startY;
            }
...
class PGVRText extends PGVRElement
{
    ...
    public function PGVRText($style)
    {
        $this->text = '';
    ...
    public function render(&$pdf, $curx = 0)
    {
        global $embed_fonts;
        ...
        if (count($lines) > 0) {
            foreach ($lines as $indexval => $line) {
                ...
                if ($cury > $pdf->getPageHeight()) {
                    $cury = $pdf->getY() + $styleh + 1;
                }
    ...
    public function setWrapWidth($width, $width2)
    {
...
class PGVRFootnote extends PGVRElement
{
    ...
    public function PGVRFootnote($style = '')
    {
        $this->text = '';
    ...
    public function rerender(&$pdf)
    {
        global $footnote_count;

        if (empty($this->num)) {
            if (empty($footnote_count)) {
                $footnote_count = 1;
            } else {
                $footnote_count++;
            }
...
class PGVRImage extends PGVRElement
{
    ...
    public function PGVRImage($file, $x, $y, $w, $h)
    {
        //		print "$file $x $y $w $h";
    ...
    public function render(&$pdf)
    {
        ...
        if (0 == $this->y) {
            ...
            if (isset($lastpicbottom)) {
                if (($pdf->page == $lastpicpage) && ($lastpicbottom > $pdf->GetY()) && ($this->x >= $lastpicleft) && ($this->x <= $lastpicright)) {
...
class PGVRLine extends PGVRElement
{
    ...
    public function PGVRLine($x1, $y1, $x2, $y2)
    {
        $this->x1 = $x1;
...
function startElement($parser, $name, $attrs)
{
    ...
    if ((0 == $processIfs || 'PGVRif' == $name) && (0 == $processGedcoms || 'PGVRGedcom' == $name) && (0 == $processRepeats || 'PGVRFacts' == $name || 'PGVRRepeatTag' == $name)) {
...
function endElement($parser, $name)
{
    ...
    if ((0 == $processIfs || 'PGVRif' == $name) && (0 == $processGedcoms || 'PGVRGedcom' == $name) && (0 == $processRepeats || 'PGVRFacts' == $name || 'PGVRRepeatTag' == $name || 'PGVRList' == $name || 'PGVRRelatives' == $name)) {
...
function characterData($parser, $data)
{
    ...
    if ($printData && (0 == $processGedcoms) && (0 == $processIfs)) {
...
function PGVRNowSHandler($attrs)
{
    ...
    $currentElement->addText(
        get_changed_date(date('j', time() - ($_SESSION['timediff'] ?? 0)) . ' ' . date('M', time() - ($_SESSION['timediff'] ?? 0)) . ' ' . date('Y', time() - ($_SESSION['timediff'] ?? 0)))
    );
}
...
function PGVRGedcomSHandler($attrs)
{
        ...
        foreach ($tags as $indexval => $tag) {
            ...
            if ($ct > 0) {
                ...
                if ($ct > 0) {
                    ...
                    $temp = preg_preg_split("/\s+/", trim($tgedrec));
    ...
    if (!empty($newgedrec)) {
        ...
        array_push($gedrecStack, [$gedrec, $fact, $desc]);
                ...
                if (mb_strlen($name) > $attrs['truncate']) {
                    ...
                    for ($i = count($words) - 2; $i >= 0; $i--) {
                        ...
                        if ($len > $attrs['truncate']) {
                            $name = get_first_letter($words[$i]) . '. ' . $name;
                        } else {
                            $name = $words[$i] . ' ' . $name;
                        }
                    }
    ...
    if (!empty($tag)) {
        ...
        if ('@id' == $tag) {
            ...
            if (empty($attrs['level'])) {
                $temp = preg_preg_split("/\s+/", trim($gedrec));
...
function PGVRRepeatTagSHandler($attrs)
{
    ...
    array_push($repeatsStack, [$repeats, $repeatBytes]);
    ...
    if (!empty($tag)) {
        if ('@desc' == $tag) {
            ...
            for ($i = 0; $i < count($tags); $i++) {
                ...
                if (!empty($t)) {
                    if (1 == $level && false === mb_strstr('CHIL,FAMS,FAMC', $t) && (!showFact($t, $id) || !showFactDetails($t, $id))) {
                        return;
                    }
...
function PGVRRepeatTagEHandler()
{
    ...
    while (false === mb_strstr($lines[$lineoffset + $repeatBytes], '<PGVRRepeatTag')) {
        $lineoffset--;
    }
    ...
    while (($ct > 0) && ($line1 < $line + 2)) {
        if (false !== mb_strstr($lines[$lineoffset + $line1], '<PGVRRepeatTag')) {
            $ct++;
        }
    ...
    for ($i = 0; $i < count($repeats); $i++) {
        ...
        xml_set_elementHandler($repeat_parser, 'startElement', 'endElement');

        //-- set the character data handler

        xml_set_character_dataHandler($repeat_parser, 'characterData');
...
function PGVRvarSHandler($attrs)
{
    ...
    if (!empty($var)) {
        if (!empty($vars[$var]['id'])) {
            ...
            $var = preg_replace(["/\[/", "/\]/", '/@fact/', '/@desc/'], ["['", "']", $tfact, $desc], $var);
...
function PGVRifSHandler($attrs)
{
    ...
    $condition = preg_replace(['/ LT /', '/ GT /'], ['<', '>'], $condition);
...
function PGVRListSHandler($attrs)
{
                        ...
                        case 'GTE':
                            ...
                            if ($ct > 0) {
                                ...
                                if ('DATE' == $t) {
                                    ...
                                    if ($date1[0]['year'] > $date2[0]['year']) {
                                        $keep = true;
                                    } elseif ($date1[0]['year'] == $date2[0]['year']) {
                                        if ($date1[0]['mon'] > $date2[0]['mon']) {
                                            $keep = true;
                                        } elseif ($date1[0]['mon'] == $date2[0]['mon']) {
                                            if ($date1[0]['day'] >= $date2[0]['day']) {
                                                $keep = true;
                                            } else {
                                                $keep = false;
                                            }
    ...
    if ('NAME' == $sortby) {
        uasort($list, 'itemsort');
    } else {
        uasort($list, 'compare_date');
    }
...
function PGVRListEHandler()
{
    ...
    while (false === mb_strstr($lines[$lineoffset + $repeatBytes], '<PGVRList') && $lineoffset + $repeatBytes > 0) {
        $lineoffset--;
    }
    ...
    while (($ct > 0) && ($line1 < $line + 2)) {
        if (false !== mb_strstr($lines[$lineoffset + $line1], '<PGVRList')) {
            $ct++;
        }
            ...
            foreach ($famids as $indexval => $famid) {
                ...
                if ($famrec) {
                    ...
                    for ($i = 0; $i < $num; $i++) {
                        ...
                        if (isset($list[$smatch[$i][1]]['generation'])) {
                            $list[$smatch[$smatch[$i][1]][1]]['generation'] = $list[$id]['generation'] + 1;
                        } else {
...
function PGVRRelativesEHandler()
{
    ...
    while (false === mb_strstr($lines[$lineoffset + $repeatBytes], '<PGVRRelatives') && $lineoffset + $repeatBytes > 0) {
        $lineoffset--;
    }
    ...
    while (($ct > 0) && ($line1 < $line + 2)) {
        if (false !== mb_strstr($lines[$lineoffset + $line1], '<PGVRRelatives')) {
            $ct++;
        }
