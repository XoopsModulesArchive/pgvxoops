 ...
 * @return bool    returns true if we need to cleanup the head, false if we don't
 * @see head_cleanup()
 */
function need_head_cleanup()
{
    ...
    $pos1 = mb_strpos($fcontents, '0 HEAD');
 ...
 * @return bool    whether or not the cleanup was successful
 ...
 * @return bool    return true if the cleanup is needed
 ...
 * @return bool    returns true if the cleanup is needed
 * @see place_cleanup()
 */
 ...
 * @return bool    returns true if cleanup was successful
...
function place_cleanup()
{
    ...
    $fcontents = preg_replace(
        "/^1 (CAST|DSCR|EDUC|IDNO|NATI|NCHI|NMR|OCCU|PROP|RELI|SSN|TITL|_MILI|_FA1|_FA2|_FA3|_FA4|_FA5|_FA6)(\s*)$[\s]+(^2 TYPE(.*)[\s]+)?(^2 DATE(.*)[\s]+)?^2 PLAC (.*)$/m",
        fixreplaceval('$1', '$7', '$3', '$5'),
        $fcontents
    );
...
function fix_date($datestr)
{
    ...
    if (isset($date[0])) {
        return $date[0]['day'] . ' ' . str2upper($date[0]['month']) . ' ' . $date[0]['year'];
    }
    ...
    for ($i = 0; $i < $ct; $i++) {
        ...
        if (false !== $indirec) {
            ...
            //  			print "successfully set to $rin<br>\n";
                ...
                print "<span class=\"error\">No $tag found in record<br></span>\n";
            }
        }
    }
 ...
 * @return bool    returns true if the file claims to be ANSI encoded
