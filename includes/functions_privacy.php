 ...
 */
if (mb_strstr($_SERVER['PHP_SELF'], 'functions_privacy.php')) {
    ...
    function is_dead($indirec, $cyear = '')
    {
        ...
        if (!empty($deathrec)) {
            if ($cyear == date('Y')) {
                ...
                if (preg_match('/1 DEAT Y/', $deathrec) > 0) {
                    return true;
                }
            } else {
        ...
        for ($i = 0; $i < $ct; $i++) {
            if (false === mb_strstr($match[$i][0], '@#DHEBREW@')) {
                $byear = $match[$i][1];
...
if (!function_exists('displayDetails')) {
     ...
     * @return    bool return true to show the persons details, return false to keep them private
    ...
    function displayDetailsByID($pid, $type = 'INDI')
    {
        ...
        if (!empty($username)) {
            ...
            if (isset($user_privacy[$username]['all'])) {
                if ($user_privacy[$username]['all'] >= $PRIV_USER) {
                    return true;
                }
            ...
            if (isset($user_privacy[$username][$pid])) {
                if ($user_privacy[$username][$pid] >= $PRIV_USER) {
                    return true;
                }
            ...
            if (isset($person_privacy[$pid])) {
                if ($person_privacy[$pid] >= $PRIV_USER) {
                    return true;
                }
        ...
        if (isset($PRIVACY_BY_RESN) && (true === $PRIVACY_BY_RESN)) {
            $gedrec = find_gedcom_record($pid);

            if (preg_match('/1 RESN/', $gedrec) > 0) {
                return false;
            }
        }
...
if (!function_exists('showLivingNameByID')) {
     ...
     *
     * @param string $pid the GEDCOM XRef ID for the entity to check privacy settings for
     * @return    bool return true to show the person's name, return false to keep it private
     * @author    yalnifj
     */
    ...
    function showLivingNameByID($pid)
    {
        ...
        if ((displayDetailsByID($pid)) || ($SHOW_LIVING_NAMES >= getUserAccessLevel())) {
            return true;
        }
...
if (!function_exists('showFact')) {
     ...
     * @return    bool return true to show the fact, return false to keep it private
     * @author    yalnifj
     */
    ...
    function showFact($fact, $pid)
    {
        ...
        if (isset($global_facts[$fact])) {
            if (isset($global_facts[$fact]['show'])) {
                if ($global_facts[$fact]['show'] == $PRIV_HIDE) {
                    return false;
                }
            }
        }
        ...
        if ('NAME' != $fact) {
            ...
            return $disp;
        } elseif (!displayDetails(find_gedcom_record($pid))) {
            return showLivingNameByID($pid);
        }
...
if (!function_exists('showFactDetails')) {
     ...
     * @param string $fact the GEDCOM fact tag to check the privacy settings
     * @param string $pid  the GEDCOM XRef ID for the entity to check privacy settings
     * @return    bool return true to show the fact details, return false to keep it private
     * @author    yalnifj
     */
    ...
    function showFactDetails($fact, $pid)
    {
        ...
        if (isset($global_facts[$fact])) {
            if (isset($global_facts[$fact]['details'])) {
                if ($global_facts[$fact]['details'] == $PRIV_HIDE) {
                    return false;
                }
            }
        }
...
function privatize_gedcom($gedrec)
{
    ...
    if ($gt > 0) {
        ...
        if (!$disp) {
            ...
            if (('INDI' == $type) && (!showLivingNameByID($gid))) {
                $newrec = '0 @' . $gid . "@ INDI\r\n";
        ...
        foreach ($subs as $indexval => $sub) {
            if (false === FactViewRestricted($gid, $sub)) {
                $newrec .= $sub;
            }
        }
...
function FactEditRestricted($pid, $factrec)
{
    ...
    if (isset($user['gedcomid'][$GEDCOM])) {
        trim($myindi = $user['gedcomid'][$GEDCOM]);
    }
    ...
    if ($ct > 0) {
        ...
        if ((('confidential' == $match[1]) || ('locked' == $match[1])) && ((userIsAdmin(getUserName())) || (userGedcomAdmin(getUserName())))) {
            return false;
        }

        if (('privacy' == $match[1]) && ((userIsAdmin(getUserName())) || ($myindi == $pid) || (userGedcomAdmin(getUserName())))) {
            return false;
        }

        if ('F' == mb_substr($pid, 0, 1)) {
            $famrec = find_family_record($pid);

            $parents = find_parents_in_record($famrec);

            if (('privacy' == $match[1]) && ((userIsAdmin(getUserName())) || ($myindi == $parents['HUSB']) || (userGedcomAdmin(getUserName())))) {
                return false;
            }

            if (('privacy' == $match[1]) && ((userIsAdmin(getUserName())) || ($myindi == $parents['WIFE']) || (userGedcomAdmin(getUserName())))) {
                return false;
            }
        }
    }
...
function FactViewRestricted($pid, $factrec)
{
    ...
    if (isset($user['gedcomid'][$GEDCOM])) {
        $myindi = trim($user['gedcomid'][$GEDCOM]);
    }
    ...
    if ($ct > 0) {
        ...
        if (('confidential' == $match[1]) && ((userIsAdmin(getUserName())) || (userGedcomAdmin(getUserName())))) {
            return false;
        }
