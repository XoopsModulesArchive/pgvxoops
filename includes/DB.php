 ...
 */
if (file_exists('PEAR.php') && !defined('PEAR_ERROR_RETURN')) {
    require_once __DIR__ . '/PEAR.php';
} elseif (file_exists('includes/PEAR.php') && !defined('PEAR_ERROR_RETURN')) {
    require_once __DIR__ . '/includes/PEAR.php';
}
...
class DB
{
    ...
    public function &factory($type, $options = false)
    {
        ...
        if (isset($options['debug']) && $options['debug'] >= 2) {
            ...
            require_once "includes/DB/{$type}.php";
        } else {
            @require_once "includes/DB/{$type}.php";
        }
     ...
     *
     * @param int $value the DB error code
     *
...
DB_OK => 'no error',
        ...
        return $errorMessages[$value] ?? $errorMessages[DB_ERROR];
    }
...
'database' => false,
        ...
        if (false !== ($pos = mb_strpos($dsn, '://'))) {
            $str = mb_substr($dsn, 0, $pos);

            $dsn = mb_substr($dsn, $pos + 3);
        } else {
            $str = $dsn;
        ...
        if (false !== ($at = mb_strrpos($dsn, '@'))) {
            $str = mb_substr($dsn, 0, $at);

            $dsn = mb_substr($dsn, $at + 1);

            if (false !== ($pos = mb_strpos($str, ':'))) {
                $parsed['username'] = rawurldecode(mb_substr($str, 0, $pos));

                $parsed['password'] = rawurldecode(mb_substr($str, $pos + 1));
            } else {
        ...
        if ($dsn) {
            if (false === ($pos = mb_strpos($dsn, '?'))) {
                ...
                $parsed['database'] = rawurldecode(mb_substr($dsn, 0, $pos));
...
class DB_result
{
     ...
     * @param null $rownum    the row number to fetch (index starts at 0)
     *
    ...
    {
        if (DB_FETCHMODE_DEFAULT === $fetchmode) {
            $fetchmode = $this->fetchmode;
        }

        if (DB_FETCHMODE_OBJECT === $fetchmode) {
            $fetchmode = DB_FETCHMODE_ASSOC;

            $object_class = $this->fetchmode_object_class;
        }

        if (null !== $this->limit_from) {
            if (null === $this->row_counter) {
                $this->row_counter = $this->limit_from;

                // Skip rows

                if (false === $this->dbh->features['limit']) {
                    $i = 0;
            ...
            if ('emulate' === $this->dbh->features['limit']) {
    ...
    {
        if ('emulate' === $this->dbh->features['numrows']
            ...
            while (false !== ($res->fetchInto($tmp, DB_FETCHMODE_ORDERED))) {
                $i++;
            }
     ...
     *
     * @return int  the current row being looked at.  Starts at 1.
     */
