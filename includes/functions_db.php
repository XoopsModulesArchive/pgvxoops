 ...
 * @param bool $show_error whether or not to show any error messages
...
function dbquery($sql, $show_error = true)
{
    ...
    if (!empty($SQL_LOG)) {
        $fp = fopen($INDEX_DIRECTORY . '/sql_log.txt', 'ab');
...
function dbgetrow($sql, $show_error = true)
{
    ...
    if (DB::isError($row)) {
        if ($show_error) {
            print '<span class="error"><b>ERROR:' . $row->getMessage() . " <br>SQL:</b>$sql</span><br><br>\n";
        }
    }
...
function db_prep($item)
{
    ...
    if (is_object($DBCONN)) {
        return $DBCONN->escapeSimple($item);
    }
...
function find_family_record($famid)
{
    ...
    if (isset($famlist[$famid]['gedcom']) && ($famlist[$famid]['file'] == $GEDCOM)) {
...
function find_person_record($pid)
{
    ...
    if ((isset($indilist[$pid]['gedcom'])) && ($indilist[$pid]['file'] == $GEDCOM)) {
...
function find_gedcom_record($pid)
{
    ...
    if ((isset($famlist[$pid]['gedcom'])) && ($famlist[$pid]['file'] == $GEDCOM)) {
        return $famlist[$pid]['gedcom'];
    }

    if ((isset($sourcelist[$pid]['gedcom'])) && ($sourcelist[$pid]['file'] == $GEDCOM)) {
        return $sourcelist[$pid]['gedcom'];
    }

    if ((isset($otherlist[$pid]['gedcom'])) && ($otherlist[$pid]['file'] == $GEDCOM)) {
...
function find_source_record($sid)
{
    ...
    if (isset($sourcelist[$sid]['gedcom'])) {
...
function import_record($indirec, $update = false)
{
    ...
    if ('INDI' == $type) {
        ...
        foreach ($names as $indexval => $name) {
            if ($j > 0) {
                $sql = 'INSERT INTO ' . $TBLPREFIX . "names VALUES('" . $DBCONN->escapeSimple($gid) . "','" . $DBCONN->escapeSimple($FILE) . "','" . $DBCONN->escapeSimple($name[0]) . "','" . $DBCONN->escapeSimple($name[1]) . "','" . $DBCONN->escapeSimple($name[2]) . "','" . $DBCONN->escapeSimple(
                    $name[3]
                ) . "')";
        ...
        $sql = 'INSERT INTO '
               ...
               . "','"
               . $DBCONN->escapeSimple($indi['file'])
               . "','"
               . $DBCONN->escapeSimple($indi['rin'])
               . "','"
               . $DBCONN->escapeSimple($names[0][0])
               . "',-1,'"
               . $DBCONN->escapeSimple($indi['gedcom'])
               . "','"
               . $DBCONN->escapeSimple($names[0][1])
               . "','"
               . $DBCONN->escapeSimple($names[0][2])
               . "')";
        ...
        $sql = 'INSERT INTO '
               . $TBLPREFIX
               . "families (f_id, f_file, f_husb, f_wife, f_chil, f_gedcom) VALUES ('"
               . $DBCONN->escapeSimple($gid)
               . "','"
               . $DBCONN->escapeSimple($fam['file'])
               . "','"
               . $DBCONN->escapeSimple($fam['HUSB'])
               . "','"
               . $DBCONN->escapeSimple($fam['WIFE'])
               . "','"
               . $DBCONN->escapeSimple($fam['CHIL'])
               . "','"
               . $DBCONN->escapeSimple($fam['gedcom'])
               . "')";
        ...
        $subindi = preg_preg_split('/1 TITL /', $indirec);

        if (count($subindi) > 1) {
            $pos = mb_strpos($subindi[1], "\n1", 0);

            if ($pos) {
                $subindi[1] = mb_substr($subindi[1], 0, $pos);
            }
...
function add_new_name($gid, $newname, $letter, $surname, $indirec)
{
    ...
    $indilist[$gid]['names'][] = [$newname, $letter, $surname, 'C'];
...
function update_places($gid, $indirec, $update = false)
{
    ...
    for ($i = 0; $i < $pt; $i++) {
        ...
        foreach ($secalp as $indexval => $place) {
            ...
            $key = mb_strtolower($place . '_' . $level . '_' . $parent_id);
            ...
            if (isset($placecache[$key])) {
                ...
                if (false === mb_strpos($placecache[$key][1], $gid . ',')) {
        ...
        if ($table == $TBLPREFIX . 'individuals') {
            ...
            if ('sqlite' != $DBTYPE) {
                ...
                foreach ($info as $indexval => $field) {
                    ...
                    if ('i_letter' == $field['name']) {
                        $has_first_letter = true;
                    }

                    if ('i_surname' == $field['name']) {
                        $has_surname = true;
                    }
                }
            } else {
...
function get_source_add_title_list()
{
    ...
    while (false !== ($row = &$res->fetchRow(DB_FETCHMODE_ASSOC))) {
        ...
        if (0 == $ct) {
            $ct = preg_match("/\d _HEB (.*)/", $row['s_gedcom'], $match);
        }
...
function get_indi_list()
{
    ...
    while (false !== ($row = &$res->fetchRow(DB_FETCHMODE_ASSOC))) {
        ...
        $indi['names'] = [[$row['i_name'], $row['i_letter'], $row['i_surname'], 'A']];
    ...
    while (false !== ($row = &$res->fetchRow(DB_FETCHMODE_ASSOC))) {
        ...
        if (isset($indilist[$row['n_gid']])) {
            $indilist[$row['n_gid']]['names'][] = [$row['n_name'], $row['n_letter'], $row['n_surname'], $row['n_type']];
        }
    }
 ...
 * @param bool $allgeds setting if all gedcoms should be searched, default is false
 * @param mixed $ANDOR
 * @return    array array with all individuals that matched the query
 * @author    yalnifj
 */
function search_indis($query, $allgeds = false, $ANDOR = 'AND')
{
    ...
    if (!is_array($query)) {
        $sql = 'SELECT i_id, i_name, i_file, i_gedcom, i_isdead, i_letter, i_surname FROM '
               . $TBLPREFIX
               . "individuals WHERE (i_gedcom $term '"
               . $DBCONN->escapeSimple(mb_strtoupper($query))
               . "' OR i_gedcom $term '"
               . $DBCONN->escapeSimple(str2upper($query))
               . "' OR i_gedcom $term '"
               . $DBCONN->escapeSimple(str2lower($query))
               . "')";
    } else {
    ...
    if ((is_array($allgeds)) && (0 != count($allgeds))) {
        $sql .= ' AND (';

        for ($i = 0, $iMax = count($allgeds); $i < $iMax; $i++) {
    ...
    if (!DB::isError($res)) {
        while (false !== ($row = &$res->fetchRow())) {
            ...
            if ($allgeds) {
                ...
                if ($myindilist[$row[0] . '[' . $row[2] . ']']['file'] == $GEDCOM) {
                ...
                if ($myindilist[$row[0]]['file'] == $GEDCOM) {
...
function search_indis_year_range($startyear, $endyear, $allgeds = false)
{
    ...
    while ($i <= $endyear) {
        ...
        if ($REGEXP_DB) {
            $sql .= "i_gedcom REGEXP '" . $DBCONN->escapeSimple("2 DATE[^\n]* " . $i) . "'";
        } else {
            $sql .= "i_gedcom LIKE '" . $DBCONN->escapeSimple('%2 DATE%' . $i) . "%'";
        }
...
function search_fams($query, $allgeds = false, $ANDOR = 'AND')
{
    ...
    if (!is_array($query)) {
        $sql = 'SELECT f_id, f_husb, f_wife, f_file, f_gedcom FROM '
               . $TBLPREFIX
               . "families WHERE (f_gedcom $term '"
               . $DBCONN->escapeSimple(mb_strtoupper($query))
               . "' OR f_gedcom $term '"
               . $DBCONN->escapeSimple(str2upper($query))
               . "' OR f_gedcom $term '"
    ...
    if (!$allgeds) {
        $sql .= " AND f_file='" . $DBCONN->escapeSimple($GEDCOM) . "'";
    }
...
function search_fams_year_range($startyear, $endyear, $allgeds = false)
{
    ...
    while ($i <= $endyear) {
        ...
        if ($REGEXP_DB) {
            $sql .= "f_gedcom REGEXP '" . $DBCONN->escapeSimple("2 DATE[^\n]* " . $i) . "'";
        } else {
            $sql .= "f_gedcom LIKE '" . $DBCONN->escapeSimple('%2 DATE%' . $i) . "%'";
        }
...
function search_sources($query, $allgeds = false)
{
    ...
    $sql = 'SELECT s_id, s_name, s_file, s_gedcom FROM '
                    . $TBLPREFIX
                    . "sources WHERE (s_gedcom $term '"
                    . $DBCONN->escapeSimple(mb_strtoupper($query))
                    . "' OR s_gedcom $term '"
                    . $DBCONN->escapeSimple(str2upper($query))
                    . "' OR s_gedcom $term '"
    ...
    if (!$allgeds) {
        $sql .= " AND s_file='" . $DBCONN->escapeSimple($GEDCOM) . "'";
    }
...
function search_repos($query, $allgeds = false)
{
    ...
    $sql = 'SELECT o_id, o_file, o_gedcom FROM ' . $TBLPREFIX . "other WHERE o_type='REPO' AND (o_gedcom $term '" . $DBCONN->escapeSimple(mb_strtoupper($query)) . "' OR o_gedcom $term '" . $DBCONN->escapeSimple(str2upper($query)) . "' OR o_gedcom $term '" . $DBCONN->escapeSimple(
        str2lower($query)
    ) . "')";

    if (!$allgeds) {
        $sql .= " AND o_file='" . $DBCONN->escapeSimple($GEDCOM) . "'";
    }
...
function get_place_parent_id($parent, $level)
{
    ...
    for ($i = 0; $i < $level; $i++) {
        $escparent = str_replace("?", '\\\\\\?', $DBCONN->escapeSimple($parent[$i]));
...
function get_place_list()
{
    ...
    if (0 == $level) {
        $sql = 'SELECT p_place FROM ' . $TBLPREFIX . "places WHERE p_level=0 AND p_file='$GEDCOM' ORDER BY p_place";
    } else {
...
function search_places($sql, $splace)
{
    ...
    while (false !== ($row = &$res->fetchRow())) {
        ...
        if (empty($splace)) {
            $ct = preg_match_all("/\d PLAC (.*)/", $row[1], $match, PREG_SET_ORDER);
        } else {
            $ct = preg_match_all("/\d PLAC (.*$splace.*)/i", $row[1], $match, PREG_SET_ORDER);
        }
        ...
        for ($i = 0; $i < $ct; $i++) {
            ...
            $levels = preg_preg_split('/,/', $place);        // -- split the place into comma seperated values
...
function get_indi_alpha()
{
    ...
    $hungarianex = ['DZS', 'CS', 'DZ', 'GY', 'LY', 'NY', 'SZ', 'TY', 'ZS'];
    ...
    while (false !== ($row = &$res->fetchRow(DB_FETCHMODE_ASSOC))) {
        ...
        if ('danish' == $LANGUAGE || 'norwegian' == $LANGUAGE) {
            if (in_array(mb_strtoupper($letter), $danishex, true)) {
                if ('OE' == mb_strtoupper($letter)) {
                    $letter = 'Ø';
                } elseif ('AE' == mb_strtoupper($letter)) {
                    $letter = 'Æ';
                } elseif ('AA' == mb_strtoupper($letter)) {
                    $letter = 'Å';
                }
            }
        }
        ...
        if (mb_strlen($letter) > 1) {
            if (ord($letter) < 92) {
                if ('hungarian' != $LANGUAGE && in_array($letter, $hungarianex, true)) {
                    $letter = mb_substr($letter, 0, 1);
                }

                if (('danish' != $LANGUAGE || 'norwegian' != $LANGUAGE) && in_array($letter, $danishex, true)) {
                    $letter = mb_substr($letter, 0, 1);
                }
            }
        }
...
function get_alpha_indis($letter)
{
    ...
    if ('hungarian' == $LANGUAGE) {
        if (mb_strlen($letter) >= 2) {
            ...
            if (isset($text)) {
                $sql .= "(i_letter = '" . $DBCONN->escapeSimple($letter) . "' AND i_letter != '" . $DBCONN->escapeSimple($text) . "') ";
            } else {
        ...
        if (isset($text)) {
            $sql .= "(i_letter = '" . $DBCONN->escapeSimple($letter) . "' OR i_letter = '" . $DBCONN->escapeSimple($text) . "') ";
        } elseif ('A' == $letter) {
            $sql .= "i_letter LIKE '" . $DBCONN->escapeSimple($letter) . "' ";
        } else {
            $sql .= "i_letter LIKE '" . $DBCONN->escapeSimple($letter) . "%' ";
    ...
    while (false !== ($row = &$res->fetchRow(DB_FETCHMODE_ASSOC))) {
        ...
        if (mb_substr($row['i_letter'], 0, 1) == mb_substr($letter, 0, 1) || (isset($text) ? mb_substr($row['i_letter'], 0, 1) == mb_substr($text, 0, 1) : false)) {
            $indi = [];

            $indi['names'] = [[$row['i_name'], $row['i_letter'], $row['i_surname'], 'P']];
    ...
    if ('hungarian' == $LANGUAGE) {
        if (mb_strlen($letter) >= 2) {
            ...
            if (isset($text)) {
                $sql .= "(n_letter = '" . $DBCONN->escapeSimple($letter) . "' AND n_letter != '" . $DBCONN->escapeSimple($text) . "') ";
            } else {
    ...
    while (false !== ($row = &$res->fetchRow(DB_FETCHMODE_ASSOC))) {
        ...
        if (mb_substr($row['n_letter'], 0, 1) == mb_substr($letter, 0, 1) || (isset($text) ? mb_substr($row['n_letter'], 0, 1) == mb_substr($text, 0, 1) : false)) {
            if (!isset($indilist[$row['i_id']])) {
                $indi = [];

                $indi['names'] = [[$row['i_name'], $row['i_letter'], $row['i_surname'], 'P'], [$row['n_name'], $row['n_letter'], $row['n_surname'], $row['n_type']]];
                ...
                $indilist[$row['i_id']]['names'][] = [$row['n_name'], $row['n_letter'], $row['n_surname'], $row['n_type']];
    ...
    foreach ($myindilist as $gid => $indi) {
        ...
        for ($i = 0; $i < $ct; $i++) {
            ...
            if (!empty($hname)) {
                ...
                if (!isset($famlist[$famid]['surnames']) || 0 == count($famlist[$famid]['surnames'])) {
                    $famlist[$famid]['surnames'] = $surnames;
                } else {
                    array_merge($famlist[$famid]['surnames'], $surnames);
                }
...
function get_surname_fams($surname)
{
    ...
    foreach ($myindilist as $gid => $indi) {
        ...
        for ($i = 0; $i < $ct; $i++) {
            ...
            foreach ($indi['names'] as $indexval => $namearray) {
                if (false !== mb_stristr($namearray[2], $surname)) {
                    $hname = sortable_name_from_name($namearray[0]);
                }
            }
