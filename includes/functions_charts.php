...
function print_family_parents($famid, $sosa = 0)
{
    ...
    if (!empty($upfamid) and ('preview' != $view)) {
        print '<td valign="middle" rowspan="2">';

        print_url_arrow($upfamid, (0 == $sosa ? "?famid=$upfamid&amp;show_full=$show_full" : "#$upfamid"), "#$upfamid", 1);
    ...
    if ($sosa > 0) {
        ...
        if (showFact('MARR', $famid)) {
            print_simple_fact($famrec, 'MARR', $parents['WIFE']);
        } else {
...
function print_family_children($famid, $childid = '', $sosa = 0)
{
    ...
    if (userCanEdit(getUserName())) {
        if ((isset($show_changes)) && (isset($pgv_changes[$famid . '_' . $GEDCOM]))) {
            ...
            if ($ct > 0) {
                ...
                for ($i = 0; $i < $ct; $i++) {
                    if (!in_array($match[$i][1], $children, true)) {
                ...
                if ((0 == count($newchildren)) && (0 == count($oldchildren))) {
                    $children = [];
    ...
    if ((count($children) > 0) || (count($newchildren) > 0) || (count($oldchildren) > 0)) {
        foreach ($children as $indexval => $chil) {
            if (!in_array($chil, $oldchildren, true)) {
                print "<tr>\n";
                ...
                if (0 != $sosa) {
                    ...
                    for ($f = 0; $f <= $maxfam; $f++) {
                        ...
                        if (!empty($divrec) and ('preview' != $view)) {
    ...
    if (('preview' != $view) && (0 == $sosa) && (userCanEdit(getUserName()))) {
...
function print_family_facts($famid, $sosa = 0)
{
    ...
    if (displayDetailsByID($famid, 'FAM')) {
        ...
        $nonfacts = ['FAMS', 'FAMC', 'MAY', 'BLOB', 'HUSB', 'WIFE', 'CHIL', '_MEND', ''];
        ...
        if ((0 == $sosa) && userCanEdit(getUserName())) {
            if ((isset($show_changes)) && (isset($pgv_changes[$famid . '_' . $GEDCOM]))) {
                if (empty($newrec)) {
                    $newrec = find_record_in_file($famid);
                }
        ...
        if ((count($indifacts) > 0) || (count($otheritems) > 0)) {
            ...
            if ($SHOW_FAM_ID_NUMBERS and '' != $famid) {
                print " ($famid)";
            }
        ...
        if (('preview' != $view) && (0 == $sosa) && (userCanEdit(getUserName()))) {
            ...
            $addfacts = array_merge(CheckFactUnique(['NCHI', 'MARL', 'DIV', 'ANUL', 'DIVF', 'ENGA', 'MARB', 'MARC', 'MARS'], $indifacts, 'FAM'), ['CENS', 'MARR', 'RESI', 'SLGS', 'MARR_CIVIL', 'MARR_RELIGIOUS', 'MARR_PARTNERS']);
 ...
 * @return string validated root ID
 */
function check_rootid($rootid)
{
    ...
    if (empty($rootid)) {
        ...
        if ((!empty($user['rootid'][$GEDCOM])) && (find_person_record($user['rootid'][$GEDCOM]))) {
            $rootid = $user['rootid'][$GEDCOM];
        } elseif ((!empty($user['gedcomid'][$GEDCOM])) && (find_person_record($user['gedcomid'][$GEDCOM]))) {
            $rootid = $user['gedcomid'][$GEDCOM];
        }
        ...
        if (empty($rootid)) {
            ...
            if ((!empty($PEDIGREE_ROOT_ID)) && (find_person_record($PEDIGREE_ROOT_ID))) {
                $rootid = $PEDIGREE_ROOT_ID;
            } else {
...
function print_url_arrow($id, $url, $label, $dir = 2)
{
    ...
    if ('rtl' == $TEXT_DIRECTION and 0 == $dir) {
        $adir = 1;
    }

    if ('rtl' == $TEXT_DIRECTION and 1 == $dir) {
        $adir = 0;
    }
    ...
    print "<a href=\"$url\" onmouseover=\"swap_image('"
          . $astyle
          . $id
          . "',$adir); window.status ='"
          . $label
          . "'; return true; \" onmouseout=\"swap_image('"
          . $astyle
          . $id
          . "',$adir); window.status=''; return true; \"><img id=\""
          . $astyle
          . $id
          . '" src="'
          . $PGV_IMAGE_DIR
          . '/'
          . $PGV_IMAGES[$astyle]['other']
          . "\" hspace=\"0\" vspace=\"0\" border=\"0\" alt=\"$label\" title=\"$label\"></a>";
}
...
function get_sosa_name($sosa)
{
    ...
    if ('danish' == $LANGUAGE || 'norwegian' == $LANGUAGE || 'swedish' == $LANGUAGE) {
        $addname = '';

        $father = mb_strtolower($pgv_lang['father']);

        $mother = mb_strtolower($pgv_lang['mother']);

        $grand = 'be' . ('danish' == $LANGUAGE ? 'dste' : 'ste');

        $great = 'olde';

        $tip = 'tip' . ('danish' == $LANGUAGE ? '-' : 'p-');
        ...
        $sosaname = str2upper(mb_substr($sosaname, 0, 1)) . mb_substr($sosaname, 1);
    ...
    if ('dutch' == $LANGUAGE) {
        if ($gen & 256) {
            $sosaname .= $pgv_lang['sosa_11'];
        }

        if ($gen & 128) {
            $sosaname .= $pgv_lang['sosa_10'];
        }

        if ($gen & 64) {
            $sosaname .= $pgv_lang['sosa_9'];
        }

        if ($gen & 32) {
            $sosaname .= $pgv_lang['sosa_8'];
        }

        if ($gen & 16) {
            $sosaname .= $pgv_lang['sosa_7'];
        }

        if ($gen & 8) {
            $sosaname .= $pgv_lang['sosa_6'];
        }

        if ($gen & 4) {
            $sosaname .= $pgv_lang['sosa_5'];
        }

        $gen = $gen - floor($gen / 4) * 4;

        if (3 == $gen) {
            $sosaname .= $pgv_lang['sosa_4'] . $pgv_lang['sosa_3'] . $pgv_lang['sosa_2'];
        }

        if (2 == $gen) {
            $sosaname .= $pgv_lang['sosa_3'] . $pgv_lang['sosa_2'];
        }

        if (1 == $gen) {
            $sosaname .= $pgv_lang['sosa_2'];
        }

        if ($sosa % 2) {
            $sosaname .= mb_strtolower($pgv_lang['mother']);
        } else {
            $sosaname .= mb_strtolower($pgv_lang['father']);
        }
    ...
    if ('finnish' == $LANGUAGE) {
        ...
        for ($i = $gen; $i > 0; $i--) {
            if (!(floor($sosa / (pow(2, $i))) % 2)) {
                ...
                $sosaname .= mb_substr($mother, 0, 3) . 'din';
            }
        }
        ...
        if ('i' == mb_substr($sosaname, 0, 1)) {
            $sosaname = str2upper(mb_substr($sosaname, 0, 1)) . mb_substr($sosaname, 1);
        } else {
            $sosaname = str2upper(mb_substr($mother, 0, 2)) . mb_substr($sosaname, 2);
        }
    }

    if ('french' == $LANGUAGE) {
        if ($gen > 4) {
            $sosaname = 'Arrière(x' . ($gen - 1) . ')-';
        } else {
            for ($i = $gen; $i > 1; $i--) {
                $sosaname .= 'Arrière-';
            }
    ...
    if ('hebrew' == $LANGUAGE) {
        ...
        for ($i = $gen; $i >= 0; $i--) {
            ...
            } elseif (!(floor($sosa / (pow(2, $i))) % 2)) {
                $addname .= 'f';
            } else {
                $addname .= 'm';
            }

            if (0 == $i || 3 == mb_strlen($addname)) {
                if (3 == mb_strlen($addname)) {
                    if ('f' == mb_substr($addname, 2, 1)) {
                        $addname = $grandfather . $great;
                    } else {
                        $addname = $grandmother . $great;
                    }
                } elseif (2 == mb_strlen($addname)) {
                    if ('f' == mb_substr($addname, 1, 1)) {
                        $addname = $grandfather;
                    } else {
                        $addname = $grandmother;
                    }
    ...
    return $pgv_lang["sosa_$sosa"] ?? ((($sosa % 2) ? $pgv_lang['mother'] : $pgv_lang['father']) . ' ' . floor($sosa / 2));
}
...
function print_childbox_popup($rootid, $indirec, $url)
{
    ...
    for ($f = 0, $fMax = count($cfamids); $f < $fMax; $f++) {
    ...
    if ('rtl' == $TEXT_DIRECTION) {
        print 'rtl" style="position:absolute; right:';
    } else {
        print 'ltr" style="position:absolute; left:';
    }
