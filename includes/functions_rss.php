        ...
        foreach ($dayfamlist as $gid => $fam) {
            ...
            if (!empty($text)) {
                ...
                if (!empty($parents['HUSB'])) {
                    if (('living' == $filter) && (1 == is_dead_id($parents['HUSB']))) {
                        $display = false;
                    } elseif ($HIDE_LIVE_PEOPLE) {
                ...
                if ($display) {
                    if (!empty($parents['WIFE'])) {
                        if (('living' == $filter) && (1 == is_dead_id($parents['WIFE']))) {
                            $display = false;
                        } elseif ($HIDE_LIVE_PEOPLE) {
...
function getGedcomStats()
{
    ...
    if (count($surnames) > 0) {
        ...
        foreach ($surnames as $indexval => $surname) {
            ...
            if (in_array(ord(mb_substr($surname['name'], 0, 2)), $RTLOrd, true)) {
                //                      	if (ord(substr($surname["name"], 0, 2),$RTLOrd)){}

                $data .= '<a href="' . $SERVER_URL . 'indilist.php?surname=' . urlencode($surname['name']) . '">' . $surname['name'] . '</a>';
            } else {
                $data .= '<a href="' . $SERVER_URL . 'indilist.php?surname=' . $surname['name'] . '">' . $surname['name'] . '</a>';
            }
...
function getTop10Surnames($block = true, $config = '')
{
    ...
    $PGV_BLOCKS['print_block_name_top10']['config'] = ['num' => 10];
