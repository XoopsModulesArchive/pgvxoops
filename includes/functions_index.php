...
function check_for_import($ged)
{
    ...
    if ((!file_exists($indexfile)) || (filesize($indexfile) <= 1)) {
        return false;
    }
...
function find_family_record($famid)
{
    ...
    if (isset($famlist[$famid])) {
        return preg_replace("/\\\'/", "'", $famlist[$famid]['gedcom']);
    }
...
function find_person_record($pid)
{
    ...
    if (isset($indilist[$pid])) {
        return preg_replace("/\\\'/", "'", $indilist[$pid]['gedcom']);
    }
...
function find_gedcom_record($pid)
{
    ...
    if (isset($famlist[$pid])) {
        return preg_replace("/\\\'/", "'", $famlist[$pid]['gedcom']);
    }

    if (isset($sourcelist[$pid])) {
        return preg_replace("/\\\'/", "'", $sourcelist[$pid]['gedcom']);
    }

    if (isset($otherlist[$pid])) {
        return preg_replace("/\\\'/", "'", $otherlist[$pid]['gedcom']);
    }

    $pid = mb_strtoupper($pid);
...
function find_source_record($sid)
{
    ...
    if (isset($sourcelist[$sid])) {
        return preg_replace("/\\\'/", "'", $sourcelist[$sid]['gedcom']);
    }
...
function find_repo_record($rid)
{
    ...
    if (isset($otherlist[$rid])) {
        return preg_replace("/\\\'/", "'", $otherlist[$rid]['gedcom']);
    }
...
function update_family_name($gid, $fam)
{
    ...
    if ((!empty($fam['WIFE'])) && (isset($indilist[$fam['WIFE']]['names'][0][0]))) {
        $name .= $indilist[$fam['WIFE']]['names'][0][0];
    } else {
        $name .= '';
    }
...
function get_source_add_title_list()
{
    ...
    foreach ($sourcelist as $sid => $value) {
        ...
        if (0 == $ct) {
            $ct = preg_match("/\d _HEB (.*)/", $value['gedcom'], $match);
        }
...
function get_repo_list()
{
    ...
    foreach ($otherlist as $key => $value) {
        if (($value['file'] == $GEDCOM) && ('REPO' == $value['type'])) {
            $repo = [];
...
function search_indis_year_range($startyear, $endyear)
{
    ...
    foreach ($indilist as $gid => $indi) {
        ...
        for ($i = 1; $i <= $ct; $i++) {
            if (($recmatch[1][$i - 1] >= $startyear) && ($recmatch[1][$i - 1] <= $endyear)) {
                $myindilist[$gid] = $indi;
            }
        }
    }
...
function search_fams($query)
{
    ...
    foreach ($famlist as $gid => $fam) {
        if (isset($fam['gedcom'])) {
            $ct = preg_match("/($query)/i", str2upper($fam['gedcom']), $recmatch);
        }

        if (0 == $ct) {
            $ct = preg_match("/($query)/is", str2upper($fam['name']), $recmatch);
        }
    ...
    foreach ($list as $key => $value) {
        ...
        for ($i = 0; $i < $ct; $i++) {
            ...
            if ((isset($levels[$level])) && ($good)) {
                ...
                if (false === mb_stristr($found, $levels[$level] . ';')) {
...
function search_places($list, $splace)
{
    ...
    foreach ($list as $key => $value) {
        ...
        if (empty($splace)) {
            ...
            $ct = preg_match_all("/\d PLAC (.*$splace.*)/i", $value['gedcom'], $match, PREG_SET_ORDER);
        }
...
function get_indi_alpha()
{
    ...
    foreach ($indilist as $gid => $indi) {
        foreach ($indi['names'] as $indexval => $name) {
            if ($SHOW_MARRIED_NAMES || 'C' != $name[3]) {
                $letter = $name[1];
...
function get_surname_indis($surname)
{
    ...
    foreach ($gids as $indexval => $gid) {
        foreach ($indilist[$gid]['names'] as $indexval => $name) {
            if ($SHOW_MARRIED_NAMES || 'C' != $name[3]) {
                if (mb_stristr(str2upper($surname), str2upper($name[2]))) {
                    $tindilist[$gid] = $indilist[$gid];
                }
            }
        }
        ...
        if ('delete' != $change['type']) {
            ...
            if ('INDI' == $type) {
                ...
                for ($i = 0; $i < $ct; $i++) {
                    ...
                    if (isset($famlist[$famid])) {
                        update_family_name($famid, $famlist[$famid]);
                    }
                }
            } elseif ('FAM' == $type) {
