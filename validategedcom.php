...
if ((isset($_POST['action'])) && ('cleanup' == $_POST['action'])) {
    ...
    if (file_is_writeable($GEDCOMS[$GEDCOM]['path']) && (file_exists($GEDCOMS[$GEDCOM]['path']))) {
        ...
        while (!feof($fp)) {
            ...
            if (isset($_POST['cleanup_places']) && 'YES' == $_POST['cleanup_places']) {
                if (false !== ($sample = need_place_cleanup())) {
                    $l_placecleanup = true;
            ...
            if ('NA' != $_POST['xreftype']) {
                $filechanged = true;

                xref_change($_POST['xreftype']);
            }

            if ('YES' == $_POST['utf8convert']) {
                $filechanged = true;
...
while (!feof($fp)) {
    ...
    if (!$l_placecleanup && false !== ($placesample = need_place_cleanup())) {
        $l_placecleanup = true;
    }

    if (!$l_datecleanup && false !== ($datesample = need_date_cleanup())) {
        $l_datecleanup = true;
    }
