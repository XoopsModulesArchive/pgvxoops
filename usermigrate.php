...
if ('backup' == $proceed) {
    print_header($pgv_lang['um_backup']);
} else {
    print print_header($pgv_lang['um_header']);
}
...
if ('backup' == $proceed) {
    ...
    if ((!isset($_POST['um_config'])) && (!isset($_POST['um_gedcoms'])) && (!isset($_POST['um_gedsets'])) && (!isset($_POST['um_logs'])) && (!isset($_POST['um_usinfo']))) {
        ...
        <form action="<?php print $PHP_SELF; ?>" method="post">
            <table align="center">
                <tr class="label"><?php print $pgv_lang['options']; ?></tr>
                <br>
                <tr>
                    <td><input type="checkbox" name="um_config" value="yes" checked></td>
                    <td style="text-align:<?php if ('ltr' == $TEXT_DIRECTION) {
            print 'left';
        } else {
            print 'right';
        } ?>; "><?php print $pgv_lang['um_bu_config']; ?></td>
                </tr>
                <tr>
                    <td><input type="checkbox" name="um_gedcoms" value="yes" checked></td>
                    <td style="text-align:<?php if ('ltr' == $TEXT_DIRECTION) {
            print 'left';
        } else {
            print 'right';
        } ?>; "><?php print $pgv_lang['um_bu_gedcoms']; ?></td>
                </tr>
                <tr>
                    <td><input type="checkbox" name="um_gedsets" value="yes" checked></td>
                    <td style="text-align:<?php if ('ltr' == $TEXT_DIRECTION) {
            print 'left';
        } else {
            print 'right';
        } ?>; "><?php print $pgv_lang['um_bu_gedsets']; ?></td>
                </tr>
                <tr>
                    <td><input type="checkbox" name="um_logs" value="yes" checked></td>
                    <td style="text-align:<?php if ('ltr' == $TEXT_DIRECTION) {
            print 'left';
        } else {
            print 'right';
        } ?>; "><?php print $pgv_lang['um_bu_logs']; ?></td>
                </tr>
                <tr>
                    <td><input type="checkbox" name="um_usinfo" value="yes" checked></td>
                    <td style="text-align:<?php if ('ltr' == $TEXT_DIRECTION) {
            print 'left';
        } else {
            print 'right';
        } ?>; "><?php print $pgv_lang['um_bu_usinfo']; ?></td>
                </tr>
            </table>
            <br><br>
    ...
    if (isset($_POST['um_usinfo'])) {
        ...
        if (('db' == $PGV_DATABASE) && ('authentication_mysql.php' == $AUTHENTICATION_MODULE)) {
        ...
        if (file_exists($INDEX_DIRECTORY . 'authenticate.php')) {
            $flist[] = $INDEX_DIRECTORY . 'authenticate.php';
        }

        if (file_exists($INDEX_DIRECTORY . 'news.dat')) {
            $flist[] = $INDEX_DIRECTORY . 'news.dat';
        }

        if (file_exists($INDEX_DIRECTORY . 'messages.dat')) {
            $flist[] = $INDEX_DIRECTORY . 'messages.dat';
        }

        if (file_exists($INDEX_DIRECTORY . 'blocks.dat')) {
            $flist[] = $INDEX_DIRECTORY . 'blocks.dat';
        }

        if (file_exists($INDEX_DIRECTORY . 'favorites.dat')) {
            $flist[] = $INDEX_DIRECTORY . 'favorites.dat';
        }
    }
    ...
    if (isset($_POST['um_gedsets'])) {
        ...
        if (file_exists($INDEX_DIRECTORY . 'gedcoms.php')) {
            $flist[] = $INDEX_DIRECTORY . 'gedcoms.php';
        }
        ...
        foreach ($GEDCOMS as $key => $gedcom) {
            ...
            if (file_exists($INDEX_DIRECTORY . $gedcom['gedcom'] . '_conf.php')) {
                $flist[] = $INDEX_DIRECTORY . $gedcom['gedcom'] . '_conf.php';
            }

            // Privacy files

            if (file_exists($INDEX_DIRECTORY . $gedcom['gedcom'] . '_priv.php')) {
                $flist[] = $INDEX_DIRECTORY . $gedcom['gedcom'] . '_priv.php';
            }
        }
    }
    ...
    if (isset($_POST['um_logs'])) {
        foreach ($GEDCOMS as $key => $gedcom) {
            ...
            if (file_exists($INDEX_DIRECTORY . $gedcom['gedcom'] . 'pgv_counters.php')) {
                $flist[] = $INDEX_DIRECTORY . $gedcom['gedcom'] . 'pgv_counters.php';
            }
            ...
            while ($file = readdir($dir_var)) {
                if ((mb_strpos($file, '.log') > 0) && (false !== mb_strstr($file, 'srch-' . $gedcom['gedcom']))) {
                    $flist[] = $INDEX_DIRECTORY . $file;
                }
            }
        ...
        while ($file = readdir($dir_var)) {
            if ((mb_strpos($file, '.log') > 0) && (false !== mb_strstr($file, 'pgv-'))) {
                $flist[] = $INDEX_DIRECTORY . $file;
            }
        }
    ...
    if (count($flist) > 0) {
        ...
        if (('db' == $PGV_DATABASE) && ('authentication_mysql.php' == $AUTHENTICATION_MODULE) && (isset($_POST['um_usinfo']))) {
...
if (('db' == $PGV_DATABASE) && ('authentication_index.php' == $AUTHENTICATION_MODULE)) {
...
if (('export' == $proceed) || ('exportovr' == $proceed)) {
    print "\n\t<h2>" . $pgv_lang['um_header'] . '</h2>';
...
if (('import' != $proceed) && ('export' != $proceed) && ('exportovr' != $proceed)) {
    print "\n\t<h2>" . $pgv_lang['um_header'] . '</h2>'; ?>
    <br><br>
...
if ('import' == $proceed) {
    ...
    if (false === (file_exists($INDEX_DIRECTORY . 'authenticate.php'))) {
    ...
    if (false === (file_exists($INDEX_DIRECTORY . 'messages.dat'))) {
        ...
        foreach ($messages as $newid => $message) {
            $sql = 'INSERT INTO '
                   . $TBLPREFIX
                   . "messages VALUES ($newid, '"
                   . $DBCONN->escapeSimple($message['from'])
                   . "','"
                   . $DBCONN->escapeSimple($message['to'])
                   . "','"
                   . $DBCONN->escapeSimple($message['subject'])
                   . "','"
                   . $DBCONN->escapeSimple($message['body'])
                   . "','"
                   . $DBCONN->escapeSimple($message['created'])
                   . "')";
    ...
    if (false === (file_exists($INDEX_DIRECTORY . 'favorites.dat'))) {
    ...
    if (false === (file_exists($INDEX_DIRECTORY . 'news.dat'))) {
    ...
    if (false === (file_exists($INDEX_DIRECTORY . 'blocks.dat'))) {
        ...
        foreach ($allblocks as $bid => $blocks) {
            ...
            $sql = 'INSERT INTO '
                        . $TBLPREFIX
                        . "blocks VALUES ($bid, '"
                        . $DBCONN->escapeSimple($blocks['username'])
                        . "', '"
                        . $blocks['location']
                        . "', '"
                        . $blocks['order']
                        . "', '"
                        . $DBCONN->escapeSimple($blocks['name'])
                        . "', '"
                        . $DBCONN->escapeSimple(serialize($blocks['config']))
                        . "')";
