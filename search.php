...
if ('general' == $action) {
    ...
    if ((isset($query)) && ('' != $query)) {
        ...
        if ((1 == count($myindilist)) && (0 == count($myfamlist)) && (0 == count($mysourcelist))) {
            foreach ($myindilist as $pid => $indi) {
                if ($ALLOW_CHANGE_GEDCOM) {
                    $pid = mb_substr($pid, 0, mb_strpos($pid, '['));
                }
        ...
        if ((0 == count($myindilist)) && (1 == count($myfamlist)) && (0 == count($mysourcelist))) {
            foreach ($myfamlist as $famid => $fam) {
                if ($ALLOW_CHANGE_GEDCOM) {
                    $famid = mb_substr($famid, 0, mb_strpos($famid, '['));
                }
        ...
        if ((0 == count($myindilist)) && (0 == count($myfamlist)) && (1 == count($mysourcelist))) {
            foreach ($mysourcelist as $sid => $source) {
                if ($ALLOW_CHANGE_GEDCOM) {
                    $sid = mb_substr($sid, 0, mb_strpos($sid, '['));
                }
...
if ('soundex' == $action) {
    if ((!empty($lastname)) || (!empty($firstname)) || (!empty($place))) {
        ...
        foreach ($geds as $indexval => $value) {
            ...
            if ('index' != $PGV_DATABASE || $value['gedcom'] == $GEDCOM) {
                $GEDCOM = $value['gedcom'];
                ...
                foreach ($indilist as $key => $value) {
                    foreach ($value['names'] as $indexval => $namearray) {
                        ...
                        if (!empty($lastname)) {
                            ...
                            if (soundex($surname) == soundex($lastname)) {
                                $savel = true;
                            }
                        ...
                        if (!empty($firstname)) {
                            ...
                            $pos1 = mb_strpos($namearray[0], '/');

                            if (false === $pos1) {
                                $pos1 = mb_strlen($namearray[0]);
                            }

                            $fname = mb_substr($namearray[0], 0, $pos1);

                            $fnames = preg_preg_split("/\s/", trim($fname));
                            ...
                            if ((true === $savel || empty($lastname)) && true === $savef) {
                                $save = true;
                            } else {
                                $save = false;
                            }
                    ...
                    if ((!empty($place)) || (!empty($year))) {
                        ...
                        if (!empty($place)) {
                            ...
                            if ($pt > 0) {
                                ...
                                for ($pp = 0, $ppMax = count($match[1]); $pp < $ppMax; $pp++) {
                                    $places[$pp] = preg_preg_split("/,\s/", trim($match[1][$pp]));
                                }

                                $cp = count($places);

                                for ($p = 0; $p < $cp; $p++) {
                                    for ($pp = 0, $ppMax = count($places[$p]); $pp < $ppMax; $pp++) {
                                        if (soundex(trim($places[$p][$pp])) == soundex(trim($place))) {
                                            $savep = true;
                                        }
                                    }
                                }
                            ...
                            if ((true === $savel || empty($lastname)) && (true === $savef || empty($firstname)) && true === $savep) {
                                $save = true;
                            } else {
                                $save = false;
                            }
                    ...
                    if ($save) {
                        $sindilist[(string)$key . '[' . $GEDCOM . ']'] = $value;
                    }
                }
            }
        }
...
if ('general' == $action && isset($myquery)) {
    print $myquery;
} else {
    print '';
}
...
if ('general' == $action) {
    if ((isset($query)) && ('' != $query)) {
        ...
        if (count($myindilist) > 0) {
            print '<td class="list_label"><img src="' . $PGV_IMAGE_DIR . '/' . $PGV_IMAGES['indis']['large'] . '" border="0" width="25" alt=""> ' . $pgv_lang['people'] . '</td>';
        }
        ...
        if ($cti > 0) {
            ...
            foreach ($myindilist as $key => $value) {
                if ($ALLOW_CHANGE_GEDCOM && 'index' != $PGV_DATABASE) {
                    $p1 = mb_strpos($key, '[');

                    if (false !== $p1) {
                        $p2 = mb_strpos($key, ']');

                        $GEDCOM = mb_substr($key, $p1 + 1, $p2 - $p1 - 1);

                        $key = mb_substr($key, 0, $p1);
                ...
                if (!displayDetailsByID($key) && showLivingNameByID($key)) {
                    ...
                    if (preg_match('/' . str2upper($query) . '/i', str2upper($record)) > 0) {
                        print_list_person($key, [check_NN(sortable_name_from_name($name)), $value['file']]);
                    } else {
                        $indi_hide++;
                    }
                } else {
        ...
        if ($ctf > 0) {
            ...
            foreach ($myfamlist as $key => $value) {
                ...
                print_list_family($key, [check_NN($value['name']), $value['file']]);
        ...
        if ($cti > 0 || $cts > 0 || $ctf > 0) {
            ...
            if ($cts > 0) {
                print '<td>' . $pgv_lang['total_sources'] . ' ' . $cts . '</td>';
            }
        ...
        print '<br><div class="warning" style=" text-align: center;"><i>' . $pgv_lang['no_results'] . "</i><br></div>\n\t\t";
    }
}
    ...
    if ((!empty($lastname)) || (!empty($firstname)) || (!empty($place))) {
        ...
        if ($ct > 0) {
            ...
            foreach ($sindilist as $key => $value) {
                ...
                print_list_person($key, [check_NN(get_sortable_name($key)), $value['file']]);

                print "\n";

                if ($i == floor($ct / 2) && $ct > 9) {
                    print "\n\t\t</ul></td>\n\t\t<td class=\"list_value_wrap\"><ul>";
                }
