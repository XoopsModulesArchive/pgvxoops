    ...
    foreach ($pgv_changes as $cid => $changes) {
        for ($i = 0, $iMax = count($changes); $i < $iMax; $i++) {
            ...
            if (0 == $i) {
                ...
                if ('INDI' == $type) {
                    print '<b>' . PrintReady(get_person_name($change['gid'])) . '</b> &lrm;(' . $change['gid'] . ")&lrm;<br>\n";
                } elseif ('FAM' == $type) {
                    print '<b>' . PrintReady(get_family_descriptor($change['gid'])) . '</b> &lrm;(' . $change['gid'] . ")&lrm;<br>\n";
                } elseif ('SOUR' == $type) {
                    print '<b>' . PrintReady(get_source_descriptor($change['gid'])) . '</b> &lrm;(' . $change['gid'] . ")&lrm;<br>\n";
                } else {
                    print '<b>' . $factarray[$type] . '</b> &lrm;(' . $change['gid'] . ")&lrm;<br>\n";
                }

                if ('INDI' == $type) {
                    print "<a href=\"#\" onclick=\"return show_diff('individual.php?pid=" . $change['gid'] . '&amp;ged=' . $change['gedcom'] . "&amp;show_changes=yes');\">" . $pgv_lang['view_change_diff'] . "</a> | \n";
                }

                if ('FAM' == $type) {
                    print "<a href=\"#\" onclick=\"return show_diff('family.php?famid=" . $change['gid'] . '&amp;ged=' . $change['gedcom'] . "&amp;show_changes=yes');\">" . $pgv_lang['view_change_diff'] . "</a> | \n";
                }

                if ('SOUR' == $type) {
                    print "<a href=\"#\" onclick=\"return show_diff('source.php?sid=" . $change['gid'] . '&amp;ged=' . $change['gedcom'] . "&amp;show_changes=yes');\">" . $pgv_lang['view_change_diff'] . "</a> | \n";
                }
