...
if (($level < 0) || ($level > $MEDIA_DIRECTORY_LEVELS)) {
    ...
    $level = 0;
} elseif (0 == preg_match("'^$MEDIA_DIRECTORY'", $directory)) {
    $directory = $MEDIA_DIRECTORY;
...
<span class="subheaders"><?php print $thistitle; ?>
    <?php
    if ($embed) {
        print_help_link('manage_media_help', 'qm');
    } else {
        print_help_link('find_media_help', 'qm');
    }
    ?>
</span>
    </div>
    ...
    <form action="findmedia.php" method="GET">
        ...
        <div class="center">
            <table class="list-table">
                ...
                <tr>
                    <td>
                    <td class="list-label <?php print $TEXT_DIRECTION; ?>"><input type="checkbox" name="showthumb" value="true" <?php if ($showthumb) {
        print 'checked="checked"';
    } ?>" onclick="javascript: this.form.submit()"><?php print $pgv_lang['show_thumbnail']; ?>
...
$badmedia = ['.', '..', 'CVS', 'thumbs', 'index.php', 'MediaInfo.txt'];
if (empty($action)) {
    $action = 'filter';
}
...
if ('newdir' == $action) {
    ...
    for ($i = 0, $iMax = count($badchars); $i < $iMax; $i++) {
        $pos = mb_strpos($newdir, $badchars[$i]);
    ...
    if ($clean) {
        ...
        if (file_exists($directory . 'index.php')) {
            ...
            $fp = @fopen($directory . $newdir . '/index.php', 'w+b');
        ...
        if (file_exists($thumbdir . 'index.php')) {
            ...
            $fp = @fopen($thumbdir . $newdir . '/index.php', 'w+b');
...
if ('filter' == $action) {
    ...
    if ($level > 0) {
        $levels = preg_preg_split("'/'", $directory);
        ...
        $levels = preg_preg_split("'/'", $thumbdir);
    ...
    while (false !== ($media = $d->read())) {
        if (!in_array($media, $badmedia, true)) {
    ...
    if (count($images)) {
        ...
        foreach ($images as $indexval => $imag) {
            ...
            if ($applyfilter) {
                $isvalid = (false !== mb_strpos(str2lower($imag), str2lower($filter)));
            } else {
                $isvalid = true;
            }
