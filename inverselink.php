...
} elseif ('source' == $linkto) {
    $toitems = $pgv_lang['to_source'];
} elseif ('family' == $linkto) {
...
if (1 == preg_match("/M\d{4,8}/", $mediaid, $matches)) {
    $mediaid = $matches[0];
} else {
    $paramok = false;
}
...
if ('choose' == $action && $paramok) {
    ...
    if ($indirec) {
        ...
        for ($i = 0, $iMax = count($tag); $i < $iMax; $i++) {
            if ('' != $text[$i]) {
                // we don't have linked notes yet

                if ('NOTE' == $tag[$i]) {
                    $mediarec .= textblock_to_note($glevels[$i], $text[$i]);
                } elseif ('1' == $islink[$i]) { // source link
                    $mediarec .= $glevels[$i] . ' ' . $tag[$i] . ' @' . $text[$i] . "@\r\n";
                } else {
                    $mediarec .= $glevels[$i] . ' ' . $tag[$i] . ' ' . $text[$i] . "\r\n";
                }
            } elseif ('2' == $glevels[$i]) { // skip any sub fields if empty
                while (($i < count($tag) - 1) && ('2' != $glevels[$i + 1])) {
                    $i += 1;
                }
            }
        }
