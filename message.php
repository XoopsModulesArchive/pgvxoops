...
if ('all' == $to && !userIsAdmin(getUserName())) {
    print '<span class="error">' . $pgv_lang['no_to_user'] . '</span><br>';
...
if (('send' == $action) && (isset($_SESSION['good_to_send'])) && (true === $_SESSION['good_to_send'])) {
    ...
    if (!isset(Request::getString('HTTP_REFERER', '', 'SERVER')) || false === mb_stristr(Request::getString('HTTP_REFERER', '', 'SERVER'), 'message.php')) {
    ...
    if ('compose' != $action) {
        ...
        foreach ($toarray as $indexval => $to) {
            ...
            if (addMessage($message)) {
                print $pgv_lang['message_sent'] . ' - ' . $to;
            }
...
if ('compose' == $action) {
    ...
    if (empty($username)) {
        print 'return validateEmail(document.messageform.from_email);';
    } else {
        print 'return checkForm(this);';
    }
    ...
    if (empty($username)) {
        ...
        print "<td><input type=\"text\" name=\"from_name\" size=\"40\" value=\"$from_name\"></td></tr><tr><td valign=\"top\" align=\"right\">"
              . $pgv_lang['message_from']
              . "</td><td><input type=\"text\" name=\"from_email\" size=\"40\" value=\"$from_email\"><br>"
    ...
    if ('messaging2' == $method) {
        print $pgv_lang['messaging2_help'];
    }
} elseif ('delete' == $action) {
    if (deleteMessage($id)) {
        print $pgv_lang['message_deleted'];
    }
}
