    ...
    for ($i = 1; $i < 6; $i++) {
        ...
        if (!empty($_FILES['mediafile' . $i]['name'])) {
            ...
            if (!move_uploaded_file($_FILES['mediafile' . $i]['tmp_name'], $MEDIA_DIRECTORY . basename($_FILES['mediafile' . $i]['name']))) {
                ...
                if (!empty($_POST['genthumb' . $i]) && ('yes' == $_POST['genthumb' . $i])) {
                    ...
                    if (!$thumbgenned) {
                        $error .= $pgv_lang['thumbgen_error'] . $filename . '<br>';
                    } else {
                        print $thumbnail . ' ' . $pgv_lang['thumb_genned'] . '<br>';
                    }
                }
            }
...
if (!is_writable($MEDIA_DIRECTORY) || !$MULTI_MEDIA) {
    ...
    for ($i = 1; $i < 6; $i++) {
        ...
        if ('' != $ThumbSupport) {
            $ThumbSupport = mb_substr($ThumbSupport, 2);    // Trim off first ", "
