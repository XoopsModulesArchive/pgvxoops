    ...
    if ('preview' != $view) {
        if (isset($pgv_changes[$sid . '_' . $GEDCOM])) {
            if (!isset($show_changes)) {
                ...
                if (userCanAccept(getUserName())) {
                    print "<a href=\"source.php?sid=$sid&amp;action=accept\">" . $pgv_lang['accept_all'] . '</a> | ';
                }
...
if (!empty($newsource)) {
    ...
    for ($i = 1; $i <= $lct; $i++) {
        ...
        if (($i == $lct) || (1 == $line[0])) {
            $newsourcefacts[] = [$factrec, $linenum];
...
//-- new fact link
if (('preview' != $view) && (userCanEdit(getUserName())) && ($display)) {
    $addfacts = array_merge(CheckFactUnique(['AUTH', 'ABBR', 'TITL', 'PUBL', 'TEXT'], $sourcefacts, 'SOUR'), ['NOTE', 'OBJE', 'REPO']);
...
if (($ci > 0) || ($cf > 0)) {
    ...
    if ($ci > 0) {
        print '<td class="list_label">' . $pgv_lang['individuals'] . "</td>\n";
    }
    ...
    if (count($myfamlist) > 0) {
        ...
        if ($cf > 0) {
            ...
            foreach ($myfamlist as $key => $value) {
                $name = preg_replace(["/ [jJsS][rR]\.?,/", '/ I+,/', '/^[a-z. ]*/'], [',', ',', ''], $value['name']);

                $names = preg_preg_split('/[,+]/', $name);

                $value['name'] = check_NN($names);

                print_list_family($key, [get_family_descriptor($key), $value['file']]);
    ...
    print '&nbsp;&nbsp;&nbsp;<span class="warning"><i>' . $pgv_lang['no_results'] . '</span>';
}
