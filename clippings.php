...
function add_clipping($clipping)
{
    ...
    if ((false === $clipping['id']) || ('' == $clipping['id'])) {
        return false;
    }

    if (!id_in_cart($clipping['id'])) {
        if ('indi' == $clipping['type']) {
            ...
            if (displayDetailsByID($clipping['id'], mb_strtoupper($clipping['type']))) {
                $cart[] = $clipping;
            } else {
                return false;
            }
...
if ('add1' == $action) {
    ...
    if ($ret) {
        if ('fam' == $type) {
            ...
            } elseif ('members' == $others) {
                add_family_members($id);
            } elseif ('descendants' == $others) {
            ...
            } elseif ('ancestors' == $others) {
                add_ancestors($id);
            } elseif ('ancestorsfamilies' == $others) {
    ...
    if ('/' == $SERVER_URL[mb_strlen($SERVER_URL) - 1]) {
        $dSERVER_URL = mb_substr($SERVER_URL, 0, -1);
    } else {
    ...
    for ($i = 0; $i < $ct; $i++) {
        ...
        if ('indi' == $clipping['type']) {
            ...
            for ($k = 0; $k < $ft; $k++) {
                ...
                $filename = mb_substr($match[$k][1], mb_strrpos($match[$k][1], '\\'));
    ...
    if ($mediacount > 0) {
        ...
        for ($m = 0; $m < $mediacount; $m++) {
            print '<li><a href="' . $MEDIA_DIRECTORY . "$media[$m]\">" . mb_substr($media[$m], mb_strrpos($media[$m], '/')) . '</a></li>';
        }
...
if (0 == $ct) {
    ...
    print "\r\n\t<table class=\"list_table\">\r\n\t\t<tr>\r\n\t\t\t<td class=\"list_label\">"
          . $pgv_lang['type']
          . '</td><td class="list_label">'
          . $pgv_lang['id']
          . '</td><td class="list_label">'
          . $pgv_lang['name_description']
          . '</th><td class="list_label">'
          . $pgv_lang['remove']
          . "</td>\r\n\t\t</tr>";
    ...
    for ($i = 0; $i < $ct; $i++) {
        ...
        if ('indi' == $clipping['type']) {
            print '<img src="' . $PGV_IMAGE_DIR . '/' . $PGV_IMAGES['indis']['large'] . '" border="0">';
        } elseif ('fam' == $clipping['type']) {
            print '<img src="' . $PGV_IMAGE_DIR . '/' . $PGV_IMAGES['sfamily']['large'] . '" border="0">';
        } elseif ('source' == $clipping['type']) {
            print '<img src="' . $PGV_IMAGE_DIR . '/' . $PGV_IMAGES['source']['large'] . '" border="0">';
        }
        ...
        if ('indi' == $clipping['type']) {
            ...
            if ('fam' == $clipping['type']) {
                ...
                if (($husb_ok) and ($wife_ok)) {
                    $dName = get_family_descriptor($clipping['id']);
                } else {
                    $dName = $pgv_lang['family_private'];
                }
    ...
    if ('download' != $action) {
        ...
        <table>
            <tr>
                <td><input type="checkbox" name="convert" value="yes"></td>
                <td><?php print $pgv_lang['utf8_to_ansi'];

        print_help_link('utf8_ansi_help', 'qm'); ?></td>
            </tr>
            <tr>
                <td><input type="checkbox" name="remove" value="yes" checked></td>
                <td><?php print $pgv_lang['remove_custom_tags'];

        print_help_link('remove_tags_help', 'qm'); ?></td>
            </tr>
        </table>
