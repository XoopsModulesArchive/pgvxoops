...
if (userIsAdmin(getUserName()) && 'generate' == $action && !empty($file) && !empty($thumb)) {
...
if ($ct > $max) {
    ...
    print '<td align="' . ('ltr' == $TEXT_DIRECTION ? 'left' : 'right') . '">';
    ...
    print '</td><td align="' . ('ltr' == $TEXT_DIRECTION ? 'right' : 'left') . '">';
...
for ($i = 0; $i < $count; $i++) {
    ...
    if ($MEDIA_EXTERNAL && (mb_strstr($value['file'], '://') || mb_stristr($value['file'], 'mailto:'))) {
    ...
    } elseif (file_exists(filename_decode($value['file']))) {
    ...
    if (userIsAdmin(getUserName()) && $nothumb && function_exists('imagecreatefromjpeg') && function_exists('imagejpeg')) {
        if ((!mb_strstr($value['file'], 'mailto:')) && (file_exists(filename_decode($value['file'])) || (mb_strstr($value['file'], '://')))) {
            ...
            if ($ct > 0) {
                ...
                if ('jpg' == $ext || 'jpeg' == $ext) {
                    print "<a href=\"medialist.php?action=generate&amp;max=$max&amp;start=$start&amp;file=" . urlencode($value['file']) . '&amp;thumb=' . urlencode($value['thumb']) . '">' . $pgv_lang['generate_thumbnail'] . 'JPG</a><br>';
                }

                if ('gif' == $ext && function_exists('imagecreatefromgif') && function_exists('imagegif')) {
                    print "<a href=\"medialist.php?action=generate&amp;max=$max&amp;start=$start&amp;file=" . urlencode($value['file']) . '&amp;thumb=' . urlencode($value['thumb']) . '">' . $pgv_lang['generate_thumbnail'] . 'GIF</a><br>';
                }

                if ('png' == $ext && function_exists('imagecreatefrompng') && function_exists('imagepng')) {
                    print "<a href=\"medialist.php?action=generate&amp;max=$max&amp;start=$start&amp;file=" . urlencode($value['file']) . '&amp;thumb=' . urlencode($value['thumb']) . '">' . $pgv_lang['generate_thumbnail'] . 'PNG</a><br>';
                }
            }
        }
    }
    ...
    if ((!mb_strstr($value['file'], '://')) && (!mb_strstr($value['file'], 'mailto:')) && (!file_exists(filename_decode($value['file'])))) {
