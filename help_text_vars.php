...
if (mb_strstr($_SERVER['PHP_SELF'], 'help_text_vars')) {
...
$vpos = mb_strpos($pgv_lang['enter_terms'], ':', 0);
if ($vpos > 0) {
    $enter_terms = mb_substr($pgv_lang['enter_terms'], 0, $vpos);
} else {
    $enter_terms = $pgv_lang['enter_terms'];
}
$vpos = mb_strpos($pgv_lang['soundex_search'], ':', 0);
if ($vpos > 0) {
    $soundex_search = mb_substr($pgv_lang['soundex_search'], 0, $vpos);
} else {
    $soundex_search = $pgv_lang['soundex_search'];
}
