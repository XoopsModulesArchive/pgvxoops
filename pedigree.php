...
function adjust_subtree($index, $diff)
{
    ...
    if (!$SHOW_EMPTY_BOXES && empty($treeid[$index])) {
        return;
    }
...
function collapse_tree($index, $curgen, $diff)
{
    ...
    if (($curgen < ($PEDIGREE_GENERATIONS - 1)) || (1 == $index % 2)) {
        $diff = $fdiff;
    }
...
if ('preview' == $view) {
    print '<h2>' . str_replace('#PEDIGREE_GENERATIONS#', convert_number($PEDIGREE_GENERATIONS), $pgv_lang['gen_ped_chart']) . ':';
} else {
    print '<h2>' . $pgv_lang['index_header'] . ':';
}
//print "<br>".$name."</h2>";
...
if (($PEDIGREE_GENERATIONS < 5) && (false === $show_full)) {
...
if (3 == $PEDIGREE_GENERATIONS) {
    $baseyoffset += $pbheight * 1.6;
}
if ((true === isset($max_generation)) || (true === isset($min_generation))) {
    $baseyoffset += 20;
}
...
$minyoffset = 0;
for ($i = 0, $iMax = count($treeid); $i < $iMax; $i++) {
    if ($SHOW_EMPTY_BOXES || !empty($treeid[$i])) {
        if (!empty($offsetarray[$i])) {
            if ((0 == $minyoffset) || ($minyoffset > $offsetarray[$i]['y'])) {
                $minyoffset = $offsetarray[$i]['y'];
            }
        }
    }
}
...
if (('preview' != $view) && ($offsetarray[0]['y'] + $baseyoffset < 300)) {
    adjust_subtree(0, 300 - ($offsetarray[0]['y'] + $baseyoffset));
}
...
for ($i = ($treesize - 1); $i >= 0; $i--) {
    ...
    if (!empty($treeid[$i]) || $SHOW_EMPTY_BOXES) {
        ...
        if ((1 == $curgen) && (!empty($treeid[$i])) && (count(find_family_ids($treeid[$i])) > 0)) {
            $widthadd = 20;
        }
        ...
        if ('rtl' == $TEXT_DIRECTION) {
            print '.0" style="position:absolute; right:';
        } else {
            print '.0" style="position:absolute; left:';
        }
        ...
        if ((1 == $curgen) && (count(find_family_ids($treeid[$i])) > 0)) {
            $did = 1;

            if ($i > ($treesize / 2) + ($treesize / 4) - 1) {
                $did++;
            }
...
if (displayDetails($indirec) || showLivingName($indirec)) {
    ...
    if ($famids || ($num > 1)) {
        ...
        if ('preview' != $view) {
            if ('rtl' == $TEXT_DIRECTION) {
                print "<a href=\"#\" onclick=\"return togglechildrenbox();\" onmouseover=\"swap_image('larrow',1);\" onmouseout=\"swap_image('larrow',1);\">";
            } else {
                print "<a href=\"#\" onclick=\"return togglechildrenbox();\" onmouseover=\"swap_image('larrow',0);\" onmouseout=\"swap_image('larrow',0);\">";
            }

            if ('rtl' == $TEXT_DIRECTION) {
                print '<img id="larrow" src="' . $PGV_IMAGE_DIR . '/' . $PGV_IMAGES['rarrow']['other'] . '" border="0" alt="">';
            } else {
                print '<img id="larrow" src="' . $PGV_IMAGE_DIR . '/' . $PGV_IMAGES['larrow']['other'] . '" border="0" alt="">';
            }
