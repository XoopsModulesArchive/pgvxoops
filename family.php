...
if ($showLivingHusb) {
    ...
    if ('Individual ' !== get_person_name($parents['HUSB'])) {
        $title = get_person_name($parents['HUSB']);

        $none = 0;
    }

    if ($showLivingWife && ('Individual ' !== get_person_name($parents['WIFE']))) {
        if (0 == $none) {
            $title .= ' + ';
        }
...
if ($none) {
    print_header($pgv_lang['family_info']);
}
if (empty($parents['HUSB']) || empty($parents['WIFE'])) {
    $link_relation = 0;
} else {
    $link_relation = 1;
}
        ...
        if (isset($pgv_changes[$famid . '_' . $GEDCOM])) {
            ...
            if (userCanAccept(getUserName())) {
                ...
                if ('index' != $PGV_DATABASE) {
                    $submenu['link'] = "family.php?famid=$famid&amp;action=accept";
                } else {
                    $submenu['link'] = '#';
