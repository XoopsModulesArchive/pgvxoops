...
// Remove slashes
if (isset($ufullname)) {
    $ufullname = stripslashes($ufullname);
}
...
if ('createuser' == $action) {
    ...
    while (mb_strlen($uusername) > $i) {
        if (true !== mb_stristr($alphabet, $uusername[$i])) {
            $pass = false;
    ...
    if (true === $pass) {
        if (false !== getUser($uusername)) {
            ...
            foreach ($GEDCOMS as $ged => $gedarray) {
                ...
                $ged = preg_replace(["/\./", '/-/', '/ /'], ['_', '_', '_'], $ged);

                $varname = "gedcomid_$ged";

                if (isset($$varname)) {
                    $user['gedcomid'][$file] = $$varname;
                }

                $varname = "rootid_$ged";

                if (isset($$varname)) {
                    $user['rootid'][$file] = $$varname;
                }

                $varname = "canedit_$ged";

                if (isset($$varname)) {
                    $user['canedit'][$file] = $$varname;
                } else {
                    $user['canedit'][$file] = 'none';
                }
            }

            $user['password'] = crypt($pass1);

            if ((isset($canadmin)) && ('yes' == $canadmin)) {
                $user['canadmin'] = true;
            } else {
                $user['canadmin'] = false;
            }

            if ((isset($visibleonline)) && ('yes' == $visibleonline)) {
                $user['visibleonline'] = true;
            } else {
                $user['visibleonline'] = false;
            }

            if ((isset($editaccount)) && ('yes' == $editaccount)) {
                $user['editaccount'] = true;
            } else {
                $user['editaccount'] = false;
            }

            if (!isset($new_user_theme)) {
                $new_user_theme = '';
            }
            ...
            if (!isset($new_contact_method)) {
                $new_contact_method = 'messaging2';
            }

            $user['contactmethod'] = $new_contact_method;

            if (isset($new_default_tab)) {
                $user['default_tab'] = $new_default_tab;
            }
...
if ('edituser2' == $action) {
    ...
    if (true === $pass) {
        if (($uusername != $oldusername) && (false !== getUser($uusername))) {
            print '<span class="error">' . $pgv_lang['duplicate_username'] . '</span><br>';
            ...
            if (!empty($user_language)) {
                $newuser['language'] = $user_language;
            }
            ...
            if (!empty($new_contact_method)) {
                $newuser['contactmethod'] = $new_contact_method;
            }

            if (isset($new_default_tab)) {
                $newuser['default_tab'] = $new_default_tab;
            }
            ...
            foreach ($GEDCOMS as $ged => $gedarray) {
                ...
                if (isset($$varname)) {
                    $newuser['gedcomid'][$file] = $$varname;
                }

                $varname = "rootid_$ged";

                if (isset($$varname)) {
                    $newuser['rootid'][$file] = $$varname;
                }

                $varname = "canedit_$ged";

                if (isset($$varname)) {
                    $newuser['canedit'][$file] = $$varname;
                } else {
                    $user['canedit'][$file] = 'none';
                }
            }
            ...
            if ($olduser['username'] != getUserName()) {
                ...
                $newuser['canadmin'] = $olduser['canadmin'];
            }

            if ((isset($visibleonline)) && ('yes' == $visibleonline)) {
                $newuser['visibleonline'] = true;
            } else {
                $newuser['visibleonline'] = false;
            }

            if ((isset($editaccount)) && ('yes' == $editaccount)) {
                $newuser['editaccount'] = true;
            } else {
            ...
            if (($olduser['verified_by_admin'] != $newuser['verified_by_admin']) && (!empty($newuser['verified_by_admin']))) {
                ...
                if ('/' == mb_substr($SERVER_URL, -1)) {
                    $message['subject'] = str_replace('#SERVER_NAME#', mb_substr($SERVER_URL, 0, (mb_strlen($SERVER_URL) - 1)), $pgv_lang['admin_approved']);

                    $message['body'] = str_replace('#SERVER_NAME#', $SERVER_URL, $pgv_lang['admin_approved']) . $pgv_lang['you_may_login'] . "\r\n\r\n" . mb_substr($SERVER_URL, 0, (mb_strlen($SERVER_URL) - 1)) . "/index.php?command=user\r\n";
                } else {
...
if ('edituser' == $action) {
    ...
    if (!isset($user['contactmethod'])) {
        $user['contactmethod'] = 'none';
    }
    ...
    <form name="editform" method="post" action="useradmin.php" onsubmit="return checkform(this);">
        ...
        <table class="list_table <?php print $TEXT_DIRECTION; ?>" border="0">
            <tr>
                <td class="facts_label"><?php print $pgv_lang['username'];

    print_help_link('useradmin_username_help', 'qm') ?></td>
                <td class="facts_value"><input type="text" name="uusername" value="<?php print $user['username'] ?>"></td>
            </tr>
            <tr>
                <td class="facts_label"><?php print $pgv_lang['fullname'];

    print_help_link('useradmin_fullname_help', 'qm') ?></td>
                <td class="facts_value"><input type="text" name="ufullname" value="<?php print PrintReady($user['fullname']) ?>" size="50"></td>
            </tr>
            <tr>
                <td class="facts_label"><?php print $pgv_lang['gedcomid'];

    print_help_link('useradmin_gedcomid_help', 'qm') ?></td>
                <td class="facts_value">
                    <table align="<?php print 'rtl' == $TEXT_DIRECTION ? 'right' : 'left'; ?>">
                        <?php
                        foreach ($GEDCOMS
                        as $ged => $gedarray) {
                            $file = $ged;
    ...
    <tr>
      <td class=" facts_label"><?php print $pgv_lang['rootid'];

    print_help_link('useradmin_rootid_help', 'qm') ?></td>
                            <td class="facts_value">
                                <table align="<?php print 'rtl' == $TEXT_DIRECTION ? 'right' : 'left'; ?>">
                                    <?php
                                    foreach ($GEDCOMS
    ...
    <tr>
      <td class=" facts_label"><?php print $pgv_lang['password'];

    print_help_link('useradmin_password_help', 'qm') ?></td>
                                        <td class="facts_value"><input type="password" name="pass1"><br><?php print $pgv_lang['leave_blank']; ?></td>
                                    </tr>
                                    <tr>
                                        <td class="facts_label"><?php print $pgv_lang['confirm'];

    print_help_link('useradmin_conf_password_help', 'qm') ?></td>
                                        <td class="facts_value"><input type="password" name="pass2"></td>
                                    </tr>
                                    <tr>
                                        <td class="facts_label"><?php print $pgv_lang['can_admin'];

    print_help_link('useradmin_can_admin_help', 'qm') ?></td>
                                        <td class="facts_value"><input type="checkbox" name="canadmin" value="yes" <?php if ($user['canadmin']) {
    ...
    if ($user['username'] == getUserName()) {
        print ' disabled="disabled"';
                                    ...
                                    <tr>
                                        <td class="facts_label"><?php print $pgv_lang['can_edit'];

    print_help_link('useradmin_can_edit_help', 'qm') ?></td>
                                        <td class="facts_value">
                                            <?php
                                            foreach ($GEDCOMS as $ged => $gedarray) {
                                                $file = $ged;
                                                ...
                                                if (isset($user['canedit'][$file])) {
                                                    if (true === $user['canedit'][$file]) {
                                                        $user['canedit'][$file] = 'yes';
                                                    }
                                                } else {
                                                    $user['canedit'][$file] = 'no';
                                                }
                                    ...
                                    <tr>
                                        <td class="facts_label"><?php print $pgv_lang['emailadress'];

    print_help_link('useradmin_email_help', 'qm') ?></td>
                                        <td class="facts_value"><input type="text" name="emailadress" value="<?php print $user['email'] ?>" size="50"></td>
                                    </tr>
                                    <tr>
                                        <td class="facts_label"><?php print $pgv_lang['verified'];

    print_help_link('useradmin_verified_help', 'qm') ?></td>
                                        <td class="facts_value"><input type="checkbox" name="verified" value="yes" <?php if ($user['verified']) {
        print 'checked="checked"';
    } ?>></td>
                                    </tr>
                                    <tr>
                                        <td class="facts_label"><?php print $pgv_lang['verified_by_admin'];

    print_help_link('useradmin_verbyadmin_help', 'qm') ?></td>
                                        <td class="facts_value"><input type="checkbox" name="verified_by_admin" value="yes" <?php if ($user['verified_by_admin']) {
        print 'checked="checked"';
    } ?>></td>
                                    </tr>
                                    <?php /* added by Kurt - end */ ?>
                                    <tr>
                                        <td class="facts_label"><?php print $pgv_lang['change_lang'];

    print_help_link('edituser_change_lang_help', 'qm'); ?></td>
                                        <td class="facts_value" valign="top"><?php
                                    ...
                                    <?php if ($ALLOW_USER_THEMES) { ?>
                                        <tr>
                                            <td class="facts_label" valign="top" align="left"><?php print $pgv_lang['user_theme'];
                                                print_help_link('useradmin_user_theme_help', 'qm'); ?></td>
                                    ...
                                    <?php } ?>
                                    <tr>
                                        <td class="facts_label"><?php print $pgv_lang['user_contact_method'];

    print_help_link('useradmin_user_contact_help', 'qm') ?></td>
                                        <td class="facts_value"><select name="new_contact_method">
                                                <?php if ($PGV_STORE_MESSAGES) { ?>
                                                    <option value="messaging" <?php if ('messaging' == $user['contactmethod']) {
        print 'selected="selected"';
    } ?>><?php print $pgv_lang['messaging']; ?></option>
                                                    <option value="messaging2" <?php if ('messaging2' == $user['contactmethod']) {
        print 'selected="selected"';
    } ?>><?php print $pgv_lang['messaging2']; ?></option>
                                                <?php } else { ?>
                                                    <option value="messaging3" <?php if ('messaging3' == $user['contactmethod']) {
        print 'selected="selected"';
    } ?>><?php print $pgv_lang['messaging3']; ?></option>
                                                <?php } ?>
                                                <option value="mailto" <?php if ('mailto' == $user['contactmethod']) {
        print 'selected="selected"';
    } ?>><?php print $pgv_lang['mailto']; ?></option>
                                                <option value="none" <?php if ('none' == $user['contactmethod']) {
        print 'selected="selected"';
                                    ...
                                    <tr>
                                        <td class="facts_label"><?php print $pgv_lang['visibleonline'];

    print_help_link('useradmin_visibleonline_help', 'qm') ?></td>
                                        <td class="facts_value"><input type="checkbox" name="visibleonline" value="yes" <?php if ($user['visibleonline']) {
        print 'checked="checked"';
    } ?>></td>
                                    </tr>
                                    <tr>
                                        <td class="facts_label"><?php print $pgv_lang['editaccount'];

    print_help_link('useradmin_editaccount_help', 'qm') ?></td>
                                        <td class="facts_value"><input type="checkbox" name="editaccount" value="yes" <?php if ($user['editaccount']) {
        print 'checked="checked"';
    } ?>></td>
                                    </tr>
                                    <tr>
                                        <td class="facts_label"><?php print $pgv_lang['user_default_tab'];

    print_help_link('useradmin_user_default_tab_help', 'qm') ?></td>
                                        <td class="facts_value"><select name="new_default_tab">
                                                <option value="0" <?php if (0 == @$user['default_tab']) {
        print 'selected="selected"';
    } ?>><?php print $pgv_lang['personal_facts']; ?></option>
                                                <option value="1" <?php if (1 == @$user['default_tab']) {
        print 'selected="selected"';
    } ?>><?php print $pgv_lang['notes']; ?></option>
                                                <option value="2" <?php if (2 == @$user['default_tab']) {
        print 'selected="selected"';
    } ?>><?php print $pgv_lang['ssourcess']; ?></option>
                                                <option value="3" <?php if (3 == @$user['default_tab']) {
        print 'selected="selected"';
    } ?>><?php print $pgv_lang['media']; ?></option>
                                                <option value="4" <?php if (4 == @$user['default_tab']) {
        print 'selected="selected"';
...
?>
    ...
    <table class="list_table <?php print $TEXT_DIRECTION; ?>">
        ...
        foreach ($users as $username => $user) {
            ...
            if (!isset($language_settings[$user['language']])) {
                $user['language'] = $LANGUAGE;
            }
            ...
            if ('ltr' == $TEXT_DIRECTION) {
                print "\t<td class=\"list_value_wrap\"><a href=\"useradmin.php?action=deleteuser&amp;username=" . urlencode($username) . "\" onclick=\"return confirm('" . $pgv_lang['confirm_user_delete'] . " $username?');\">" . $pgv_lang['delete'] . "</a></td>\n";
            } elseif (begRTLText($username)) {
                print "\t<td class=\"list_value_wrap\"><a href=\"useradmin.php?action=deleteuser&amp;username=" . urlencode($username) . "\" onclick=\"return confirm('?" . $pgv_lang['confirm_user_delete'] . " $username');\">" . $pgv_lang['delete'] . "</a></td>\n";
            } else {
                print "\t<td class=\"list_value_wrap\"><a href=\"useradmin.php?action=deleteuser&amp;username=" . urlencode($username) . "\" onclick=\"return confirm('?$username " . $pgv_lang['confirm_user_delete'] . " ');\">" . $pgv_lang['delete'] . "</a></td>\n";
            }
            ...
            if ('ltr' == $TEXT_DIRECTION) {
                print "\t<td class=\"list_value_wrap\">" . $user['fullname'] . "&lrm;</td>\n";
            } else {
                print "\t<td class=\"list_value_wrap\">" . $user['fullname'] . "&rlm;</td>\n";
            }

            print "\t<td class=\"list_value_wrap\">"
                  . $pgv_lang['lang_name_' . $user['language']]
                  . '<br><img src="'
                  . $language_settings[$user['language']]['flagsfile']
                  . '" class="flag" alt="'
                  . $pgv_lang['lang_name_' . $user['language']]
                  . '" title="'
                  . $pgv_lang['lang_name_'
            ...
            foreach ($GEDCOMS as $gedid => $gedcom) {
                $vval = $user['canedit'][$gedid] ?? 'none';

                if ('' == $vval) {
                    $vval = 'none';
                }

                $uged = $user['gedcomid'][$gedid] ?? '';

                if ('accept' == $vval || 'admin' == $vval) {
                    print '<li class="warning">';
                } else {
                    print '<li>';
                }
                ...
                if ('' != $uged) {
                    print '<a href="individual.php?pid=' . $uged . '&ged=' . $gedid . '">' . $gedid . "</a></li>\n";
                } else {
                    print $gedid . "</li>\n";
                }
            }
    ...
    <form name="newform" method="post" action="<?php print $PHP_SELF; ?>" onsubmit="return checkform(this);">
        ...
        <table class="list_table <?php print $TEXT_DIRECTION; ?>">
            <tr>
                <td class="facts_label"><?php print $pgv_lang['username'];
                    print_help_link('useradmin_username_help', 'qm') ?></td>
                <td class="facts_value"><input type="text" name="uusername" tabindex="1"></td>
            </tr>
            <tr>
                <td class="facts_label"><?php print $pgv_lang['fullname'];
                    print_help_link('useradmin_fullname_help', 'qm') ?></td>
                <td class="facts_value"><input type="text" name="ufullname" tabindex="2" size="50"></td>
            </tr>
            <tr>
                <td class="facts_label"><?php print $pgv_lang['gedcomid'];
                    print_help_link('useradmin_gedcomid_help', 'qm') ?></td>
                            ...
                            <td class="facts_value">
                                <table align="<?php print 'rtl' == $TEXT_DIRECTION ? 'right' : 'left'; ?>">
                                    ...
                                    <tr>
                                        <td class="facts_label"><?php print $pgv_lang['password'];
                                            print_help_link('useradmin_password_help', 'qm') ?></td>
                                        <td class="facts_value"><input type="password" name="pass1" tabindex="5"></td>
                                    </tr>
                                    <tr>
                                        <td class="facts_label"><?php print $pgv_lang['confirm'];
                                            print_help_link('useradmin_conf_password_help', 'qm') ?></td>
                                        <td class="facts_value"><input type="password" name="pass2" tabindex="6"></td>
                                    </tr>
                                    <tr>
                                        <td class="facts_label"><?php print $pgv_lang['can_admin'];
                                            print_help_link('useradmin_can_admin_help', 'qm') ?></td>
                                        <td class="facts_value"><input type="checkbox" name="canadmin" value="yes" tabindex="7"></td>
                                    </tr>
                                    <tr>
                                        <td class="facts_label"><?php print $pgv_lang['can_edit'];
                                            print_help_link('useradmin_can_edit_help', 'qm') ?></td>
                                    ...
                                    <tr>
                                        <td class="facts_label"><?php print $pgv_lang['emailadress'];
                                            print_help_link('useradmin_email_help', 'qm') ?></td>
                                        <td class="facts_value"><input type="text" name="emailadress" value="" size="50" tabindex="9"></td>
                                    </tr>
                                    <tr>
                                        <td class="facts_label"><?php print $pgv_lang['verified'];
                                            print_help_link('useradmin_verified_help', 'qm') ?></td>
                                        <td class="facts_value"><input type="checkbox" name="verified" value="yes" tabindex="10" checked></td>
                                    </tr>
                                    <tr>
                                        <td class="facts_label"><?php print $pgv_lang['verified_by_admin'];
                                            print_help_link('useradmin_verbyadmin_help', 'qm') ?></td>
                                        <td class="facts_value"><input type="checkbox" name="verified_by_admin" value="yes" tabindex="11" checked></td>
                                    </tr>
                                    <?php /* added by Kurt - end */ ?>
                                    <tr>
                                        <td class="facts_label"><?php print $pgv_lang['change_lang'];
                                            print_help_link('useradmin_change_lang_help', 'qm'); ?></td>
                                        <td class="facts_value" valign="top"><?php
                                            if ($ENABLE_MULTI_LANGUAGE) {
                                                print '<select name="user_language" style="{ font-size: 9pt; }">';
